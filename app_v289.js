...
function loadState(){
  ...
  // Emergency fallback only. Normal v2.8.9 operation never scans all legacy copies.
...
function save(){
  try{
    // v2.8.9: mark the active project for deterministic multi-device cloud sync.
    try{
...
$('deleteProject').onclick=()=>{let p=cur();if(p&&confirm('Projekt wirklich löschen?')){try{window.ProjectBauOneDrive?.recordProjectDeletion?.(p.id)}catch(_){} S.projects=S.projects.filter(x=>x.id!==p.id);A=null;save()}};
...
function drawFloorplan(preview=null){
  ...
  try{
    ...
    // v2.8.9: Walls are always rendered first. This prevents sanitary/furniture
...
function drawFpObject(o,preview=false){
  ...
  if(o.type==='wall'){
    // v2.8.9: authoritative robust wall pass.
...
/* v2.8.9 – authoritative runtime version stamp */
(()=>{
  const VERSION='2.8.9 PRO';
