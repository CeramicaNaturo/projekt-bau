...
function nearestWallEndpoint(p,maxDistance=null){
  // v2.9.10: endpoint magnet is based on SCREEN distance, not a fixed CAD-cm value.
...
function fpBeginHeldWallDrag(hit,startPoint){
  ...
  fpWallMoveHold.connected=[]; // v2.9.10 selected wall only
  ...
  if(fpTool==='select' && fpSelectedId){
    ...
    if(o.type==='wall'){
      if(fpEndpointDrag==='start'){
        ...
        // v2.9.10: only the selected wall moves.
