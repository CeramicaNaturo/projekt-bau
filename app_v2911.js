...
function nearestWallEndpoint(p,maxDistance=null){
  // v2.9.11: endpoint magnet is based on SCREEN distance, not a fixed CAD-cm value.
  // This makes corner snapping reliable on tablet, PC and at every zoom level.
  const z=Math.max(.15,Number(fpZoom)||1);
  const touchPx=(window.matchMedia?.('(pointer: coarse)')?.matches)?52:34;
  const limit=Number.isFinite(Number(maxDistance)) ? Number(maxDistance) : touchPx/z;
...
function fpBeginHeldWallDrag(hit,startPoint){
  ...
  fpWallMoveHold.connected=[]; // v2.9.11 selected wall only
...
function floorStart(ev){
  ...
  if(fpTool==='wall'){
    ...
    // v2.9.11 magnetic start:
    // 1) the corner actually touched by the user ALWAYS wins,
    // 2) continuous-chain endpoint is used only if the pointer is still close to it,
    // 3) otherwise start at the pointer/grid.
    const endpointHit=nearestWallEndpoint(p);
    const z=Math.max(.15,Number(fpZoom)||1);
    const chainLimit=((window.matchMedia?.('(pointer: coarse)')?.matches)?52:34)/z;
    const nearLast=fpLastWallEnd &&
      Math.hypot(Number(p.x)-Number(fpLastWallEnd.x),Number(p.y)-Number(fpLastWallEnd.y))<=chainLimit;

    if(endpointHit){
      fpStart={x:Number(endpointHit.x),y:Number(endpointHit.y)};
    }else if(nearLast){
      fpStart={x:Number(fpLastWallEnd.x),y:Number(fpLastWallEnd.y)};
    }else{
  ...
  if(fpTool==='select' && fpSelectedId){
    ...
    if(o.type==='wall'){
      if(fpEndpointDrag==='start'){
        ...
        // v2.9.11: only the selected wall moves.
...
function floorEnd(ev){
  ...
  if((fpTool==='wall') && fpStart){
    ...
    // Final safety lock: saved wall start must exactly equal an existing corner
    // whenever the start gesture was inside the magnetic snap radius.
    const finalStartHit=nearestWallEndpoint(fpStart,1.5);
    const finalStart=finalStartHit
      ? {x:Number(finalStartHit.x),y:Number(finalStartHit.y)}
...
function drawAllWallJoints(){
  ...
  const addPoint=(p,w)=>{
    let group=seen.find(g=>Math.hypot(g.x-p.x,g.y-p.y)<=0.05);
    if(!group){
