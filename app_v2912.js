...
function fpBeginHeldWallDrag(hit,startPoint){
  ...
  fpWallMoveHold.connected=[]; // v2.9.12 selected wall only
...
function floorStart(ev){
  ...
  if(fpTool==='wall'){
    ...
    const z=Math.max(.15,Number(fpZoom)||1);
    const _r=fpCanvas?.getBoundingClientRect?.();
    const _bs=(_r&&_r.width>0&&fpCanvas?.width)?fpCanvas.width/_r.width:1;
    const chainLimit=((window.matchMedia?.('(pointer: coarse)')?.matches)?72:46)*_bs/z;
  ...
  if(fpTool==='select' && fpSelectedId){
    ...
    if(o.type==='wall'){
      if(fpEndpointDrag==='start'){
        ...
        // v2.9.12: only the selected wall moves.
