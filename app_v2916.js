...
function setSelectedWallGeometry(){
  ...
  if(requested!=='' && requested!=null && Number.isFinite(Number(requested))){
...
function fpBeginHeldWallDrag(hit,startPoint){
  ...
  fpWallMoveHold.connected=[]; // v2.9.16 selected wall only
...
function floorStart(ev){
  ...
  if(fpTool==='wall'){
    ...
    const screenHit=fpMagneticEndpointFromEvent(ev);
    const rect=fpCanvas?.getBoundingClientRect?.();
  ...
  if(fpTool==='select' && fpSelectedId){
    ...
    if(o.type==='wall'){
      if(fpEndpointDrag==='start'){
        ...
        // v2.9.16: only the selected wall moves.
...
function floorEnd(ev){
  ...
  if((fpTool==='wall') && fpStart){
    ...
    let finalStart={x:Number(fpStart.x),y:Number(fpStart.y)};
    if(fpWallStartAnchor){
...
function snapObjectToWall(o,x,y){
  ...
  // Mirror sits on the wall face. Furniture / sanitary objects project into
  // the room so their back edge touches the inner wall face.
...
function fpObjectSideDimensionData(o){
  ...
  function nearest(origin,dir){
    ...
    for(const w of walls){
      const hit=fpRaySegmentIntersection(origin,dir,w);
      if(hit && hit.distance>0.25 && (!best || hit.distance<best.distance))best=hit;
    }
    return best;
  }
...
function fpDrawObjectOwnDimensions(o){ fpDrawObjectSideWallDimensions(o); }
...
/* === v2.9.16 Object Properties Editor === */
...
function fpEnsurePropertyPanel(){
  ...
  p.innerHTML=`
    ...
    <div class="fp-prop-grid">
      ...
      <label>Abstand links (cm)<input id="fp-prop-left" inputmode="decimal"></label>
      <label>Abstand rechts (cm)<input id="fp-prop-right" inputmode="decimal"></label>
      <label>Höhe ab Boden (cm)<input id="fp-prop-floorheight" inputmode="decimal"></label>
...
function fpOpenPropertyPanel(o){
  ...
  set('fp-prop-left', o.distanceLeftCm ?? '');
  set('fp-prop-right', o.distanceRightCm ?? '');
  set('fp-prop-floorheight', o.floorHeightCm ?? o.z ?? 0);
...
function fpApplyPropertyPanel(){
  ...
  o.distanceLeftCm=Math.max(0,fpNum(val('fp-prop-left'),o.distanceLeftCm||0));
  o.distanceRightCm=Math.max(0,fpNum(val('fp-prop-right'),o.distanceRightCm||0));
  o.floorHeightCm=Math.max(0,fpNum(val('fp-prop-floorheight')));
  ...
  o.angle=o.rotation; o.z=o.floorHeightCm; o.inset=o.insetDepthCm;
...
/* === v2.9.16 Visible Objekt-bearbeiten button === */
...
/* === v2.9.16 Object wall auto-stop / magnetic wall contact === */
function fpWallSegmentsForCollision(){
  return (fpObjects||[]).filter(o=>o && o.type==='wall').map(w=>{
    const x1=Number(w.x1??w.x??0), y1=Number(w.y1??w.y??0);
    const x2=Number(w.x2??((w.x??0)+(w.width??0))), y2=Number(w.y2??((w.y??0)+(w.height??0)));
    return {w,x1,y1,x2,y2,dx:x2-x1,dy:y2-y1};
  });
}
function fpObjectHalfExtents(o){
  const rot=((Number(o.rotation??o.angle??0)%360)+360)%360;
  let ww=Math.abs(Number(o.width??o.w??60)), hh=Math.abs(Number(o.depth??o.height??o.h??60));
  if((rot>45&&rot<135)||(rot>225&&rot<315)){ const t=ww;ww=hh;hh=t; }
  return {hx:ww/2,hy:hh/2};
}
function fpClampObjectToWalls(o){
  if(!o || o.type==='wall') return false;
  const segs=fpWallSegmentsForCollision();
  if(!segs.length) return false;
  let x=Number(o.x??0), y=Number(o.y??0);
  const e=fpObjectHalfExtents(o), snap=18; // cm-world tolerance
  const vertical=segs.filter(s=>Math.abs(s.dx)<Math.abs(s.dy));
  const horizontal=segs.filter(s=>Math.abs(s.dy)<=Math.abs(s.dx));
  const ys=(s)=>[Math.min(s.y1,s.y2),Math.max(s.y1,s.y2)];
  const xs=(s)=>[Math.min(s.x1,s.x2),Math.max(s.x1,s.x2)];

  // nearest surrounding faces whose span overlaps the object
  let L=null,R=null,T=null,B=null;
  vertical.forEach(s=>{
    const [mn,mx]=ys(s); if(y+e.hy<mn || y-e.hy>mx) return;
    const wx=(s.x1+s.x2)/2;
    if(wx<=x && (!L||wx>L.v)) L={v:wx,s};
    if(wx>=x && (!R||wx<R.v)) R={v:wx,s};
  });
  horizontal.forEach(s=>{
    const [mn,mx]=xs(s); if(x+e.hx<mn || x-e.hx>mx) return;
    const wy=(s.y1+s.y2)/2;
    if(wy<=y && (!T||wy>T.v)) T={v:wy,s};
    if(wy>=y && (!B||wy<B.v)) B={v:wy,s};
  });

  let nx=x,ny=y;
  if(L){
    const min=L.v+e.hx;
    if(nx<min || Math.abs((nx-e.hx)-L.v)<=snap) nx=min;
  }
  if(R){
    const max=R.v-e.hx;
    if(nx>max || Math.abs((nx+e.hx)-R.v)<=snap) nx=max;
  }
  if(T){
    const min=T.v+e.hy;
    if(ny<min || Math.abs((ny-e.hy)-T.v)<=snap) ny=min;
  }
  if(B){
    const max=B.v-e.hy;
    if(ny>max || Math.abs((ny+e.hy)-B.v)<=snap) ny=max;
  }
  const changed=Math.abs(nx-x)>.001||Math.abs(ny-y)>.001;
  if(changed){o.x=nx;o.y=ny;}
  return changed;
}
function fpApplySelectedWallCollision(){
  const o=(fpObjects||[]).find(x=>x.id===fpSelectedId);
  if(!o||o.type==='wall') return;
  if(fpClampObjectToWalls(o)){
    try{renderFloorplan();}catch(e){}
  }
}
/* capture phase: constrain continuously while an object is being dragged */
(function(){
  if(!fpCanvas)return;
  let active=false;
  fpCanvas.addEventListener('pointerdown',()=>{active=true;},true);
  fpCanvas.addEventListener('pointermove',()=>{if(active)setTimeout(fpApplySelectedWallCollision,0);},true);
  fpCanvas.addEventListener('pointerup',()=>{active=false;setTimeout(fpApplySelectedWallCollision,0);},true);
  fpCanvas.addEventListener('pointercancel',()=>{active=false;},true);
  fpCanvas.addEventListener('touchmove',()=>{if(active)setTimeout(fpApplySelectedWallCollision,0);},{capture:true,passive:true});
})();
