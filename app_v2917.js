...
function fpBeginHeldWallDrag(hit,startPoint){
  ...
  fpWallMoveHold.connected=[]; // v2.9.17 selected wall only
...
function fpWallDimensionGeometry(wall){
  const sx=Number(wall.x1),sy=Number(wall.y1),ex=Number(wall.x2),ey=Number(wall.y2);
  const dx=ex-sx,dy=ey-sy;
  const rawLen=Math.hypot(dx,dy);
  if(!(rawLen>0.001))return {sx,sy,ex,ey,len:0,rawLen:0,startInset:0};
  const ux=dx/rawLen,uy=dy/rawLen;

  // Interior / added wall: if it starts from an existing wall corner, measure
  // from the visible face of that existing wall, not from its centre axis.
  let startInset=0;
  const cs=wall.connectedStart;
  if(cs?.wallId){
    const parent=(fpObjects||[]).find(w=>w.type==='wall'&&w.id===cs.wallId);
    if(parent)startInset=Math.max(0,Number(parent.thickness||fpWallThickness||15))/2;
  }

  startInset=Math.min(startInset,Math.max(0,rawLen-1));
  return {
    sx:sx+ux*startInset,
    sy:sy+uy*startInset,
    ex,ey,
    len:Math.max(0,rawLen-startInset),
    rawLen,startInset,ux,uy
  };
}
  ...
  if(fpTool==='select' && fpSelectedId){
    ...
    if(o.type==='wall'){
      if(fpEndpointDrag==='start'){
        ...
        // v2.9.17: only the selected wall moves.
...
/* === v2.9.17 Object Properties Editor === */
...
/* === v2.9.17 Visible Objekt-bearbeiten button === */
