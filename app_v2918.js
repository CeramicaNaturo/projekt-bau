...
function fpBeginHeldWallDrag(hit,startPoint){
  ...
  fpWallMoveHold.connected=[]; // v2.9.18 selected wall only
  ...
  if(fpTool==='select' && fpSelectedId){
    ...
    if(o.type==='wall'){
      if(fpEndpointDrag==='start'){
        ...
        // v2.9.18: only the selected wall moves.
...
/* === v2.9.18 Object Properties Editor === */
...
function fpDistanceRayHit(origin,dir,ignoreWallId=null){
  ...
  for(const w of (fpObjects||[])){
    ...
    const hit=fpRaySegmentIntersection(origin,dir,w);
    if(hit && hit.distance>=-0.001 && (!best || hit.distance<best.distance))best=hit;
  }
  return best;
}
...
/* === v2.9.18 Visible Objekt-bearbeiten button === */
