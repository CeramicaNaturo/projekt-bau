...
function fpBeginHeldWallDrag(hit,startPoint){
  ...
  fpWallMoveHold.connected=[]; // v2.9.20 selected wall only
...
/* === v2.9.20 ARCHITECTURAL INNER-FACE MEASUREMENT ===
...
/* === v2.9.20 WALL-TO-WALL HARD STOP ===
...
function floorMove(ev){
  ...
  if(fpTool==='select' && fpSelectedId){
    ...
    if(o.type==='wall'){
      if(fpEndpointDrag==='start'){
        ...
        const raw=snapAnglePoint({x:orig.x1,y:orig.y1},{x:orig.x2+dx,y:orig.y2+dy});
        const stop=fpClampWallEndToWalls({x:orig.x1,y:orig.y1},raw,[o.id]);
        const ep=stop.point;
        ...
        // v2.9.20: only the selected wall moves.
  ...
  if((fpTool==='wall') && fpStart){
    ...
    const hardStop=fpClampWallEndToWalls(fpStart,smart.point,[connectedWallId]);
    fpPreview={
      id:'preview',
      type:fpTool,
      x1:fpStart.x,
...
function floorEnd(ev){
  ...
  if((fpTool==='wall') && fpStart){
    ...
    const hardStop=fpClampWallEndToWalls(fpStart,smart.point,[connectedStartId]);
...
/* === v2.9.20 Object Properties Editor === */
...
/* === v2.9.20 Visible Objekt-bearbeiten button === */
