...
function fpBeginHeldWallDrag(hit,startPoint){
  ...
  fpWallMoveHold.connected=[]; // v2.9.21 selected wall only
...
/* === v2.9.21 ARCHITECTURAL INNER-FACE MEASUREMENT ===
...
/* === v2.9.21 WALL-TO-WALL HARD STOP ===
...
/* v2.9.21: A wall that starts on an existing boundary wall may only grow
...
function floorMove(ev){
  ...
  if(fpTool==='select' && fpSelectedId){
    ...
    if(o.type==='wall'){
      if(fpEndpointDrag==='start'){
        ...
        const stop=fpClampWallEndToWalls({x:orig.x2,y:orig.y2},raw,[o.id]);
        const ep=stop.point;
        ...
        const stop=fpClampWallEndToWalls({x:orig.x1,y:orig.y1},raw,[o.id,anchorId]);
        const ep=stop.point;
        ...
        // v2.9.21: only the selected wall moves.
  ...
  if((fpTool==='wall') && fpStart){
    ...
    const hardStop=fpClampWallEndToWalls(fpStart,interior.point,[connectedWallId]);
    fpPreview={
      id:'preview',
      type:fpTool,
      x1:fpStart.x,
...
function floorEnd(ev){
  ...
  if((fpTool==='wall') && fpStart){
    ...
    const hardStop=fpClampWallEndToWalls(fpStart,interior.point,[connectedStartId]);
...
/* === v2.9.21 Object Properties Editor === */
...
/* === v2.9.21 Visible Objekt-bearbeiten button === */
