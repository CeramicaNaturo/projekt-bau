...
function fpBeginHeldWallDrag(hit,startPoint){
  ...
  fpWallMoveHold.connected=[]; // v2.9.22 selected wall only
...
function fpHardStopObjectAgainstWalls(o,desiredX,desiredY,originX,originY){
  ...
  let hit=false;
  const snapTolerance=12; // cm: magnetic zero-contact zone

  // Iterate so a corner can stop the object against two walls in the same move.
  for(let pass=0;pass<4;pass++){
    ...
    for(const w of (fpObjects||[])){
      ...
      // Object support in wall-normal and wall-tangent directions.
      const supportN=fpCollisionSupport(o,nx,ny);
      const supportT=fpCollisionSupport(o,tx,ty);
      const wallHalf=Math.max(1,Number(w.thickness||fpWallThickness||15))/2;
      const clearance=wallHalf+supportN;

      // Only collide while the object's footprint overlaps the finite wall span.
      const t=(x-x1)*tx+(y-y1)*ty;
      if(t+supportT<0 || t-supportT>len)continue;

      const d0=(ox-x1)*nx+(oy-y1)*ny;
      let dd=(x-x1)*nx+(y-y1)*ny;

      // Preserve the side from which the drag started. This turns every wall,
      // including internal partition walls, into a solid barrier.
...
/* === v2.9.22 ARCHITECTURAL INNER-FACE MEASUREMENT ===
...
function fpWallInnerFaceRaw(wall){
  ...
  // wallOutsideNormal already determines the exterior side; the exact opposite
  // direction is the room/interior side.
  const out=wallOutsideNormal(wall);
  const inn={nx:-out.nx,ny:-out.ny};
  const half=Math.max(0,Number(wall.thickness||fpWallThickness||15))/2;

  return {
    wall,
    a:{x:x1+inn.nx*half,y:y1+inn.ny*half},
    b:{x:x2+inn.nx*half,y:y2+inn.ny*half},
    axisA:{x:x1,y:y1},axisB:{x:x2,y:y2},
    inn,out,half,len
  };
}
...
function fpEffectiveInnerWallSegment(wall){
  const face=fpWallInnerFaceRaw(wall);
  const start=fpEffectiveInnerCorner(wall,'start');
  const end=fpEffectiveInnerCorner(wall,'end');
...
function fpWallDimensionGeometry(wall){
  const seg=fpEffectiveInnerWallSegment(wall);
  const sx=seg.x1,sy=seg.y1,ex=seg.x2,ey=seg.y2;
...
/* === v2.9.22 WALL-TO-WALL HARD STOP ===
...
/* v2.9.22: A wall that starts on an existing boundary wall may only grow
...
/* === v2.9.22 ROBUST WALL COLLISION ENGINE ===
...
function fpResolveWallEndpoint(start,end,opts={}){
  ...
  // 3. Object hard-stop. This is the final hard barrier.
  let objects=fpClampWallBeforeObjects(start,p,ignoreIds,opts.wallThickness||fpWallThickness||15);
  p=objects.point;
...
function floorMove(ev){
  ...
  if(fpTool==='select' && fpSelectedId){
    ...
    if(o.type==='wall'){
      if(fpEndpointDrag==='start'){
        ...
        const stop=fpResolveWallEndpoint({x:orig.x2,y:orig.y2},raw,{
        ...
        const stop=fpResolveWallEndpoint({x:orig.x1,y:orig.y1},raw,{
        ...
        const wallHit=fpClampWallEndToWalls(
        ...
        if(roomOK && !wallHit && !objHit){
          ...
          // Hard stop: keep last valid saved/original position.
          o.x1=orig.x1;o.y1=orig.y1;o.x2=orig.x2;o.y2=orig.y2;
        }

        // v2.9.22: only the selected wall moves.
  ...
  if((fpTool==='wall') && fpStart){
    ...
    const interior=fpKeepWallOnInteriorSide(fpStart,smart.point,connectedWallId);
    const hardStop=fpResolveWallEndpoint(fpStart,interior.point,{
...
function floorEnd(ev){
  ...
  if((fpTool==='wall') && fpStart){
    ...
    const interior=fpKeepWallOnInteriorSide(fpStart,smart.point,connectedStartId);
    const hardStop=fpResolveWallEndpoint(fpStart,interior.point,{
      anchorWallId:connectedStartId,
      wallThickness:fpWallThickness
    });
    const ep=hardStop.point;
...
function fpDrawObjectOwnDimensions(o){ fpDrawObjectFourWallDimensions(o); }
...
/* === v2.9.22 Object Properties Editor === */
...
function fpDistanceRayHit(origin,dir,ignoreWallId=null){
  ...
  for(const w of (fpObjects||[])){
    ...
    // Object position dimensions must terminate on the effective room-side
    // wall face. This also works at 45° mitered corners.
    const seg=fpEffectiveInnerWallSegment(w);
    const effectiveWall={x1:seg.x1,y1:seg.y1,x2:seg.x2,y2:seg.y2,id:w.id,type:'wall',sourceWall:w};
    const hit=fpRaySegmentIntersection(origin,dir,effectiveWall);
    if(hit && hit.distance>=-0.001 && (!best || hit.distance<best.distance)){
      hit.wall=w;
      hit.effectiveSegment=seg;
      best=hit;
    }
  }
  return best;
...
/* === v2.9.22 Visible Objekt-bearbeiten button === */
