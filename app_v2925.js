...
function fpBeginHeldWallDrag(hit,startPoint){
  ...
  fpWallMoveHold.connected=[]; // v2.9.25 selected wall only
...
/* === v2.9.25 ARCHITECTURAL INNER-FACE MEASUREMENT ===
...
/* === v2.9.25 WALL-TO-WALL HARD STOP ===
...
/* v2.9.25: A wall that starts on an existing boundary wall may only grow
...
/* === v2.9.25 ROBUST WALL COLLISION ENGINE ===
...
function fpWallRect2924(start,end,thickness=15){
  ...
  if(L<0.001)return [];
  const nx=-dy/L,ny=dx/L,h=Math.max(.1,Number(thickness)||15)/2;
  return [
    {x:sx+nx*h,y:sy+ny*h},
    {x:ex+nx*h,y:ey+ny*h},
    {x:ex-nx*h,y:ey-ny*h},
...
function fpObjectPoly2924(o){
  ...
  const defs=fpDefaultObjectDimensions?.(o.type)||[60,60];
  const scale=Math.max(.01,Number(o.scale)||1);
  // v2.9.25: collision footprint is always based on exact entered dimensions.
  ...
  if(fpTool==='select' && fpSelectedId){
    ...
    if(o.type==='wall'){
      if(fpEndpointDrag==='start'){
        ...
        // v2.9.25: only the selected wall moves.
...
function drawFpObject(o,preview=false){
  ...
  if(o.type==='door'){
    ...
    // v2.9.25: symbol base MUST equal declared default 90 x 90 cm.
...
/* === v2.9.25 Object Properties Editor === */
...
/* === v2.9.25 Visible Objekt-bearbeiten button === */
