...
function fpBeginHeldWallDrag(hit,startPoint){
  ...
  fpWallMoveHold.connected=[]; // v2.9.26 selected wall only
...
/* === v2.9.26 ARCHITECTURAL INNER-FACE MEASUREMENT ===
...
/* === v2.9.26 WALL-TO-WALL HARD STOP ===
...
/* v2.9.26: A wall that starts on an existing boundary wall may only grow
...
/* === v2.9.26 ROBUST WALL COLLISION ENGINE ===
...
function fpObjectPoly2924(o){
  ...
  const defs=fpDefaultObjectDimensions?.(o.type)||[60,60];
  const scale=Math.max(.01,Number(o.scale)||1);
  // v2.9.26: collision footprint is always based on exact entered dimensions.
  ...
  if(fpTool==='select' && fpSelectedId){
    ...
    if(o.type==='wall'){
      if(fpEndpointDrag==='start'){
        ...
        // v2.9.26: only the selected wall moves.
...
function drawFpObject(o,preview=false){
  ...
  if(o.type==='door'){
    ...
    // v2.9.26: symbol base MUST equal declared default 90 x 90 cm.
...
/* === v2.9.26 Object Properties Editor === */
...
/* === v2.9.26 UNIVERSAL HEIGHT ABOVE FLOOR === */
...
/* === v2.9.26 Visible Objekt-bearbeiten button === */
