...
function smartWallEndpointLegacy2927(start,p){
  // v2.9.27 drawing rule:
...
function smartWallEndpoint(start,p){
  const base=smartWallEndpointLegacy2927(start,p);
  const corner=fpSnapPointToOuterCorner2927(base.point,35,null);
  if(corner)return {...base,point:{x:corner.x,y:corner.y},snapped:true,
    target:{kind:'outer-corner',wallId:corner.wallId,end:corner.end,x:corner.x,y:corner.y}};
  return base;
}
...
function fpBeginHeldWallDrag(hit,startPoint){
  ...
  fpWallMoveHold.connected=[]; // v2.9.27 selected wall only
...
/* === v2.9.27 ARCHITECTURAL INNER-FACE MEASUREMENT ===
...
/* === v2.9.27 ARCHITECTURAL MITER WALL MODEL === */
...
function fpEffectiveInnerWallSegment2927(wall){
  const s=fpMiterInnerCorner2927(wall,'start');
  const e=fpMiterInnerCorner2927(wall,'end');
  return {wall,x1:s.x,y1:s.y,x2:e.x,y2:e.y,a:s,b:e,len:Math.hypot(e.x-s.x,e.y-s.y)};
}
...
/* === v2.9.27 WALL-TO-WALL HARD STOP ===
...
/* v2.9.27: A wall that starts on an existing boundary wall may only grow
...
/* === v2.9.27 ROBUST WALL COLLISION ENGINE ===
...
function fpObjectPoly2924(o){
  ...
  const defs=fpDefaultObjectDimensions?.(o.type)||[60,60];
  const scale=Math.max(.01,Number(o.scale)||1);
  // v2.9.27: collision footprint is always based on exact entered dimensions.
...
function floorMove(ev){
  ...
  if(fpTool==='select' && fpSelectedId){
    ...
    if(o.type==='wall'){
      if(fpEndpointDrag==='start'){
        ...
        const ep=stop.point;
        const corner2927=fpSnapPointToOuterCorner2927(ep,35,o.id);
        const fixed2927=corner2927||ep;
        o.x1=fixed2927.x;o.y1=fixed2927.y;o.x2=orig.x2;o.y2=orig.y2;
        ...
        const corner2927=fpSnapPointToOuterCorner2927(ep,35,o.id);
        const fixed2927=corner2927||ep;
        o.x1=orig.x1;o.y1=orig.y1;o.x2=fixed2927.x;o.y2=fixed2927.y;
        ...
        // v2.9.27: only the selected wall moves.
  ...
  if((fpTool==='wall') && fpStart){
    ...
    const hardStop=fpClampWallBySolid2924(fpStart,preliminary.point,{
      anchorWallId:connectedWallId,
      ignoreIds:[],
      thickness:fpWallThickness
    });
...
function floorEnd(ev){
  ...
  if((fpTool==='wall') && fpStart){
    ...
    const finalOuterCorner=fpSnapPointToOuterCorner2927(ep,35,connectedStartId);
    if(finalOuterCorner){
      ep={x:finalOuterCorner.x,y:finalOuterCorner.y};
      hardStop.hit={kind:'outer-corner',wallId:finalOuterCorner.wallId,end:finalOuterCorner.end,distanceCm:0};
    }
...
function drawFpObject(o,preview=false){
  ...
  if(o.type==='wall'){
    // v2.9.27 mitered architectural wall polygon.
  ...
  if(o.type==='door'){
    ...
    // v2.9.27: symbol base MUST equal declared default 90 x 90 cm.
...
/* === v2.9.27 Object Properties Editor === */
...
/* === v2.9.27 UNIVERSAL HEIGHT ABOVE FLOOR === */
...
/* === v2.9.27 Visible Objekt-bearbeiten button === */
