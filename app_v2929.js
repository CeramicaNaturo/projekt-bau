...
function fpBeginHeldWallDrag(hit,startPoint){
  ...
  fpWallMoveHold.connected=[]; // v2.9.29 selected wall only
...
/* === v2.9.29 ARCHITECTURAL INNER-FACE MEASUREMENT ===
...
/* === v2.9.29 ARCHITECTURAL MITER WALL MODEL === */
...
function fpEffectiveInnerWallSegment2927(wall){
  ...
  // Free end = exact end of the inner face.
  // Joined end = mathematical intersection of both inner faces (miter corner).
  const sharedStart=fpWallsSharingOuterCorner2927(wall,'start',.05);
  const sharedEnd=fpWallsSharingOuterCorner2927(wall,'end',.05);

  let s={x:f.a.x,y:f.a.y};
  let e={x:f.b.x,y:f.b.y};

  if(sharedStart.length){
    const m=fpMiterInnerCorner2927(wall,'start');
    s={x:m.x,y:m.y};
  }
  if(sharedEnd.length){
    const m=fpMiterInnerCorner2927(wall,'end');
    e={x:m.x,y:m.y};
  }
  ...
  if(fpTool==='select' && fpSelectedId){
    ...
    if(o.type==='wall'){
      if(fpEndpointDrag==='start'){
        ...
        // v2.9.29: only the selected wall moves.
...
function floorEnd(ev){
  ...
  if((fpTool==='wall') && fpStart){
    ...
    // v2.9.29: endpoint intentionally does not magnetize to existing corners.
...
function drawFpObject(o,preview=false){
  ...
  if(o.type==='wall'){
    // v2.9.29 mitered architectural wall polygon.
  ...
  if(o.type==='door'){
    ...
    // v2.9.29: symbol base MUST equal declared default 90 x 90 cm.
...
/* === v2.9.29 Object Properties Editor === */
...
/* === v2.9.29 UNIVERSAL HEIGHT ABOVE FLOOR === */
...
/* === v2.9.29 Visible Objekt-bearbeiten button === */
