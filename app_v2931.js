...
function fpBeginHeldWallDrag(hit,startPoint){
  ...
  fpWallMoveHold.connected=[]; // v2.9.31 selected wall only
...
/* === v2.9.31 ARCHITECTURAL INNER-FACE MEASUREMENT ===
...
/* === v2.9.31 ARCHITECTURAL MITER WALL MODEL === */
...
function fpEffectiveInnerWallSegment2927(wall){
  const f=fpInnerWallFace2927(wall);
  const outer=fpOuterWallFace2927(wall);
  const dx=outer.b.x-outer.a.x,dy=outer.b.y-outer.a.y;
  const L=Math.hypot(dx,dy)||1;
  const ux=dx/L,uy=dy/L;
  const t=Math.max(0,Number(wall.thickness||fpWallThickness||15));

  const sharedStart=fpWallsSharingOuterCorner2927(wall,'start',.05);
  const sharedEnd=fpWallsSharingOuterCorner2927(wall,'end',.05);

  // CRITICAL RULE:
  // Every dimension endpoint is an INNER CORNER.
  //
  // Joined end -> intersection of the two inner wall faces.
  // Free 45° end -> inner tip of the 45° cut, therefore one wall-thickness
  // inward along the wall direction.
  let s=sharedStart.length
    ? fpMiterInnerCorner2927(wall,'start')
    : {x:f.a.x+ux*t,y:f.a.y+uy*t};

  let e=sharedEnd.length
    ? fpMiterInnerCorner2927(wall,'end')
    : {x:f.b.x-ux*t,y:f.b.y-uy*t};
  ...
  return {
    ...
    trueInner:true,
    freeStartInset:sharedStart.length?0:t,
    freeEndInset:sharedEnd.length?0:t
  };
}
...
function fpInnerDimensionRule2931(wall){
  const g=fpEffectiveInnerWallSegment2927(wall);
  return {
    start:{x:g.x1,y:g.y1},
    end:{x:g.x2,y:g.y2},
    lengthCm:g.len,
    rule:'INNER_CORNER_TO_INNER_CORNER'
  };
}
  ...
  if(fpTool==='select' && fpSelectedId){
    ...
    if(o.type==='wall'){
      if(fpEndpointDrag==='start'){
        ...
        // v2.9.31: only the selected wall moves.
...
function floorEnd(ev){
  ...
  if((fpTool==='wall') && fpStart){
    ...
    // v2.9.31: endpoint intentionally does not magnetize to existing corners.
...
function drawFpObject(o,preview=false){
  ...
  if(o.type==='wall'){
    // v2.9.31 mitered architectural wall polygon.
  ...
  if(o.type==='door'){
    ...
    // v2.9.31: symbol base MUST equal declared default 90 x 90 cm.
...
/* === v2.9.31 Object Properties Editor === */
...
/* === v2.9.31 UNIVERSAL HEIGHT ABOVE FLOOR === */
...
/* === v2.9.31 Visible Objekt-bearbeiten button === */
