...
function fpBeginHeldWallDrag(hit,startPoint){
  ...
  fpWallMoveHold.connected=[]; // v2.9.32 selected wall only
...
/* === v2.9.32 ARCHITECTURAL INNER-FACE MEASUREMENT ===
...
/* === v2.9.32 ARCHITECTURAL MITER WALL MODEL === */
...
function fpEffectiveInnerWallSegment2927(wall){
  ...
  // v2.9.32 DIMENSION RULE:
  // Measure the effective ROOM-SIDE wall face.
  // - joined corner: exact intersection of both inner faces
  // - free end: exact endpoint of the inner face
  // Wall thickness is NEVER subtracted along the wall direction.
  let s=sharedStart.length ? fpMiterInnerCorner2927(wall,'start')
                           : {x:f.a.x,y:f.a.y};
  let e=sharedEnd.length   ? fpMiterInnerCorner2927(wall,'end')
                           : {x:f.b.x,y:f.b.y};
...
function fpInnerDimensionRule2932(wall){
  const g=fpEffectiveInnerWallSegment2927(wall);
  return {
    start:{x:g.x1,y:g.y1},
    end:{x:g.x2,y:g.y2},
    lengthCm:g.len,
    rule:'ROOM_SIDE_FACE_ENDPOINT_TO_ENDPOINT'
  };
}
  ...
  if(fpTool==='select' && fpSelectedId){
    ...
    if(o.type==='wall'){
      if(fpEndpointDrag==='start'){
        ...
        // v2.9.32: only the selected wall moves.
...
function floorEnd(ev){
  ...
  if((fpTool==='wall') && fpStart){
    ...
    // v2.9.32: endpoint intentionally does not magnetize to existing corners.
...
function drawFpObject(o,preview=false){
  ...
  if(o.type==='wall'){
    // v2.9.32 mitered architectural wall polygon.
  ...
  if(o.type==='door'){
    ...
    // v2.9.32: symbol base MUST equal declared default 90 x 90 cm.
...
/* === v2.9.32 Object Properties Editor === */
...
/* === v2.9.32 UNIVERSAL HEIGHT ABOVE FLOOR === */
...
/* === v2.9.32 Visible Objekt-bearbeiten button === */
