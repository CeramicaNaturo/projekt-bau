...
function fpBeginHeldWallDrag(hit,startPoint){
  ...
  fpWallMoveHold.connected=[]; // v2.9.33 selected wall only
...
/* === v2.9.33 ARCHITECTURAL INNER-FACE MEASUREMENT ===
...
/* === v2.9.33 ARCHITECTURAL MITER WALL MODEL === */
...
function fpEffectiveInnerWallSegment2927(wall){
  ...
  /*
   * v2.9.33 EFFECTIVE INNER-CORNER RULE
   *
  ...
  if(fpTool==='select' && fpSelectedId){
    ...
    if(o.type==='wall'){
      if(fpEndpointDrag==='start'){
        ...
        // v2.9.33: only the selected wall moves.
...
function floorEnd(ev){
  ...
  if((fpTool==='wall') && fpStart){
    ...
    // v2.9.33: endpoint intentionally does not magnetize to existing corners.
...
function drawFpObject(o,preview=false){
  ...
  if(o.type==='wall'){
    // v2.9.33 mitered architectural wall polygon.
  ...
  if(o.type==='door'){
    ...
    // v2.9.33: symbol base MUST equal declared default 90 x 90 cm.
...
/* === v2.9.33 Object Properties Editor === */
...
/* === v2.9.33 UNIVERSAL HEIGHT ABOVE FLOOR === */
...
/* === v2.9.33 Visible Objekt-bearbeiten button === */
