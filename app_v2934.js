...
function fpBeginHeldWallDrag(hit,startPoint){
  ...
  fpWallMoveHold.connected=[]; // v2.9.34 selected wall only
...
/* === v2.9.34 ARCHITECTURAL INNER-FACE MEASUREMENT ===
...
/* === v2.9.34 ARCHITECTURAL MITER WALL MODEL === */
...
function fpEffectiveInnerWallSegment2927(wall){
  ...
  const sharedStart=fpWallsSharingOuterCorner2927(wall,'start',.08);
  const sharedEnd=fpWallsSharingOuterCorner2927(wall,'end',.08);

  /*
   * v2.9.34 EFFECTIVE INNER-CORNER RULE
   *
   * The saved wall line terminates at the visible 45°/miter cut.
   * The effective room corner lies one wall-thickness farther along the
   * wall axis for every connected end.
   *
   * Examples with t=10 cm:
   *   raw 280 + 10 + 10 = 300 cm
   *   raw 181 + 10 + 10 = 201 cm
   *   raw 191 + 10       = 201 cm
   *
   * A free end receives no artificial extension.
   */
  const startExt=sharedStart.length ? t : 0;
  const endExt=sharedEnd.length ? t : 0;
  ...
  return {
    ...
    a:s,b:e,
    len:rawLen+startExt+endExt,
    rawLen,
    startExtension:startExt,
    endExtension:endExt,
    trueInner:true,
    dimensionRule:'MITER_ENDPOINT_PLUS_WALL_THICKNESS'
  };
}
...
function fpWallDimensionGeometry(wall){
  const seg=fpEffectiveInnerWallSegment2927(wall);
  ...
  return {
    sx,sy,ex,ey,len,
    rawLen:seg.rawLen||len,
    startInset:0,
    ux:dx/len,uy:dy/len,
    startExtension:seg.startExtension||0,
    endExtension:seg.endExtension||0,
    effectiveInner:true,
    trueInner:true
  };
}
  ...
  if(fpTool==='select' && fpSelectedId){
    ...
    if(o.type==='wall'){
      if(fpEndpointDrag==='start'){
        ...
        // v2.9.34: only the selected wall moves.
...
function floorEnd(ev){
  ...
  if((fpTool==='wall') && fpStart){
    ...
    // v2.9.34: endpoint intentionally does not magnetize to existing corners.
...
function drawAllProfessionalWallDimensions(){
  ...
  for(const wall of walls){
    ...
    if(fpDimensionLayoutMap.has(wall.id)){
      ...
      drawInnerEffectiveWallDimension2934(wall);    // iç efektif ölçü
      rendered++;
    }
  }
...
function drawInnerEffectiveWallDimension2934(wall){
  if(!fpShowMeasures || !wall)return;
  let d=fpDimensionLayoutMap.get(wall.id);
  if(!d){prepareDimensionLayout();d=fpDimensionLayoutMap.get(wall.id);}
  if(!d)return;

  const z=Math.max(.2,fpZoom||1);
  const {sx,sy,ex,ey,ux,uy,dx,dy,out}=d;
  const len=Math.hypot(ex-sx,ey-sy);
  if(len<1)return;

  // Existing dimension uses OUTSIDE normal. Effective room dimension is
  // deliberately rendered on the exact opposite, INNER side.
  ...
  const tw=fpCtx.measureText(text).width,pad=5/z,bh=18/z;
  fpCtx.fillStyle='rgba(255,255,255,.96)';
  fpCtx.fillRect(-tw/2-pad,-bh/2,tw+pad*2,bh);
  fpCtx.fillStyle='#1d4ed8';fpCtx.fillText(text,0,0);
  fpCtx.restore();
}
...
function drawFpObject(o,preview=false){
  ...
  if(o.type==='wall'){
    // v2.9.34 mitered architectural wall polygon.
  ...
  if(o.type==='door'){
    ...
    // v2.9.34: symbol base MUST equal declared default 90 x 90 cm.
...
/* === v2.9.34 Object Properties Editor === */
...
/* === v2.9.34 UNIVERSAL HEIGHT ABOVE FLOOR === */
...
/* === v2.9.34 Visible Objekt-bearbeiten button === */
