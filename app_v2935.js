...
function fpBeginHeldWallDrag(hit,startPoint){
  ...
  fpWallMoveHold.connected=[]; // v2.9.35 selected wall only
...
/* === v2.9.35 ARCHITECTURAL INNER-FACE MEASUREMENT ===
...
/* === v2.9.35 ARCHITECTURAL MITER WALL MODEL === */
...
function fpEffectiveInnerWallSegment2927(wall){
  ...
  return {
    ...
    a:s,b:e,
    len:Math.max(0,rawLen-startInset-endInset),
    rawLen,
    startInset,
    endInset,
    trueInner:true,
  ...
  if(fpTool==='select' && fpSelectedId){
    ...
    if(o.type==='wall'){
      if(fpEndpointDrag==='start'){
        ...
        // v2.9.35: only the selected wall moves.
...
function floorEnd(ev){
  ...
  if((fpTool==='wall') && fpStart){
    ...
    // v2.9.35: endpoint intentionally does not magnetize to existing corners.
...
function drawInnerEffectiveWallDimension2935(wall){
  try{
    ...
    const innerLen=Math.max(0,rawLen-startInset-endInset);
...
function drawFpObject(o,preview=false){
  ...
  if(o.type==='wall'){
    // v2.9.35 mitered architectural wall polygon.
  ...
  if(o.type==='door'){
    ...
    // v2.9.35: symbol base MUST equal declared default 90 x 90 cm.
...
/* === v2.9.35 Object Properties Editor === */
...
/* === v2.9.35 UNIVERSAL HEIGHT ABOVE FLOOR === */
...
/* === v2.9.35 Visible Objekt-bearbeiten button === */
