...
/* === v2.9.36 INNER-LENGTH WALL INPUT MODEL ===
...
function fpWallInnerInputLength2936(wall){
  ...
  if(Number.isFinite(Number(wall.innerLengthCm)) && Number(wall.innerLengthCm)>0)
    return Number(wall.innerLengthCm);

  // Migration rule for existing drawings: the value already visible to the user
  // becomes the inner length; geometry will expand on the next edit/apply.
...
function fpApplyInnerLengthToWall2936(wall,innerLength,angleDeg){
  ...
  const outer=fpWallOuterGeometryLength2936(inner,wall);
...
function fpBeginHeldWallDrag(hit,startPoint){
  ...
  fpWallMoveHold.connected=[]; // v2.9.36 selected wall only
...
/* === v2.9.36 ARCHITECTURAL INNER-FACE MEASUREMENT ===
...
/* === v2.9.36 ARCHITECTURAL MITER WALL MODEL === */
  ...
  if(fpTool==='select' && fpSelectedId){
    ...
    if(o.type==='wall'){
      if(fpEndpointDrag==='start'){
        ...
        // v2.9.36: only the selected wall moves.
...
function floorEnd(ev){
  ...
  if((fpTool==='wall') && fpStart){
    ...
    // v2.9.36: endpoint intentionally does not magnetize to existing corners.
...
function drawInnerEffectiveWallDimension2935(wall){
  try{
    ...
    const innerLen=Number.isFinite(Number(wall.innerLengthCm)) && Number(wall.innerLengthCm)>0 ? Number(wall.innerLengthCm) : Math.max(0,rawLen-startInset-endInset);
...
function drawFpObject(o,preview=false){
  ...
  if(o.type==='wall'){
    // v2.9.36 mitered architectural wall polygon.
  ...
  if(o.type==='door'){
    ...
    // v2.9.36: symbol base MUST equal declared default 90 x 90 cm.
...
/* === v2.9.36 Object Properties Editor === */
...
/* === v2.9.36 UNIVERSAL HEIGHT ABOVE FLOOR === */
...
/* === v2.9.36 Visible Objekt-bearbeiten button === */
