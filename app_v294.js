...
function generateDirectPDFReport(){
  ...
  // Open synchronously from the user gesture so tablet/mobile browsers do not block the PDF tab.
...
function buildPrintReport(){
  ...
  if(projectInformation){
    ...
    pdf.setFontSize(12); pdf.setFont(undefined,'bold'); pdf.text('PROJEKTINFORMATIONEN / AUSZUFÜHRENDE ARBEITEN',14,y); y+=6;
    pdf.setFont(undefined,'normal'); pdf.setFontSize(10);
    const infoLines=pdf.splitTextToSize(projectInformation,pw-28);
    for(const line of infoLines){ensure(6);pdf.text(line,14,y);y+=5;}
    y+=3;
  }
