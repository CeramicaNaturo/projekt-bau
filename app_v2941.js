...
/* === v2.9.41 INNER-LENGTH WALL INPUT MODEL ===
...
function fpApplyInnerLengthToWall2936(wall,innerLength,angleDeg){
  ...
  // v2.9.41: one source of truth for thickness.
  wall.thickness=t;
  fpWallThickness=t;
...
function calculateFloorAreaM2(objects){
  /*
   * v2.9.41 — BODENFLÄCHE = echte lichte Innenfläche
   *
...
function fpBeginHeldWallDrag(hit,startPoint){
  ...
  fpWallMoveHold.connected=[]; // v2.9.41 selected wall only
...
/* === v2.9.41 ARCHITECTURAL INNER-FACE MEASUREMENT ===
...
/* === v2.9.41 ARCHITECTURAL MITER WALL MODEL === */
  ...
  if(fpTool==='select' && fpSelectedId){
    ...
    if(o.type==='wall'){
      if(fpEndpointDrag==='start'){
        ...
        // v2.9.41: only the selected wall moves.
...
function floorEnd(ev){
  ...
  if((fpTool==='wall') && fpStart){
    ...
    // v2.9.41: endpoint intentionally does not magnetize to existing corners.
...
function nearestWallForObject(p){
  ...
  for(const w of (fpObjects||[])){
    ...
    // v2.9.41: objects reference the true room-side INNER wall face,
...
function snapObjectToWall(o,x,y){
  ...
  // v2.9.41: near.point IS the true room-side inner wall face.
...
function drawFpObject(o,preview=false){
  ...
  if(o.type==='wall'){
    // v2.9.41 mitered architectural wall polygon.
  ...
  if(o.type==='door'){
    ...
    // v2.9.41: symbol base MUST equal declared default 90 x 90 cm.
...
/* === v2.9.41 Object Properties Editor === */
...
/* === v2.9.41 UNIVERSAL HEIGHT ABOVE FLOOR === */
...
/* === v2.9.41 Visible Objekt-bearbeiten button === */
