...
function renderPlattenlegerCockpit(p){
  ...
  if(phasebar)phasebar.innerHTML=PL_PHASES.map((x,i)=>`<div class="plattenleger-phase ${i<phaseIndex?'done':''} ${i===phaseIndex?'active':''}">${esc(x[1])}</div>`).join('');
...
function drawWallElevation(){
  const canvas=$('fpWallElevationCanvas'),wall=fpEnsureWallViewSelection();
  if(!canvas)return;
  ...
  const index=fpWallViewWalls().findIndex(w=>w.id===wall.id);ctx.fillStyle='#0f172a';ctx.font='800 16px system-ui';ctx.textAlign='left';ctx.fillText(`Wand ${index+1} · 2D Elevation`,left,top-17);
}
...
(()=>{
  const VERSION='2.9.45 PRO Plattenleger';
