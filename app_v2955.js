...
function pbPhasePdfExport(p,requested){
  ...
  const doc=new JsPdf({unit:'mm',format:'a4'}),margin=16,pageW=210,pageH=297,maxW=178;let y=18,pageNo=1;
  const safe=v=>String(v??'').replace(/[\u2010-\u2015]/g,'-').replace(/\u00d7/g,'x');
  const footer=()=>{doc.setDrawColor(170);doc.line(margin,282,pageW-margin,282);doc.setFontSize(8);doc.setTextColor(100);doc.text(`Projekt Bau 2.9.55 PRO CLEAN · ${safe(p.name||'Projekt')}`,margin,288);doc.text(`Seite ${pageNo}`,pageW-margin,288,{align:'right'});doc.setTextColor(20)};
  const newPage=()=>{footer();doc.addPage();pageNo++;y=18};
...
/* Projekt Bau 2.9.55: non-destructive ERP navigation shell. */
(()=>{
  ...
  ready(()=>{
    ...
    const roadmap={
      ...
      Hilfe:'Projekt Bau 2.9.55 PRO CLEAN · Lokale Daten, OneDrive, PDF, Aufmass sowie 2D/3D CAD bleiben unverändert verfügbar.'
    };
...
(()=>{
  const VERSION='2.9.55 PRO CLEAN';
