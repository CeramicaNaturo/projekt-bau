...
async function pbBrandLogoData(){
  ...
  try{const blob=await fetch('./logo_ceramica.jpg?v=295601').then(r=>r.blob());pbBrandLogoCache=await new Promise((resolve,reject)=>{const fr=new FileReader();fr.onload=()=>resolve(fr.result);fr.onerror=reject;fr.readAsDataURL(blob)});return pbBrandLogoCache}catch(_){return null}
}
...
async function pbPhasePdfExport(p,requested){
  ...
  requested.forEach((phase,idx)=>{
    ...
    if(phase==='aufmass'){
      ...
      (d.measureGroups||[]).forEach(g=>{ensure(16);text(g.title||'Bereich',13,true);(g.rows||[]).forEach(r=>{const sum=pbMeasureNumber(r.factor||1)*pbMeasureFormula(r.formula);text(`${r.item||'Position'} | Faktor ${r.factor||1} | Formel ${r.formula||'-'} | ${formatCHNumber(sum,3)} ${r.unit||''}`,9,true,3);if(r.note)text(r.note,8,false,6)});if(g.notes)kv('Notizen',g.notes)});
...
function buildPrintReport(){
  ...
  for(let i=0;i<items.length;i+=2){
    ...
    page.innerHTML=`<div class="pdf-header">
      <div>
        <img class="pdf-company-logo" src="logo_ceramica.jpg?v=295601" alt="Ceramica Naturo">
...
/* Projekt Bau 2.9.56: non-destructive ERP navigation shell. */
(()=>{
  ...
  ready(()=>{
    ...
    const roadmap={
      ...
      Hilfe:'Projekt Bau 2.9.56 PRO CLEAN · Lokale Daten, OneDrive, PDF, Aufmass sowie 2D/3D CAD bleiben unverändert verfügbar.'
    };
...
(()=>{
  const VERSION='2.9.56 PRO CLEAN';
