...
async function pbBrandLogoData(){
  ...
  try{const blob=await fetch('./logo_ceramica.jpg?v=295702').then(r=>r.blob());pbBrandLogoCache=await new Promise((resolve,reject)=>{const fr=new FileReader();fr.onload=()=>resolve(fr.result);fr.onerror=reject;fr.readAsDataURL(blob)});return pbBrandLogoCache}catch(_){return null}
}
...
function buildPrintReport(){
  ...
  for(let i=0;i<items.length;i+=2){
    ...
    page.innerHTML=`<div class="pdf-header">
      <div>
        <img class="pdf-company-logo" src="logo_ceramica.jpg?v=295702" alt="Ceramica Naturo">
...
function pbRenderCustomersWorkspace(workspace,editId=null){
  ...
  workspace.querySelector('[data-customer-new]').onclick=()=>pbRenderCustomersWorkspace(workspace,'new');workspace.querySelectorAll('[data-customer-open]').forEach(btn=>btn.onclick=()=>pbRenderCustomersWorkspace(workspace,btn.dataset.customerOpen));workspace.querySelectorAll('[data-customer-filter]').forEach(input=>input.oninput=()=>{const filters=[...workspace.querySelectorAll('[data-customer-filter]')].map(x=>x.value.toLocaleLowerCase('de-CH'));workspace.querySelectorAll('[data-customer-open]').forEach(row=>{const cells=[...row.children];row.hidden=filters.some((f,i)=>f&&!cells[i]?.textContent.toLocaleLowerCase('de-CH').includes(f))})});
}

/* Projekt Bau 2.9.57: non-destructive ERP navigation shell. */
(()=>{
  ...
  ready(()=>{
    ...
    const roadmap={
      ...
      Hilfe:'Projekt Bau 2.9.57 PRO CLEAN · Lokale Daten, OneDrive, PDF, Aufmass sowie 2D/3D CAD bleiben unverändert verfügbar.'
    };
...
(()=>{
  const VERSION='2.9.57 PRO CLEAN';
