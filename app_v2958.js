...
function pbApplyCustomerToProject(p,c){
  ...
  const snapshot={customerId:p.customerId,customerName:p.customer,customerAddress:p.address,customerPhone:p.phone,customerEmail:c?.email||''};
  const pd=p.plattenleger?.phaseData||{};Object.values(pd).forEach(d=>{if(d&&typeof d==='object')Object.assign(d,snapshot)});(p.orders||[]).forEach(o=>Object.assign(o,snapshot));
}
...
async function pbBrandLogoData(){
  ...
  try{const blob=await fetch('./logo_ceramica.jpg?v=295803').then(r=>r.blob());pbBrandLogoCache=await new Promise((resolve,reject)=>{const fr=new FileReader();fr.onload=()=>resolve(fr.result);fr.onerror=reject;fr.readAsDataURL(blob)});return pbBrandLogoCache}catch(_){return null}
}
...
function buildPrintReport(){
  ...
  for(let i=0;i<items.length;i+=2){
    ...
    page.innerHTML=`<div class="pdf-header">
      <div>
        <img class="pdf-company-logo" src="logo_ceramica.jpg?v=295803" alt="Ceramica Naturo">
...
function pbRenderCustomerEditorV2958(workspace,editId){
  ...
  workspace.querySelector('[data-customer-save]').onclick=()=>{const obj=customer||c;workspace.querySelectorAll('[data-cf]').forEach(el=>obj[el.dataset.cf]=el.value);const cp={};workspace.querySelectorAll('[data-contact]').forEach(el=>cp[el.dataset.contact]=el.value);obj.contacts=[cp];obj.bank=obj.bank||{};workspace.querySelectorAll('[data-bank]').forEach(el=>obj.bank[el.dataset.bank]=el.value);workspace.querySelectorAll('[data-bank-check]').forEach(el=>obj.bank[el.dataset.bankCheck]=el.checked);obj.fullAddress=[obj.street,obj.houseNo].filter(Boolean).join(' ')+(obj.zip||obj.city?`, ${[obj.zip,obj.city].filter(Boolean).join(' ')}`:'');obj.updatedAt=new Date().toISOString();if(!customer)S.customers.unshift(obj);S.projects.filter(p=>p.customerId===obj.id).forEach(p=>pbApplyCustomerToProject(p,obj));save();pbRenderCustomersWorkspace(workspace);pbActionToast('Kunde gespeichert.')};
  if(customer)workspace.querySelector('[data-customer-delete]').onclick=()=>{if(!confirm('Kunde wirklich löschen?'))return;S.customers=S.customers.filter(x=>x.id!==customer.id);save();pbRenderCustomersWorkspace(workspace)};
}
...
(()=>{
  ...
  ready(()=>{
    ...
    const roadmap={
      ...
      Hilfe:'Projekt Bau 2.9.58 PRO CLEAN · Lokale Daten, OneDrive, PDF, Aufmass sowie 2D/3D CAD bleiben unverändert verfügbar.'
    };
    document.querySelectorAll('.cad-nav-item[data-module]').forEach(btn=>btn.addEventListener('click',()=>{
      activate(btn);
      const name=btn.dataset.module;
      if(name==='Kunden'){pbRenderCustomersWorkspace(workspace);workspace.classList.remove('hidden');workspace.scrollIntoView({behavior:'smooth',block:'start'});return}
