...
async function pbBrandLogoData(){
  ...
  try{const blob=await fetch('./logo_ceramica.jpg?v=296101').then(r=>r.blob());pbBrandLogoCache=await new Promise((resolve,reject)=>{const fr=new FileReader();fr.onload=()=>resolve(fr.result);fr.onerror=reject;fr.readAsDataURL(blob)});return pbBrandLogoCache}catch(_){return null}
}
...
function buildPrintReport(){
  ...
  for(let i=0;i<items.length;i+=2){
    ...
    page.innerHTML=`<div class="pdf-header">
      <div>
        <img class="pdf-company-logo" src="logo_ceramica.jpg?v=296101" alt="Ceramica Naturo">
...
/* Projekt Bau 2.9.61: non-destructive ERP navigation shell. */
(()=>{
  ...
  ready(()=>{
    ...
    const roadmap={
      ...
      Hilfe:'Projekt Bau 2.9.61 PRO CLEAN · Lokale Daten, OneDrive, PDF, Aufmass sowie 2D/3D CAD bleiben unverändert verfügbar.'
    };
    document.querySelectorAll('.cad-nav-item[data-module]').forEach(btn=>btn.addEventListener('click',()=>{
      ...
      workspace.querySelector('[data-close-module]').onclick=()=>{document.body.classList.remove('pb-customer-mode');workspace.classList.add('hidden')};
    }));
    document.querySelectorAll('.cad-nav-item[data-scroll]').forEach(btn=>btn.addEventListener('click',()=>{
      activate(btn); document.body.classList.remove('pb-customer-mode');workspace?.classList.add('hidden');
    }));
...
(()=>{
  const VERSION='2.9.61 PRO CLEAN';
