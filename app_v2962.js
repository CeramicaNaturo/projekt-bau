...
async function pbBrandLogoData(){
  ...
  try{const blob=await fetch('./logo_ceramica.jpg?v=296201').then(r=>r.blob());pbBrandLogoCache=await new Promise((resolve,reject)=>{const fr=new FileReader();fr.onload=()=>resolve(fr.result);fr.onerror=reject;fr.readAsDataURL(blob)});return pbBrandLogoCache}catch(_){return null}
}
...
function buildPrintReport(){
  ...
  for(let i=0;i<items.length;i+=2){
    ...
    page.innerHTML=`<div class="pdf-header">
      <div>
        <img class="pdf-company-logo" src="logo_ceramica.jpg?v=296201" alt="Ceramica Naturo">
...
function pbOpenCustomerRelationCreate(workspace,customer,type){
  const projects=pbCustomerProjects(customer),label=PB_CUSTOMER_RELATIONS.find(x=>x[0]===type)?.[2]||'Dokument',isProject=type==='projects';
  ...
  const modal=document.createElement('div');modal.className='pb-relation-modal';modal.innerHTML=`<div class="pb-relation-dialog" role="dialog" aria-modal="true"><div class="pb-relation-dialog-head"><div><small>NEU ERSTELLEN</small><h2>${esc(isProject?'Neues Projekt':label)}</h2></div><button type="button" data-modal-close>×</button></div><div class="pb-relation-form">
    ${isProject?`<label>Projektnummer<input value="${esc(pbNextNumber('PRO'))}" data-new-number readonly></label><label>Kunde / Firma<input value="${esc(pbCustomerName(customer))}" readonly></label><label class="wide">Projektname<input data-new-title placeholder="z. B. Badsanierung" autofocus></label><label class="wide">Adresse<input data-new-address value="${esc(customer.fullAddress||[customer.street,customer.houseNo,customer.zip,customer.city].filter(Boolean).join(' '))}"></label><label>Startdatum<input type="date" data-new-date value="${new Date().toISOString().slice(0,10)}"></label><label>Verantwortlich<input data-new-owner></label><label class="wide">Allgemeine Beschreibung<textarea rows="5" data-new-notes></textarea></label>`:`<label>Nummer<input value="${esc(pbNextNumber(pbCustomerRelationPrefix(type)))}" data-new-number readonly></label><label>Projekt<select data-new-project>${projects.map(p=>`<option value="${esc(p.id)}">${esc(p.projectNo||'')} · ${esc(p.name||'Projekt')}</option>`).join('')}</select></label><label class="wide">Bezeichnung<input data-new-title placeholder="${esc(label)}" autofocus></label><label>Datum<input type="date" data-new-date value="${new Date().toISOString().slice(0,10)}"></label><label>Status<select data-new-status><option>Entwurf</option><option>Offen</option><option>Erstellt</option><option>Abgeschlossen</option></select></label><label class="wide">Anmerkung<textarea rows="5" data-new-notes></textarea></label>`}
  </div><div class="pb-relation-dialog-actions"><button type="button" class="secondary" data-modal-close>Abbrechen</button><button type="button" class="primary" data-modal-save>${esc(isProject?'Projekt erstellen':label+' erstellen')}</button></div></div>`;
  ...
  pbBindTap(modal.querySelector('[data-modal-save]'),async()=>{
    ...
    if(isProject){const p={id:u(),projectNo:number,name:title,address:modal.querySelector('[data-new-address]')?.value||'',customerId:customer.id,customer:pbCustomerName(customer),phone:customer.phone||customer.mobile||'',startDate:date,owner:modal.querySelector('[data-new-owner]')?.value||'',description:notes,status:'Neu',createdAt:now,updatedAt:now,areas:[]};S.projects.unshift(p);pbApplyCustomerToProject(p,customer)}
      ...
      const p=S.projects.find(x=>x.id===modal.querySelector('[data-new-project]')?.value);if(!p)return;const item={id:u(),number,title,date,status:modal.querySelector('[data-new-status]')?.value||'Entwurf',notes,customerId:customer.id,projectId:p.id,createdAt:now,updatedAt:now};
...
function pbRenderCustomerRelation(workspace,customer,type='projects'){
  ...
  else target.innerHTML=`<div class="pb-relation-tools"><strong>${label}</strong><button type="button" class="primary" data-relation-add>+</button></div>${items.length?`<div class="pb-relation-cards">${items.map(x=>`<button type="button" data-relation-project="${esc(x._project?.id||'')}"><strong>${esc(x.offerNo||x.measureNo||x.orderNo||x.invoiceNo||x.number||x.title||label)}</strong><span>${esc(x._project?.name||'')}</span><small>${fmtDate(x.date||x.offerDate||x.measureDate||x.createdAt||'')}</small></button>`).join('')}</div>`:`<p class="pb-relation-empty">Für diesen Kunden sind noch keine ${label} vorhanden.</p>`}`;
  target.querySelectorAll('[data-relation-project]').forEach(btn=>pbBindTap(btn,()=>{const id=btn.dataset.relationProject;if(!id)return;A=id;document.body.classList.remove('pb-customer-mode');workspace.classList.add('hidden');render();document.getElementById('projectDetail')?.scrollIntoView({behavior:'smooth',block:'start'})}));
...
/* Projekt Bau 2.9.62: non-destructive ERP navigation shell. */
(()=>{
  ...
  ready(()=>{
    ...
    const roadmap={
      ...
      Hilfe:'Projekt Bau 2.9.62 PRO CLEAN · Lokale Daten, OneDrive, PDF, Aufmass sowie 2D/3D CAD bleiben unverändert verfügbar.'
    };
...
(()=>{
  const VERSION='2.9.62 PRO CLEAN';
