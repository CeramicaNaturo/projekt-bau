...
async function pbBrandLogoData(){
  ...
  try{const blob=await fetch('./logo_ceramica.jpg?v=296301').then(r=>r.blob());pbBrandLogoCache=await new Promise((resolve,reject)=>{const fr=new FileReader();fr.onload=()=>resolve(fr.result);fr.onerror=reject;fr.readAsDataURL(blob)});return pbBrandLogoCache}catch(_){return null}
}
...
function buildPrintReport(){
  ...
  for(let i=0;i<items.length;i+=2){
    ...
    page.innerHTML=`<div class="pdf-header">
      <div>
        <img class="pdf-company-logo" src="logo_ceramica.jpg?v=296301" alt="Ceramica Naturo">
...
function pbOpenExistingProjectPhase(workspace,project,phase){
  if(!project)return;A=project.id;const cockpit=ensurePlattenlegerProject(project);cockpit.phase=phase;localStorage.setItem(K3,JSON.stringify(S));window.PBStorage?.saveState?.(S,{reason:`open-${phase}-296301`});document.querySelector('.pb-relation-modal')?.remove();document.body.classList.remove('pb-customer-mode','pb-module-mode','pb-shell-focus');workspace.classList.add('hidden');render();requestAnimationFrame(()=>{renderPlattenlegerCockpit(project);document.getElementById('plattenlegerCockpit')?.scrollIntoView({behavior:'smooth',block:'start'})});
}
...
/* Projekt Bau 2.9.63: non-destructive ERP navigation shell. */
(()=>{
  ...
  ready(()=>{
    ...
    const roadmap={
      ...
      Hilfe:'Projekt Bau 2.9.63 PRO CLEAN · Lokale Daten, OneDrive, PDF, Aufmass sowie 2D/3D CAD bleiben unverändert verfügbar.'
    };
...
(()=>{
  const VERSION='2.9.63 PRO CLEAN';
