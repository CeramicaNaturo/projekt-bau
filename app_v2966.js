...
async function pbBrandLogoData(){
  ...
  try{const blob=await fetch('./logo_ceramica.jpg?v=296401').then(r=>r.blob());pbBrandLogoCache=await new Promise((resolve,reject)=>{const fr=new FileReader();fr.onload=()=>resolve(fr.result);fr.onerror=reject;fr.readAsDataURL(blob)});return pbBrandLogoCache}catch(_){return null}
}
...
async function pbPhasePdfExport(p,requested){
  ...
  const kv=(k,v)=>{if(v===undefined||v===null||v===''||v===false)return;ensure(7);doc.setFontSize(9);doc.setFont('helvetica','bold');doc.text(safe(k),margin,y);doc.setFont('helvetica','normal');const lines=doc.splitTextToSize(safe(v===true?'Ja':v),118);doc.text(lines,75,y);y+=Math.max(6,lines.length*4)};
  const heading=(name)=>{if(y>25)newPage();doc.setTextColor(7,94,168);text(name,18,true);doc.setTextColor(20);doc.setDrawColor(7,94,168);doc.line(margin,y-1,pageW-margin,y-1);y+=5};
  ...
  requested.forEach((phase,idx)=>{
    ...
    kv('Projekt',p.name);kv('Kunde / Firma',p.customer);kv('Adresse',p.address);kv('Verantwortlich',p.owner);kv('Exportdatum',new Date().toLocaleDateString('de-CH'));
...
async function generateDirectPDFReport(){
  ...
  try{
    ...
    const brandLogo=await pbBrandLogoData();pbAddBrandLogo(doc,brandLogo,12,7,54);
...
/* Projekt Bau 2.9.66: non-destructive ERP navigation shell. */
(()=>{
  ...
  ready(()=>{
    ...
    const roadmap={
      ...
      Hilfe:'Projekt Bau 2.9.66 PRO CLEAN · Lokale Daten, OneDrive, PDF, Aufmass sowie 2D/3D CAD bleiben unverändert verfügbar.'
    };
...
(()=>{
  const VERSION='2.9.66 PRO CLEAN';
