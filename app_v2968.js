...
async function pbBrandLogoData(){
  ...
  try{const blob=await fetch('./logo_ceramica.jpg?v=296801').then(r=>r.blob());pbBrandLogoCache=await new Promise((resolve,reject)=>{const fr=new FileReader();fr.onload=()=>resolve(fr.result);fr.onerror=reject;fr.readAsDataURL(blob)});return pbBrandLogoCache}catch(_){return null}
}
...
function pbOpenMaterialManager(mode='list',opts={}){
  ...
  const modal=$('pbMaterialManager');if(!modal)return;
...
function pbCloseMaterialManager(){
  const m=$('pbMaterialManager');if(m){m.classList.add('hidden');m.setAttribute('aria-hidden','true')}
...
function initTileTools(){
  $('openMaterialManager')?.addEventListener('click',()=>pbOpenMaterialManager('list'));
...
/* Projekt Bau 2.9.68: non-destructive ERP navigation shell. */
(()=>{
  ...
  ready(()=>{
    ...
    const roadmap={
      ...
      Hilfe:'Projekt Bau 2.9.68 PRO CLEAN · Lokale Daten, OneDrive, PDF, Aufmass sowie 2D/3D CAD bleiben unverändert verfügbar.'
    };
...
(()=>{
  const VERSION='2.9.68 PRO CLEAN';
