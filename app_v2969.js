...
async function pbBrandLogoData(){
  ...
  try{const blob=await fetch('./logo_ceramica.jpg?v=296901').then(r=>r.blob());pbBrandLogoCache=await new Promise((resolve,reject)=>{const fr=new FileReader();fr.onload=()=>resolve(fr.result);fr.onerror=reject;fr.readAsDataURL(blob)});return pbBrandLogoCache}catch(_){return null}
}
...
function pbOpenMaterialEditor(item=null,calcMode='mit Kalkulation'){
  ...
  body.innerHTML=`<section class="pb-material-page pb-material-edit-page">
    ...
    <div class="pb-material-calc-grid ${withCalc?'':'without'}">
      ...
      <div class="pb-calc-summary">
        <div><span>Materialeinzelkosten</span><b id="pbCalcMaterialCost">CHF ${pbMaterialMoney(c.materialCost)}</b></div>
        <div><span>Materialgemeinkosten</span><label><input id="pbMatOverhead" value="${pbMaterialEscape(m.overheadPct)}" inputmode="decimal"><em>%</em></label><b id="pbCalcOverhead">CHF ${pbMaterialMoney(c.overhead)}</b></div>
        <div><span>Selbstkosten</span><b id="pbCalcSelf">CHF ${pbMaterialMoney(c.selfCost)}</b></div>
        <div><span>Gewinnzuschlag</span><label><input id="pbMatProfit" value="${pbMaterialEscape(m.profitPct)}" inputmode="decimal"><em>%</em></label><b id="pbCalcProfit">CHF ${pbMaterialMoney(c.profit)}</b></div>
      </div>
  ...
  const update=()=>{
    ...
    if($('pbCalcMaterialCost'))$('pbCalcMaterialCost').textContent='CHF '+pbMaterialMoney(cc.materialCost);
    if($('pbCalcOverhead'))$('pbCalcOverhead').textContent='CHF '+pbMaterialMoney(cc.overhead);
    if($('pbCalcSelf'))$('pbCalcSelf').textContent='CHF '+pbMaterialMoney(cc.selfCost);
    if($('pbCalcProfit'))$('pbCalcProfit').textContent='CHF '+pbMaterialMoney(cc.profit);
...
/* Projekt Bau 2.9.69: non-destructive ERP navigation shell. */
(()=>{
  ...
  ready(()=>{
    ...
    const roadmap={
      ...
      Hilfe:'Projekt Bau 2.9.69 PRO CLEAN · Lokale Daten, OneDrive, PDF, Aufmass sowie 2D/3D CAD bleiben unverändert verfügbar.'
    };
...
(()=>{
  const VERSION='2.9.69 PRO CLEAN';
