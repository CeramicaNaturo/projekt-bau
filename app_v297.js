...
function smartWallEndpoint(start,p){
  // v2.9.7: angles are free. Grid snapping may still be used for normal points,
...
function fpBeginHeldWallDrag(hit,startPoint){
  ...
  fpWallMoveHold.connected=[]; // v2.9.7 selected wall only
  ...
  if(fpTool==='select' && fpSelectedId){
    ...
    if(o.type==='wall'){
      if(fpEndpointDrag==='start'){
        ...
        // v2.9.7: only the selected wall moves.
