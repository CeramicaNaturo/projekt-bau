...
async function pbBrandLogoData(){
  ...
  try{const blob=await fetch('./logo_ceramica.jpg?v=297001').then(r=>r.blob());pbBrandLogoCache=await new Promise((resolve,reject)=>{const fr=new FileReader();fr.onload=()=>resolve(fr.result);fr.onerror=reject;fr.readAsDataURL(blob)});return pbBrandLogoCache}catch(_){return null}
}
...
function renderProfessionalAufmass(c,data){
  return pbAufmassMaterialChooser(data)+pbRenderMeasureGroups(data);
...
function pbOpenMaterialEditor(item=null,calcMode='mit Kalkulation'){
  ...
  body.innerHTML=`<section class="pb-material-page pb-material-edit-page">
    ...
    <div class="pb-material-calc-grid ${withCalc?'':'without'}">
      ...
      <label>Einheit Gebinde<select id="pbMatPackageUnit">${['Packung','Stk.','kg','l','m²','m1'].map(x=>`<option ${x===m.packageUnit?'selected':''}>${x}</option>`).join('')}</select></label>
      <label>Angebotseinheit<select id="pbMatOfferUnit">${['m²','m1','Stk.','kg','l','pauschal'].map(x=>`<option ${x===m.offerUnit?'selected':''}>${x}</option>`).join('')}</select></label>
...
/* Projekt Bau 2.9.70: non-destructive ERP navigation shell. */
(()=>{
  ...
  ready(()=>{
    ...
    const roadmap={
      ...
      Hilfe:'Projekt Bau 2.9.70 PRO CLEAN · Lokale Daten, OneDrive, PDF, Aufmass sowie 2D/3D CAD bleiben unverändert verfügbar.'
    };
...
(()=>{
  const VERSION='2.9.70 PRO CLEAN';
