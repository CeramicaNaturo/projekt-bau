...
const PL_PHASE_CONTENT={
  anfrage:{title:'Kundenanfrage',desc:'Erstkontakt, Bedarf, Besichtigung und nächste Schritte strukturiert erfassen.',fields:[
    ...
    ['contact','Kontaktperson','text'],['contactPhone','Telefon','text'],['contactEmail','E-Mail','email'],
    ['objectAddress','Objektadresse','text'],['desiredDate','Gewünschter Ausführungstermin','date'],
  ...
  ],checks:[['contactConfirmed','Kontaktdaten bestätigt'],['visitConfirmed','Besichtigung vereinbart'],['plansReceived','Pläne/Unterlagen erhalten'],['photosReceived','Fotos erhalten']]},
...
async function pbBrandLogoData(){
  ...
  try{const blob=await fetch('./logo_ceramica.jpg?v=297501').then(r=>r.blob());pbBrandLogoCache=await new Promise((resolve,reject)=>{const fr=new FileReader();fr.onload=()=>resolve(fr.result);fr.onerror=reject;fr.readAsDataURL(blob)});return pbBrandLogoCache}catch(_){return null}
}
...
function pbEnsureAnfrageDefaults(p,c,data){
  ...
  data.contact=data.contact||p.customer||'';
  data.contactPhone=data.contactPhone||p.phone||'';
  const cust=(S.customers||[]).find(x=>x.id===p.customerId);
  data.contactEmail=data.contactEmail||cust?.email||'';
  data.objectAddress=data.objectAddress||p.address||'';
  data.responsible=data.responsible||p.owner||'';
  data.priority=data.priority||'Normal';data.requestStatus=data.requestStatus||'Neu';
}
...
function renderPlattenlegerPhaseContent(c){
  ...
  if(c.phase==='aufmass')host.innerHTML=`<div class="plattenleger-phase-head"><div><h4>Professionelles Bad-Aufmass</h4><p>Vollständige Aufnahme für Badumbau, Offerte, Material und Ausführung.</p></div><span class="plattenleger-phase-badge">Aufmass</span></div>${renderProfessionalAufmass(c,data)}`;
    ...
    host.innerHTML=`<div class="plattenleger-phase-head"><div><h4>${esc(schema.title)}</h4><p>${esc(schema.desc)}</p></div><span class="plattenleger-phase-badge">${esc(PL_PHASES.find(x=>x[0]===c.phase)?.[1]||'Phase')}</span></div>`+
      ...
      (c.phase==='anfrage'?`<div class="pb-anfrage-actions"><button type="button" class="secondary" data-anfrage-action="visit">Besichtigung planen</button><button type="button" class="primary" data-anfrage-action="project">Projekt speichern</button><button type="button" class="primary" data-anfrage-action="offer">Offerte erstellen</button><button type="button" class="danger" data-anfrage-action="reject">Anfrage ablehnen</button></div>`:'');
  }
  ...
  host.querySelectorAll('[data-anfrage-action]').forEach(btn=>pbBindTap(btn,()=>pbAnfrageAction(btn.dataset.anfrageAction,c,data)));
...
/* Projekt Bau 2.9.75: non-destructive ERP navigation shell. */
(()=>{
  ...
  ready(()=>{
    ...
    const roadmap={
      ...
      Hilfe:'Projekt Bau 2.9.75 PRO CLEAN · Lokale Daten, OneDrive, PDF, Aufmass sowie 2D/3D CAD bleiben unverändert verfügbar.'
    };
...
(()=>{
  const VERSION='2.9.75 PRO CLEAN';
