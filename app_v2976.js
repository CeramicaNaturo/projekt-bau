...
const PL_PHASE_CONTENT={
  anfrage:{title:'Kundenanfrage',desc:'Kunden- und Anfrageinformationen zentral erfassen.',fields:[
    ...
    ['customerCompany','Kunde / Firma','text'],['customerSalutation','Anrede','select',['Firma','Herr','Frau']],
    ['customerFirstName','Vorname','text'],['customerLastName','Nachname','text'],
    ['contactEmail','E-Mail','email'],['contactPhone','Telefon','text'],['customerMobile','Mobil','text'],
    ['customerStreet','Strasse','text'],['customerHouseNo','Hausnummer','text'],['customerZip','PLZ','text'],['customerCity','Stadt','text'],
...
async function pbBrandLogoData(){
  ...
  try{const blob=await fetch('./logo_ceramica.jpg?v=297601').then(r=>r.blob());pbBrandLogoCache=await new Promise((resolve,reject)=>{const fr=new FileReader();fr.onload=()=>resolve(fr.result);fr.onerror=reject;fr.readAsDataURL(blob)});return pbBrandLogoCache}catch(_){return null}
}
...
async function pbSaveAnfrageCustomer(c,data){
  ...
  customer.type=company?'Firma':'Privat';
...
function renderPlattenlegerPhaseContent(c){
  ...
  if(c.phase==='aufmass')host.innerHTML=`<div class="plattenleger-phase-head"><div><h4>Professionelles Bad-Aufmass</h4><p>Vollständige Aufnahme für Badumbau, Offerte, Material und Ausführung.</p></div><span class="plattenleger-phase-badge">Aufmass</span></div>${renderProfessionalAufmass(c,data)}`;
    ...
    host.innerHTML=`<div class="plattenleger-phase-head"><div><h4>${esc(schema.title)}</h4><p>${esc(schema.desc)}</p></div><span class="plattenleger-phase-badge">${esc(PL_PHASES.find(x=>x[0]===c.phase)?.[1]||'Phase')}</span></div>`+
      ...
      (c.phase==='anfrage'?`<div class="pb-anfrage-savebar"><button type="button" class="primary" data-anfrage-save>Kunde und Anfrage speichern</button></div>`:
  ...
  host.insertAdjacentHTML('afterbegin',`<div class="pb-phase-pdfbar"><strong>PDF-Ausgabe</strong><select data-pdf-phase>${PB_PDF_PHASES.map(([k,l])=>`<option value="${k}" ${k===c.phase?'selected':''}>${l}</option>`).join('')}</select><button type="button" class="primary" data-pdf-one>Phase als PDF erstellen</button></div>`);
...
/* Projekt Bau 2.9.76: non-destructive ERP navigation shell. */
(()=>{
  ...
  ready(()=>{
    ...
    const roadmap={
      ...
      Hilfe:'Projekt Bau 2.9.76 PRO CLEAN · Lokale Daten, OneDrive, PDF, Aufmass sowie 2D/3D CAD bleiben unverändert verfügbar.'
    };
...
(()=>{
  const VERSION='2.9.76 PRO CLEAN';
