...
async function pbBrandLogoData(){
  ...
  try{const blob=await fetch('./logo_ceramica.jpg?v=297701').then(r=>r.blob());pbBrandLogoCache=await new Promise((resolve,reject)=>{const fr=new FileReader();fr.onload=()=>resolve(fr.result);fr.onerror=reject;fr.readAsDataURL(blob)});return pbBrandLogoCache}catch(_){return null}
}
...
function pbAnfrageCustomerCard(data){
  ...
  return `<section class="pb-anfrage-customer-card">
    ...
    <div class="pb-customer-tabs">
      <button type="button" class="active">▧ Adressen</button>
      <button type="button">♙ Ansprechpartner</button>
      <button type="button">▦ Zahlung &amp; Bank</button>
      <button type="button">▱ Anmerkung</button>
    </div>
    <div class="pb-customer-address-panel">
      <div class="pb-address-head"><strong>${val('customerStreet')||'Neue Adresse'}</strong><span>+</span></div>
      <div class="pb-customer-address-grid">
        <label>Bezeichnung<input data-pl-phase-field="customerAddressLabel" value="${val('customerAddressLabel')}"></label>
        <label>Typ<select data-pl-phase-field="customerAddressType"><option>Standard</option><option>Rechnung</option><option>Objekt</option><option>Lieferung</option></select></label>
        <label class="wide">Abweichender Empfänger<input data-pl-phase-field="customerAltRecipient" value="${val('customerAltRecipient')}"></label>
        <label>Vorname<input data-pl-phase-field="customerFirstName" value="${val('customerFirstName')}"></label>
        <label>Nachname<input data-pl-phase-field="customerLastName" value="${val('customerLastName')}"></label>
        <label>Telefon<input data-pl-phase-field="contactPhone" value="${val('contactPhone')}"></label>
        <label>E-Mail<input type="email" data-pl-phase-field="contactEmail" value="${val('contactEmail')}"></label>
        <label class="wide">Adressergänzung<input data-pl-phase-field="customerAddressExtra" value="${val('customerAddressExtra')}"></label>
...
/* Projekt Bau 2.9.77: non-destructive ERP navigation shell. */
(()=>{
  ...
  ready(()=>{
    ...
    const roadmap={
      ...
      Hilfe:'Projekt Bau 2.9.77 PRO CLEAN · Lokale Daten, OneDrive, PDF, Aufmass sowie 2D/3D CAD bleiben unverändert verfügbar.'
    };
...
(()=>{
  const VERSION='2.9.77 PRO CLEAN';
