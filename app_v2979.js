...
function pbNavigateToProject(projectId,phase=''){
  const project=S.projects.find(p=>p.id===projectId);if(!project)return false;A=project.id;if(phase)ensurePlattenlegerProject(project).phase=phase;
  document.body.classList.remove('pb-customer-mode','pb-module-mode','pb-shell-focus');document.querySelectorAll('.cad-main>section').forEach(x=>x.classList.remove('pb-shell-visible'));document.getElementById('pbModuleWorkspace')?.classList.add('hidden');renderP();
  const panel=document.getElementById('panel');if(panel){panel.classList.add('pb-shell-visible');document.body.classList.add('pb-shell-focus');requestAnimationFrame(()=>panel.scrollIntoView({behavior:'smooth',block:'start'}))}return true;
}
...
async function pbBrandLogoData(){
  ...
  try{const blob=await fetch('./logo_ceramica.jpg?v=297901').then(r=>r.blob());pbBrandLogoCache=await new Promise((resolve,reject)=>{const fr=new FileReader();fr.onload=()=>resolve(fr.result);fr.onerror=reject;fr.readAsDataURL(blob)});return pbBrandLogoCache}catch(_){return null}
}
...
/* Projekt Bau 2.9.79: non-destructive ERP navigation shell. */
(()=>{
  ...
  ready(()=>{
    ...
    const roadmap={
      ...
      Hilfe:'Projekt Bau 2.9.79 PRO CLEAN · Lokale Daten, OneDrive, PDF, Aufmass sowie 2D/3D CAD bleiben unverändert verfügbar.'
    };
...
(()=>{
  const VERSION='2.9.79 PRO CLEAN';
