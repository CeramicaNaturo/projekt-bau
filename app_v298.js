...
function smartWallEndpoint(start,p){
  // v2.9.8: angles are free. Grid snapping may still be used for normal points,
  // but a real existing corner is always taken with its exact saved coordinate.
  const corner=nearestWallEndpoint(p,35);
  if(corner && Math.hypot(corner.x-start.x,corner.y-start.y)>3){
    return {
      point:{x:Number(corner.x),y:Number(corner.y)},
      target:{x:Number(corner.x),y:Number(corner.y),distance:Math.hypot(corner.x-start.x,corner.y-start.y),axis:'corner'},
      snapped:true
    };
  }

  const base=fpAngleSnap ? snapAnglePoint(start,p) : {x:snap(Number(p.x)),y:snap(Number(p.y))};

  // Existing opposite-wall auto-intersection is retained only where the
  // legacy ray routine can calculate it safely. It never forces an angle.
...
function fpBeginHeldWallDrag(hit,startPoint){
  ...
  fpWallMoveHold.connected=[]; // v2.9.8 selected wall only
...
function floorStart(ev){
  ...
  if(fpTool==='wall'){
    ...
    if(endpointHit){
      // Exact existing corner coordinate: never grid-round a real wall corner.
      fpStart={x:Number(endpointHit.x),y:Number(endpointHit.y)};
    }else if(fpLastWallEnd){
      // Continuous wall chain starts at the mathematically exact prior endpoint.
  ...
  if(fpTool==='select' && fpSelectedId){
    ...
    if(o.type==='wall'){
      if(fpEndpointDrag==='start'){
        ...
        // v2.9.8: only the selected wall moves.
