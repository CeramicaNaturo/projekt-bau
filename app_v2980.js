...
function pbProjectDetailHtml(project){
  ...
  return `<div class="pb-project-detail">
    ...
    <section class="pb-project-detail-files" data-project-detail-files></section>
  </div>`;
}
function pbBindProjectDetail(workspace,project){
  ...
  workspace.querySelectorAll('[data-project-status]').forEach(btn=>btn.addEventListener('click',()=>{
    project.status=btn.dataset.projectStatus;
  ...
  pbMountOwnerFiles(workspace.querySelector('[data-project-detail-files]'),'project',project.id,'Projektdateien');
}
...
async function pbBrandLogoData(){
  ...
  try{const blob=await fetch('./logo_ceramica.jpg?v=298001').then(r=>r.blob());pbBrandLogoCache=await new Promise((resolve,reject)=>{const fr=new FileReader();fr.onload=()=>resolve(fr.result);fr.onerror=reject;fr.readAsDataURL(blob)});return pbBrandLogoCache}catch(_){return null}
}
...
async function pbMountOwnerFiles(host,ownerType,ownerId,title='Dateien'){
  ...
  const draw=async()=>{
    ...
    const add=async fileList=>{
      for(const file of [...(fileList||[])])await pbPutOwnerFile({id:u(),owner:`${ownerType}:${ownerId}`,ownerType,ownerId,name:file.name,type:file.type||'application/octet-stream',size:file.size,blob:file,lastModified:file.lastModified||Date.now(),createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()});
    ...
    host.querySelectorAll('[data-owner-file-edit]').forEach(b=>b.addEventListener('click',async()=>{
      ...
      if(!rec)return;const name=prompt('Dateiname bearbeiten:',rec.name);if(name===null)return;const clean=name.trim();if(!clean)return;rec.name=clean;rec.updatedAt=new Date().toISOString();await pbPutOwnerFile(rec);await draw();
    }));
    host.querySelectorAll('[data-owner-file-delete]').forEach(b=>b.addEventListener('click',async()=>{if(!confirm('Datei wirklich löschen?'))return;await pbDeleteOwnerFile(b.dataset.ownerFileDelete);await draw();pbActionToast('Datei gelöscht.')}))
  };
  await draw();
}
...
function pbOpenAppointmentEditor(workspace,customer,projectId='',appointmentId=''){
  ...
  pbBindTap(modal.querySelector('[data-ap-save]'),async()=>{
    ...
    save();await window.PBStorage?.saveState?.(S,{reason:'appointment-save-2979'});try{window.ProjectBauOneDrive?.scheduleAutoSync?.('appointment-save')}catch(_){}
    modal.remove();pbRenderCustomerRelation(workspace,customer,'appointments');pbActionToast('Termin gespeichert.');
  });
  if(appointment)pbBindTap(modal.querySelector('[data-ap-delete]'),()=>{if(!confirm('Termin wirklich löschen?'))return;project.appointments=(project.appointments||[]).filter(x=>x.id!==appointment.id);save();modal.remove();pbRenderCustomerRelation(workspace,customer,'appointments')});
}
...
/* Projekt Bau 2.9.80: non-destructive ERP navigation shell. */
(()=>{
  ...
  ready(()=>{
    ...
    const roadmap={
      ...
      Hilfe:'Projekt Bau 2.9.80 PRO CLEAN · Lokale Daten, OneDrive, PDF, Aufmass sowie 2D/3D CAD bleiben unverändert verfügbar.'
    };
...
(()=>{
  const VERSION='2.9.80 PRO CLEAN';
