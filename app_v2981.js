...
function pbProjectLowerHtml(project){
  ...
  return `<div class="pb-project-lower-mask">
    ...
    <section class="pb-lower-card">
      ...
      <div class="pb-doc-tabs">${documentsTabs.map(x=>`<button type="button">${esc(x)}</button>`).join('')}</div>
...
function pbProjectDetailHtml(project){
  ...
  return `<div class="pb-project-detail">
    ...
    <div class="pb-project-detail-form">
      ...
      <label>Objektadresse<div class="pb-project-address-control"><input data-pd="address" value="${esc(address)}"><button type="button" class="primary" data-project-address-add>+</button></div></label>
      ...
      <label class="check wide"><input type="checkbox" data-pd="alternateAddress" ${project.alternateAddress?'checked':''}> Abweichende Projektadresse</label>
    </div>
    <div class="pb-project-finance-grid">
      <article><span>▦</span><div><small>Offerten</small><strong>CHF ${formatCHNumber(f.offer,2)}</strong></div></article>
      <article><span>↔</span><div><small>Aufträge</small><strong>CHF ${formatCHNumber(f.orders,2)}</strong></div></article>
      <article><span>▤</span><div><small>Rechnungen</small><strong>CHF ${formatCHNumber(f.invoices,2)}</strong></div></article>
      <article><span>▧</span><div><small>Zahlungen</small><strong>CHF ${formatCHNumber(f.payments,2)}</strong></div></article>
    ...
    <div class="pb-project-detail-lower">
      <button type="button" class="secondary" data-open-project-cockpit>Projekt-Cockpit öffnen</button>
    </div>
...
function pbBindProjectDetail(workspace,project){
  ...
  workspace.querySelector('[data-project-detail-save]')?.addEventListener('click',async()=>{
    ...
    const before={name:project.name||'',status:project.status||'',address:project.address||'',owner:project.owner||'',customerReference:project.customerReference||'',description:project.description||'',employees:(project.employees||[]).join(', '),alternateAddress:!!project.alternateAddress};
    ...
    project.alternateAddress=!!workspace.querySelector('[data-pd="alternateAddress"]')?.checked;
    const changes=[];const labels={name:'Bezeichnung',status:'Status',address:'Objektadresse',owner:'Projektleiter',customerReference:'Kundenreferenz',description:'Beschreibung',employees:'Mitarbeiter',alternateAddress:'abweichende Projektadresse'};
    const after={name:project.name,status:project.status,address:project.address,owner:project.owner,customerReference:project.customerReference,description:project.description,employees:project.employees.join(', '),alternateAddress:project.alternateAddress};
  ...
  workspace.querySelector('[data-open-project-cockpit]')?.addEventListener('click',()=>{
    A=project.id;document.body.classList.remove('pb-customer-mode','pb-module-mode');
    workspace.classList.add('hidden');renderP();
    const panel=document.getElementById('panel');panel?.classList.add('pb-shell-visible');document.body.classList.add('pb-shell-focus');panel?.scrollIntoView({behavior:'smooth',block:'start'});
  });
  ...
  workspace.querySelector('[data-task-add]')?.addEventListener('click',()=>{
    project.projectTasks=Array.isArray(project.projectTasks)?project.projectTasks:[];project.projectTasks.push({id:u(),title:'Neue Aufgabe',date:'',priority:'Normal',done:false});pbProjectHistoryAdd(project,'Projektaufgabe erstellt','Neue Aufgabe','Aufgabe');save();pbNavigateToProject(project.id);
  });
  workspace.querySelectorAll('[data-task-index]').forEach(row=>{
    ...
    row.querySelector('[data-task-delete]')?.addEventListener('click',()=>{if(!confirm('Aufgabe wirklich löschen?'))return;const old=task.title;project.projectTasks.splice(i,1);pbProjectHistoryAdd(project,'Projektaufgabe gelöscht',old,'Aufgabe');save();pbNavigateToProject(project.id)});
  });
...
async function pbBrandLogoData(){
  ...
  try{const blob=await fetch('./logo_ceramica.jpg?v=298101').then(r=>r.blob());pbBrandLogoCache=await new Promise((resolve,reject)=>{const fr=new FileReader();fr.onload=()=>resolve(fr.result);fr.onerror=reject;fr.readAsDataURL(blob)});return pbBrandLogoCache}catch(_){return null}
}
...
function pbCalendarEvents(customerId=''){
  ...
  projects.forEach(p=>(p.appointments||p.dates||[]).forEach(a=>events.push({...a,id:a.id||u(),projectId:p.id,projectName:p.name,employee:a.employee||p.owner||'Nicht zugewiesen',source:'appointment'})));
...
function pbMountMonthCalendar(host,events,initialMonth=new Date()){
  ...
  const draw=()=>{
    ...
    for(let w=0;w<6;w++){
      ...
      for(let d=0;d<7;d++){
        const day=new Date(ws);day.setDate(ws.getDate()+d);const iso=`${day.getFullYear()}-${String(day.getMonth()+1).padStart(2,'0')}-${String(day.getDate()).padStart(2,'0')}`,outside=day.getMonth()!==m,dayEvents=events.filter(e=>e.date===iso);
...
/* Projekt Bau 2.9.81: non-destructive ERP navigation shell. */
(()=>{
  ...
  ready(()=>{
    ...
    const roadmap={
      ...
      Hilfe:'Projekt Bau 2.9.81 PRO CLEAN · Lokale Daten, OneDrive, PDF, Aufmass sowie 2D/3D CAD bleiben unverändert verfügbar.'
    };
...
(()=>{
  const VERSION='2.9.81 PRO CLEAN';
