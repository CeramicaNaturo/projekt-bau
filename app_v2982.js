...
function pbProjectLowerHtml(project){
  ...
  return `<div class="pb-project-lower-mask">
    ...
    <section class="pb-lower-card">
      <div class="pb-lower-title"><h3>▤ Dokumente</h3></div>
      <div class="pb-doc-filter"><input data-doc-search placeholder="Suche"><input placeholder="von"><input placeholder="bis"></div>
      <div class="pb-doc-tabs">${documentsTabs.map(x=>`<button type="button" data-project-doc-tab="${esc(x)}">${esc(x)} <b>${pbProjectRelationCounts(project)[x]??''}</b></button>`).join('')}</div>
      <div class="pb-doc-list" data-doc-list>${pbProjectDocumentsHtml(project)}</div>
    </section>
...
function pbBindProjectDetail(workspace,project){
  ...
  workspace.querySelectorAll('[data-project-doc-tab]').forEach(btn=>btn.addEventListener('click',()=>{
    const tab=btn.dataset.projectDocTab;
    if(tab==='Termine'){const cust=(S.customers||[]).find(x=>x.id===project.customerId);if(cust)pbOpenAppointmentEditor(workspace,cust,project.id);return}
    if(tab==='Aufmass'||tab==='Offerten'){const c=ensurePlattenlegerProject(project);c.phase=tab==='Aufmass'?'aufmass':'offerte';A=project.id;save();document.body.classList.remove('pb-module-mode');workspace.classList.add('hidden');renderP();document.getElementById('panel')?.scrollIntoView({behavior:'smooth',block:'start'});return}
    const map={'Regieberichte':'Regieberichte','Aufträge':'Aufträge','Rechnungen':'Rechnungen','Zahlungen':'Zahlungen','Verträge':'Verträge'};
    if(map[tab]){[...document.querySelectorAll('.cad-nav-item[data-module]')].find(x=>x.dataset.module===map[tab])?.click();return}
    pbActionToast(`${tab}: Projektverknüpfungen angezeigt.`);
  }));
...
async function pbBrandLogoData(){
  ...
  try{const blob=await fetch('./logo_ceramica.jpg?v=298201').then(r=>r.blob());pbBrandLogoCache=await new Promise((resolve,reject)=>{const fr=new FileReader();fr.onload=()=>resolve(fr.result);fr.onerror=reject;fr.readAsDataURL(blob)});return pbBrandLogoCache}catch(_){return null}
}
...
/* Projekt Bau 2.9.82: non-destructive ERP navigation shell. */
(()=>{
  ...
  ready(()=>{
    ...
    const roadmap={
      ...
      Hilfe:'Projekt Bau 2.9.82 PRO CLEAN · Lokale Daten, OneDrive, PDF, Aufmass sowie 2D/3D CAD bleiben unverändert verfügbar.'
    };
...
(()=>{
  const VERSION='2.9.82 PRO CLEAN';
