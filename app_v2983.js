...
function pbProjectCalendarSync(project){
  ...
  if(project.startDate)project.calendarEntries.push({id:`project-period-${project.id}`,source:'project-period',projectId:project.id,title:project.name||'Projekt',date:project.startDate,endDate:project.endDate||project.startDate,startTime:'',endTime:'',type:'Projekt',status:project.status||'Aktiv',employee:project.owner||'',updatedAt:new Date().toISOString()});
}
...
function pbBindProjectDetail(workspace,project){
  ...
  workspace.querySelector('[data-task-add]')?.addEventListener('click',()=>{
    ...
    modal.querySelector('[data-task-modal-save]')?.addEventListener('click',()=>{const title=modal.querySelector('[data-task-new-title]').value.trim(),dt=modal.querySelector('[data-task-new-date]').value;if(!title){alert('Bitte eine Aufgabe eingeben.');return}project.projectTasks=Array.isArray(project.projectTasks)?project.projectTasks:[];const task={id:u(),title,date:dt?dt.slice(0,10):'',time:dt?dt.slice(11):'',priority:modal.querySelector('[data-task-new-priority]').value,notes:modal.querySelector('[data-task-new-notes]').value,done:false,createdAt:new Date().toISOString()};project.projectTasks.push(task);if(dt){project.appointments=Array.isArray(project.appointments)?project.appointments:[];project.appointments.push({id:`task-${task.id}`,source:'project-task',taskId:task.id,projectId:project.id,customerId:project.customerId,title:task.title,date:task.date,startTime:task.time,endTime:'',type:'Aufgabe',status:'Geplant',notes:task.notes,employee:project.owner||'',createdAt:new Date().toISOString()})}pbProjectHistoryAdd(project,'Projektaufgabe erstellt',`${task.title}${task.date?' · '+task.date+' '+task.time:''}`,'Aufgabe');save();modal.remove();pbNavigateToProject(project.id)});
  });
  workspace.querySelectorAll('[data-task-index]').forEach(row=>{
    ...
    row.querySelector('[data-task-done]')?.addEventListener('change',e=>{task.done=e.target.checked;pbProjectHistoryAdd(project,'Aufgabenstatus geändert',`${task.title}: ${task.done?'Erledigt':'Offen'}`,'Aufgabe');save()});
    row.querySelector('[data-task-title]')?.addEventListener('change',e=>{const old=task.title;task.title=e.target.value;pbProjectHistoryAdd(project,'Projektaufgabe geändert',`${old} → ${task.title}`,'Aufgabe');save()});
    row.querySelector('[data-task-date]')?.addEventListener('change',e=>{task.date=e.target.value;pbProjectHistoryAdd(project,'Aufgabentermin geändert',`${task.title}: ${task.date}`,'Aufgabe');save()});
    row.querySelector('[data-task-priority]')?.addEventListener('change',e=>{task.priority=e.target.value;pbProjectHistoryAdd(project,'Aufgabenpriorität geändert',`${task.title}: ${task.priority}`,'Aufgabe');save()});
    row.querySelector('[data-task-delete]')?.addEventListener('click',()=>{if(!confirm('Aufgabe wirklich löschen?'))return;const old=task.title;project.projectTasks.splice(i,1);project.appointments=(project.appointments||[]).filter(x=>x.taskId!==task.id);pbProjectHistoryAdd(project,'Projektaufgabe gelöscht',old,'Aufgabe');save();pbNavigateToProject(project.id)});
  });
...
async function pbBrandLogoData(){
  ...
  try{const blob=await fetch('./logo_ceramica.jpg?v=298301').then(r=>r.blob());pbBrandLogoCache=await new Promise((resolve,reject)=>{const fr=new FileReader();fr.onload=()=>resolve(fr.result);fr.onerror=reject;fr.readAsDataURL(blob)});return pbBrandLogoCache}catch(_){return null}
}
...
function pbOpenAppointmentEditor(workspace,customer,projectId='',appointmentId=''){
  ...
  pbBindTap(modal.querySelector('[data-ap-save]'),async()=>{
    ...
    save();await window.PBStorage?.saveState?.(S,{reason:'appointment-save-2981'});try{window.ProjectBauOneDrive?.scheduleAutoSync?.('appointment-save')}catch(_){}
    modal.remove();pbRenderCustomerRelation(workspace,customer,'appointments');pbActionToast('Termin gespeichert.');
  });
  if(appointment)pbBindTap(modal.querySelector('[data-ap-delete]'),()=>{if(!confirm('Termin wirklich löschen?'))return;project.appointments=(project.appointments||[]).filter(x=>x.id!==appointment.id);pbProjectHistoryAdd(project,'Termin gelöscht',appointment.title||'Termin','Termin');save();modal.remove();pbRenderCustomerRelation(workspace,customer,'appointments')});
}
...
/* Projekt Bau 2.9.83: non-destructive ERP navigation shell. */
(()=>{
  ...
  ready(()=>{
    ...
    const roadmap={
      ...
      Hilfe:'Projekt Bau 2.9.83 PRO CLEAN · Lokale Daten, OneDrive, PDF, Aufmass sowie 2D/3D CAD bleiben unverändert verfügbar.'
    };
...
(()=>{
  const VERSION='2.9.83 PRO CLEAN';
