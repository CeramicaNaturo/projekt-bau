...
function pbRenderProjectRelation(workspace,project,type='projects'){
  ...
  if(type==='baudoku'){
    target.innerHTML=`<div class="pb-relation-tools"><strong>Baudoku</strong></div><div class="pb-baudoku-placeholder"><span>▧</span><strong>Baudokumentation</strong><p>Dieser Bereich ist vorbereitet und wird später gemeinsam bearbeitet.</p></div>`;
    return;
  }
...
async function pbBrandLogoData(){
  ...
  try{const blob=await fetch('./logo_ceramica.jpg?v=298801').then(r=>r.blob());pbBrandLogoCache=await new Promise((resolve,reject)=>{const fr=new FileReader();fr.onload=()=>resolve(fr.result);fr.onerror=reject;fr.readAsDataURL(blob)});return pbBrandLogoCache}catch(_){return null}
}
...
function pbRenderCustomerRelation(workspace,customer,type='projects'){
  ...
  else if(type==='baudoku')target.innerHTML=`<div class="pb-relation-tools"><strong>Baudoku</strong></div><div class="pb-baudoku-placeholder"><span>▧</span><strong>Baudokumentation</strong><p>Dieser Bereich ist vorbereitet und wird später gemeinsam bearbeitet.</p></div>`;
...
/* Projekt Bau 2.9.88: non-destructive ERP navigation shell. */
(()=>{
  ...
  ready(()=>{
    ...
    const roadmap={
      ...
      Hilfe:'Projekt Bau 2.9.88 PRO CLEAN · Lokale Daten, OneDrive, PDF, Aufmass sowie 2D/3D CAD bleiben unverändert verfügbar.'
    };
...
(()=>{
  const VERSION='2.9.88 PRO CLEAN';
