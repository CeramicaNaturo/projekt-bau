...
S.customers=Array.isArray(S.customers)?S.customers:[];S.meta=S.meta&&typeof S.meta==='object'?S.meta:{};S.meta.counters=S.meta.counters&&typeof S.meta.counters==='object'?S.meta.counters:{};
...
/* ============================================================
   2.9.85 – GLOBALE DATENSYNCHRONISATION
   Ein Kunde, ein Projekt und die dazugehörigen Dokumente verwenden
   dieselben Stammdaten. Alle Module greifen auf dieselben verknüpften
   Datensätze zu; keine separaten Kopien als Datenquelle.
   ============================================================ */
function pbCustomerFullAddress(c){
  if(!c)return '';
  const direct=String(c.fullAddress||'').trim();
  if(direct)return direct;
  const street=[c.street,c.houseNo].filter(Boolean).join(' ').trim();
  const city=[c.zip,c.city].filter(Boolean).join(' ').trim();
  return [street,city].filter(Boolean).join(', ');
}
function pbMasterSnapshot(project,customer){
  const paymentDays=Math.max(0,Number(customer?.bank?.paymentDays)||Number(project?.paymentDays)||0);
  return {
    ...
    customerName:customer?pbCustomerName(customer):(project?.customer||''),
    customerAddress:customer?pbCustomerFullAddress(customer):(project?.address||''),
    customerEmail:customer?.email||'',
    customerPhone:customer?.phone||customer?.mobile||project?.phone||'',
    owner:project?.owner||'',
    paymentDays,
    paymentTermDays:paymentDays
  };
}
function pbStampLinkedRecord(record,project,customer,type=''){
  if(!record||typeof record!=='object')return record;
  Object.assign(record,pbMasterSnapshot(project,customer));
  record.recordType=record.recordType||type||'';
  record.updatedAt=record.updatedAt||project?.updatedAt||new Date().toISOString();
  if(record.paymentDays){
    record.paymentTermDays=record.paymentDays;
    if(!record.payment)record.payment=`Zahlbar innert ${record.paymentDays} Tagen ab Rechnungsdatum.`;
  }
  return record;
}
function pbSyncProjectLinkedRecords(project,customer){
  if(!project)return;
  const snap=pbMasterSnapshot(project,customer);
  project.customerId=snap.customerId;
  project.customer=snap.customerName;
  project.phone=snap.customerPhone;
  if(!project.address && snap.customerAddress)project.address=snap.customerAddress;

  project.plattenleger=project.plattenleger||{};
  const pd=project.plattenleger.phaseData=project.plattenleger.phaseData||{};
  Object.entries(pd).forEach(([key,value])=>{
    if(value&&typeof value==='object')pbStampLinkedRecord(value,project,customer,key);
  });

  const arrays={
    appointments:'Termin',
    projectTasks:'Aufgabe',
    orders:'Auftrag',
    regieReports:'Regiebericht',
    deliveryNotes:'Lieferschein',
    deliveries:'Lieferschein',
    acceptanceProtocols:'Abnahmeprotokoll',
    invoices:'Rechnung',
    payments:'Zahlung',
    reminders:'Mahnung',
    forms:'Formular',
  ...
  Object.entries(arrays).forEach(([key,type])=>{
    project[key]=Array.isArray(project[key])?project[key]:[];
    project[key].forEach(record=>pbStampLinkedRecord(record,project,customer,type));
  });

  // Customer payment target must always be identical in Offerte/Rechnung/Invoices.
  if(snap.paymentDays){
    pd.offerte=pd.offerte||{};
    pd.rechnung=pd.rechnung||{};
    [pd.offerte,pd.rechnung].forEach(x=>{
      pbStampLinkedRecord(x,project,customer,x===pd.offerte?'Offerte':'Rechnung');
      x.paymentDays=snap.paymentDays;
      x.paymentTermDays=snap.paymentDays;
      x.payment=`Zahlbar innert ${snap.paymentDays} Tagen ab Rechnungsdatum.`;
    });
    project.invoices.forEach(x=>{
      x.paymentDays=snap.paymentDays;
      x.paymentTermDays=snap.paymentDays;
      x.payment=x.payment||`Zahlbar innert ${snap.paymentDays} Tagen ab Rechnungsdatum.`;
    });
  }

  if(typeof pbImportLegacyProjectTasksForCalendar==='function')pbImportLegacyProjectTasksForCalendar(project);
  if(typeof pbProjectCalendarSync==='function')pbProjectCalendarSync(project);
}
function pbBuildLinkedDataIndex(){
  const index={
    customers:[],projects:[],appointments:[],tasks:[],measurements:[],offers:[],
    orders:[],regieReports:[],deliveryNotes:[],acceptanceProtocols:[],invoices:[],
    payments:[],reminders:[],forms:[],contracts:[],emails:[],letters:[],calendar:[],
    materials:[]
  };
  (S.customers||[]).forEach(c=>index.customers.push(c));
  (S.projects||[]).forEach(project=>{
    index.projects.push(project);
    const pd=project.plattenleger?.phaseData||{};
    if(pd.aufmass)index.measurements.push(pd.aufmass);
    if(pd.offerte)index.offers.push(pd.offerte);
    if(pd.rechnung)index.invoices.push(pd.rechnung);
    const map={
      appointments:'appointments',projectTasks:'tasks',orders:'orders',
      regieReports:'regieReports',deliveryNotes:'deliveryNotes',deliveries:'deliveryNotes',
      acceptanceProtocols:'acceptanceProtocols',invoices:'invoices',payments:'payments',
      reminders:'reminders',forms:'forms',contracts:'contracts',sentEmails:'emails',
      letters:'letters',calendarEntries:'calendar'
    };
    Object.entries(map).forEach(([src,dst])=>(project[src]||[]).forEach(x=>index[dst].push(x)));
  });
  (S.materialCatalog||[]).forEach(x=>index.materials.push(x));
  S.linkedData=index;
  S.meta=S.meta||{};
  S.meta.linkedDataUpdatedAt=new Date().toISOString();
  return index;
}
function pbSynchronizeAllData(){
  S.customers=Array.isArray(S.customers)?S.customers:[];
  S.projects=Array.isArray(S.projects)?S.projects:[];
  pbEnsureCustomerRecords();

  // Normalize customer master data once.
  S.customers.forEach(customer=>{
    customer.fullAddress=pbCustomerFullAddress(customer);
    customer.updatedAt=customer.updatedAt||customer.createdAt||new Date().toISOString();
  });

  // Every project and nested record points back to the same customer/project master data.
  S.projects.forEach(project=>{
    const customer=S.customers.find(c=>c.id===project.customerId)||
      S.customers.find(c=>pbCustomerName(c).toLocaleLowerCase('de-CH')===String(project.customer||'').toLocaleLowerCase('de-CH'));
    if(customer&&!project.customerId)project.customerId=customer.id;
    pbSyncProjectLinkedRecords(project,customer);
  });

  // Global invoice list, if present, receives the same snapshots.
  S.invoices=Array.isArray(S.invoices)?S.invoices:[];
  S.invoices.forEach(invoice=>{
    const project=S.projects.find(p=>p.id===invoice.projectId);
    const customer=S.customers.find(c=>c.id===(invoice.customerId||project?.customerId));
    if(project)pbStampLinkedRecord(invoice,project,customer,'Rechnung');
  });

  pbBuildLinkedDataIndex();
  return S;
}
function pbLinkedData(type,filters={}){
  const rows=Array.isArray(S.linkedData?.[type])?S.linkedData[type]:[];
  return rows.filter(row=>Object.entries(filters).every(([k,v])=>v==null||v===''||String(row?.[k]??'')===String(v)));
}
window.ProjectBauData={
  sync:()=>{pbSynchronizeAllData();save()},
  get:(type,filters={})=>pbLinkedData(type,filters),
  customer:id=>(S.customers||[]).find(x=>x.id===id)||null,
  project:id=>(S.projects||[]).find(x=>x.id===id)||null,
  relatedToProject:(projectId)=>Object.fromEntries(Object.keys(S.linkedData||{}).map(k=>[k,pbLinkedData(k,{projectId})])),
  relatedToCustomer:(customerId)=>Object.fromEntries(Object.keys(S.linkedData||{}).map(k=>[k,pbLinkedData(k,{customerId})]))
};

function pbSearchAllData(query=''){
  pbSynchronizeAllData();
  const q=String(query||'').trim().toLocaleLowerCase('de-CH');
  if(!q)return [];
  const out=[];
  Object.entries(S.linkedData||{}).forEach(([type,rows])=>{
    (rows||[]).forEach(row=>{
      const text=Object.values(row||{}).filter(v=>['string','number'].includes(typeof v)).join(' ').toLocaleLowerCase('de-CH');
      if(text.includes(q))out.push({type,row});
    });
  });
  return out.slice(0,100);
}
window.ProjectBauData.search=pbSearchAllData;
...
function pbNextNumber(prefix){
  const year=new Date().getFullYear(),key=`${prefix}-${year}`;S.meta=S.meta||{};S.meta.counters=S.meta.counters||{};
  let n=Number(S.meta.counters[key]||0);if(!n){const rx=new RegExp(`^${prefix}-${year}-(\\d+)$`);const values=[];const walk=v=>{if(!v||typeof v!=='object')return;if(Array.isArray(v)){v.forEach(walk);return}Object.values(v).forEach(x=>{if(typeof x==='string'){const m=x.match(rx);if(m)values.push(Number(m[1]))}else walk(x)})};walk({projects:S.projects,customers:S.customers});n=Math.max(0,...values)}
  n++;S.meta.counters[key]=n;return `${prefix}-${year}-${String(n).padStart(5,'0')}`;
}
function pbCustomerName(c){return String(c?.company||[c?.firstName,c?.lastName].filter(Boolean).join(' ')||'').trim()}
function pbEnsureCustomerRecords(){
  S.customers=Array.isArray(S.customers)?S.customers:[];(S.projects||[]).forEach(p=>{if(p.customerId||!p.customer)return;let c=S.customers.find(x=>pbCustomerName(x).toLocaleLowerCase('de-CH')===String(p.customer).toLocaleLowerCase('de-CH'));if(!c){c={id:u(),number:pbNextNumber('K'),type:'Privat',company:p.customer,phone:p.phone||'',street:p.address||'',createdAt:new Date().toISOString()};S.customers.push(c)}p.customerId=c.id});
}
function pbApplyCustomerToProject(p,c){
  p.customerId=c?.id||'';p.customer=c?pbCustomerName(c):'';p.phone=c?.phone||c?.mobile||'';if(c?.fullAddress)p.address=c.fullAddress;
  const paymentDays=Math.max(0,Number(c?.bank?.paymentDays)||0);
  const snapshot={customerId:p.customerId,customerName:p.customer,customerAddress:p.address,customerPhone:p.phone,customerEmail:c?.email||'',paymentDays,paymentTermDays:paymentDays};
  p.plattenleger=p.plattenleger||{};const pd=p.plattenleger.phaseData=p.plattenleger.phaseData||{};Object.values(pd).forEach(d=>{if(d&&typeof d==='object')Object.assign(d,snapshot)});(p.orders||[]).forEach(o=>Object.assign(o,snapshot));
  if(paymentDays){
    pd.offerte=pd.offerte||{};pd.offerte.paymentDays=paymentDays;pd.offerte.payment=`Zahlbar innert ${paymentDays} Tagen ab Rechnungsdatum.`;
    pd.rechnung=pd.rechnung||{};pd.rechnung.paymentDays=paymentDays;pd.rechnung.paymentTermDays=paymentDays;pd.rechnung.payment=`Zahlbar innert ${paymentDays} Tagen ab Rechnungsdatum.`;
    (p.invoices||[]).forEach(invoice=>{invoice.paymentDays=paymentDays;invoice.paymentTermDays=paymentDays});
...
$('create').onclick=()=>{
  ...
  S.projects.unshift({id:u(),projectNo:pbNextNumber('PRO'),name:$('name').value.trim(),address:$('address').value.trim(),customer:$('customer').value.trim(),phone:$('phone').value.trim(),startDate:$('startDate').value,owner:$('owner').value.trim(),description:$('description').value.trim(),areas:[]});
...
function pbSaveProjectDetails(){
  ...
  const customer=S.customers.find(c=>c.id===$('editProjectCustomer')?.value);pbApplyCustomerToProject(p,customer);
  p.phone=$('editProjectPhone')?.value?.trim()||p.phone||'';
...
function pbProjectStatusSteps(){
  return ['neu','Angebotserstellung','Vertrieb','beauftragt','verloren','Auftragserfüllung','Rechnung','warten auf Zahlungseingang','abgeschlossen'];
}
function pbProjectFinancials(project){
  const c=ensurePlattenlegerProject(project),off=pbOfferDraft(c),tot=pbOfferTotals(off);
  ...
  const orderGross=orders.reduce((s,x)=>s+(Number(x.gross)||0),0);
  const invoiceGross=invoices.reduce((s,x)=>s+(Number(x.gross||x.total)||0),0);
  const paid=payments.reduce((s,x)=>s+(Number(x.amount)||0),0);
  const receivable=Math.max(0,invoiceGross-paid);
  return {offer:tot.gross||0,orders:orderGross||tot.gross||0,invoices:invoiceGross,payments:paid,receivable,open:receivable};
}

function pbProjectHistory(project){
  project.history=Array.isArray(project.history)?project.history:[];
  return project.history;
}
function pbProjectHistoryAdd(project,title,detail='',kind='Änderung'){
  if(!project)return;
  const uinfo=window.ProjectBauAuth?.current?.();
  pbProjectHistory(project).unshift({
    id:u(),at:new Date().toISOString(),kind,title:String(title||'Projekt geändert'),
    detail:String(detail||''),user:uinfo?.name||uinfo?.email||'Projekt Bau'
  });
  if(project.history.length>500)project.history.length=500;
}
function pbProjectHistoryHtml(project){
  const items=pbProjectHistory(project);
  return items.map(x=>{
    const dt=new Date(x.at);const date=Number.isNaN(dt.getTime())?'':dt.toLocaleString('de-CH',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'});
    return `<div class="pb-history-row" data-history-search="${esc(`${x.kind} ${x.title} ${x.detail} ${x.user}`.toLowerCase())}">
      <span class="pb-history-dot">↻</span><div><strong>${esc(x.title)}</strong><small>${esc(date)} · ${esc(x.kind)} · ${esc(x.user)}</small>${x.detail?`<p>${esc(x.detail)}</p>`:''}</div>
    </div>`;
  }).join('')||'<p class="pb-empty-note">Noch keine Änderungen protokolliert.</p>';
}
function pbProjectTasksHtml(project){
  project.projectTasks=Array.isArray(project.projectTasks)?project.projectTasks:[];
  return project.projectTasks.map((x,i)=>`<div class="pb-project-task-row" data-task-index="${i}">
    <input type="checkbox" data-task-done ${x.done?'checked':''}>
    <input data-task-title value="${esc(x.title||'')}" placeholder="Aufgabe">
    <input type="date" data-task-date value="${esc(x.date||'')}">
    <select data-task-priority>${['Niedrig','Normal','Hoch','Dringend'].map(v=>`<option ${v===(x.priority||'Normal')?'selected':''}>${v}</option>`).join('')}</select>
    <button type="button" class="danger" data-task-delete>×</button>
  </div>`).join('')||'<p class="pb-empty-note">Noch keine Projektaufgaben vorhanden.</p>';
}
function pbProjectDocumentsHtml(project){
  const c=ensurePlattenlegerProject(project),d=c.phaseData||{};
  const rows=[];
  const add=(type,no,title,status='')=>rows.push({type,no,title,status});
  if(d.aufmass)add('Aufmass',d.aufmass.measureNo||'',d.aufmass.measureTitle||project.name,'');
  if(d.offerte)add('Offerten',d.offerte.offerNo||'',d.offerte.title||project.name,d.offerte.status||'');
  (project.orders||[]).forEach(x=>add('Aufträge',x.orderNo||x.sourceOfferNo||'',x.title||project.name,x.status||''));
  (project.regieReports||[]).forEach(x=>add('Regieberichte',x.number||'',x.title||'Regiebericht',x.status||''));
  (project.invoices||[]).forEach(x=>add('Rechnungen',x.invoiceNo||x.number||'',x.title||'Rechnung',x.status||''));
  (project.payments||[]).forEach(x=>add('Zahlungen',x.number||'',x.title||'Zahlung',x.status||''));
  return rows.map(x=>`<div class="pb-doc-row" data-doc-search="${esc(`${x.type} ${x.no} ${x.title} ${x.status}`.toLowerCase())}"><span>${esc(x.type)}</span><span>${esc(x.no)}</span><span>${esc(x.title)}</span><span>${esc(x.status)}</span></div>`).join('')||'<p class="pb-empty-note">Noch keine verknüpften Dokumente vorhanden.</p>';
}

const PB_PROJECT_RELATIONS=[
  ['projects','⌘','Projekte'],['appointments','▣','Termine'],['baudoku','▧','Baudoku'],
  ['measurements','⌑','Aufmass'],['offers','▤','Offerten'],['regie','▧','Regieberichte'],
  ['orders','↔','Aufträge'],['delivery','▣','Lieferscheine'],['acceptance','⌂','Abnahmeprotokolle'],
  ['invoices','▤','Rechnungen'],['payments','▣','Zahlungen'],['reminders','♧','Mahnungen'],
  ['forms','▥','Formulare'],['contracts','▧','Verträge'],['emails','✉','Gesendete E-Mails'],['letters','✉','Briefe']
];
function pbProjectRelationItems(project,type){
  pbSynchronizeAllData();
  const c=ensurePlattenlegerProject(project),d=c.phaseData||{};
  if(type==='projects')return [project];
  if(type==='appointments')return (project.appointments||project.dates||[]);
  if(type==='baudoku')return [];
  if(type==='measurements')return d.aufmass?[{...d.aufmass,_project:project}]:[];
  if(type==='offers')return d.offerte?[{...d.offerte,_project:project}]:[];
  if(type==='regie')return project.regieReports||[];
  if(type==='orders')return project.orders||[];
  if(type==='delivery')return project.deliveryNotes||[];
  if(type==='acceptance')return project.acceptanceProtocols||[];
  if(type==='invoices')return project.invoices||[];
  if(type==='payments')return project.payments||[];
  if(type==='reminders')return project.reminders||[];
  if(type==='forms')return project.forms||[];
  if(type==='contracts')return project.contracts||[];
  if(type==='emails')return project.sentEmails||[];
  if(type==='letters')return project.letters||[];
  return [];
}
function pbProjectRelationBadge(project,type){return pbProjectRelationItems(project,type).length}
function pbProjectRelationsHtml(project){
  return `<section class="pb-project-relations-1zu1">
    <div class="pb-customer-relation-tabs pb-project-relation-tabs">
      ${PB_PROJECT_RELATIONS.map(([type,icon,label],i)=>`<button type="button" data-project-relation="${type}" class="${i===0?'active':''}"><span>${icon}</span>${label}${pbProjectRelationBadge(project,type)?`<b>${pbProjectRelationBadge(project,type)}</b>`:''}</button>`).join('')}
    </div>
    <div class="pb-customer-relation-content pb-project-relation-content" data-project-relation-content></div>
  </section>`;
}
function pbProjectRelationRowTitle(type,x,project){
  if(type==='projects')return project.name||'Projekt';
  return x.title||x.name||x.offerNo||x.measureNo||x.orderNo||x.invoiceNo||x.number||x.subject||({
    appointments:'Termin',measurements:'Aufmass',offers:'Offerte',regie:'Regiebericht',orders:'Auftrag',delivery:'Lieferschein',
    acceptance:'Abnahmeprotokoll',invoices:'Rechnung',payments:'Zahlung',reminders:'Mahnung',forms:'Formular',contracts:'Vertrag',emails:'E-Mail',letters:'Brief'
  }[type]||'Eintrag');
}
function pbRenderProjectRelation(workspace,project,type='projects'){
  const target=workspace.querySelector('[data-project-relation-content]');if(!target)return;
  const items=pbProjectRelationItems(project,type),label=PB_PROJECT_RELATIONS.find(x=>x[0]===type)?.[2]||type;
  if(type==='projects'){
    target.innerHTML=`<div class="pb-relation-tools"><strong>Projekte</strong></div>
      <div class="pb-relation-table"><div class="pb-relation-row head"><span>ERSTELLT</span><span>NUMMER</span><span>BEZEICHNUNG</span><span>STATUS</span></div>
      <button type="button" class="pb-relation-row" data-project-self><span>${fmtDate((project.createdAt||project.startDate||'').slice?.(0,10)||'')}</span><span>${esc(project.projectNo||project.id||'')}</span><span>${esc(project.name||'')}</span><span>${esc(project.status||'Aktiv')}</span></button></div>`;
    target.querySelector('[data-project-self]')?.addEventListener('click',()=>pbNavigateToProject(project.id));
    return;
  }
  if(type==='appointments'){
    target.innerHTML=`<div class="pb-relation-tools"><strong>Termine</strong><button type="button" class="primary" data-project-relation-add>+</button></div>
      <div class="pb-relation-cards">${items.map(x=>`<button type="button" data-project-appointment="${esc(x.id||'')}"><strong>${esc(pbProjectRelationRowTitle(type,x,project))}</strong><span>${fmtDate(x.date||'')} ${esc(x.startTime||'')}</span><small>${esc(x.status||'')}</small></button>`).join('')||'<p class="pb-relation-empty">Noch keine Termine vorhanden.</p>'}</div>`;
    const customer=(S.customers||[]).find(x=>x.id===project.customerId);
    target.querySelector('[data-project-relation-add]')?.addEventListener('click',()=>customer&&pbOpenAppointmentEditor(workspace,customer,project.id));
    target.querySelectorAll('[data-project-appointment]').forEach(btn=>btn.addEventListener('click',()=>customer&&pbOpenAppointmentEditor(workspace,customer,project.id,btn.dataset.projectAppointment)));
    return;
  }
  if(type==='baudoku'){
    pbRenderBaudoku(target,project);
    return;
  }
  const addable=['measurements','offers','regie','orders','delivery','acceptance','invoices','payments','reminders','forms','contracts','emails','letters'].includes(type);
  target.innerHTML=`<div class="pb-relation-tools"><strong>${esc(label)}</strong>${addable?'<button type="button" class="primary" data-project-relation-add>+</button>':''}</div>
    ${items.length?`<div class="pb-relation-cards">${items.map(x=>`<button type="button" data-project-relation-item><strong>${esc(pbProjectRelationRowTitle(type,x,project))}</strong><span>${esc(x.number||x.offerNo||x.measureNo||x.orderNo||x.invoiceNo||'')}</span><small>${fmtDate(x.date||x.offerDate||x.measureDate||x.createdAt||'')}</small></button>`).join('')}</div>`:`<p class="pb-relation-empty">Für dieses Projekt sind noch keine ${esc(label)} vorhanden.</p>`}`;

  const create=()=>{
    if(type==='measurements'||type==='offers'){
      const c=ensurePlattenlegerProject(project);c.phase=type==='measurements'?'aufmass':'offerte';A=project.id;save();
      document.body.classList.remove('pb-module-mode');workspace.classList.add('hidden');renderP();document.getElementById('panel')?.scrollIntoView({behavior:'smooth',block:'start'});return;
    }
    const moduleMap={regie:'Regieberichte',orders:'Aufträge',invoices:'Rechnungen',payments:'Zahlungen',contracts:'Verträge'};
    if(moduleMap[type]){[...document.querySelectorAll('.cad-nav-item[data-module]')].find(x=>x.dataset.module===moduleMap[type])?.click();return}
    pbActionToast(`${label}: Erstellung ist vorbereitet.`);
  };
  target.querySelector('[data-project-relation-add]')?.addEventListener('click',create);
  target.querySelectorAll('[data-project-relation-item]').forEach(btn=>btn.addEventListener('click',create));
}
function pbBindProjectRelations(workspace,project){
  const tabs=[...workspace.querySelectorAll('[data-project-relation]')];
  tabs.forEach(btn=>btn.addEventListener('click',()=>{
    tabs.forEach(x=>x.classList.toggle('active',x===btn));
    pbRenderProjectRelation(workspace,project,btn.dataset.projectRelation);
  }));
  pbRenderProjectRelation(workspace,project,'projects');
}

function pbProjectLowerHtml(project){
  const resources=Array.isArray(project.resources)?project.resources:[];
  const resourceText=resources.map(x=>x.name||x).filter(Boolean).join(', ');
  const created=project.createdAt?new Date(project.createdAt).toLocaleString('de-CH'):'-';
  const updated=project.updatedAt?new Date(project.updatedAt).toLocaleString('de-CH'):'-';
  const documentsTabs=['Termine','Aufmass','Offerten','Regieberichte','Aufträge','Lieferscheine','Abnahmeprotokolle','Rechnungen','Zahlungen','Mahnungen','Formulare','Verträge'];
  return `<div class="pb-project-lower-mask">
    <section class="pb-lower-card">
      <div class="pb-lower-title"><h3>⌘ Projektmanagement</h3></div>
      <div class="pb-management-grid">
        <label>Startdatum<input type="date" data-pm="startDate" value="${esc(project.startDate||'')}"></label>
        <label>Enddatum<input type="date" data-pm="endDate" value="${esc(project.endDate||'')}"></label>
        <label class="wide">Ressourcenbedarf<div class="pb-resource-line"><input data-pm="resources" value="${esc(resourceText)}" placeholder="Ressourcen / Mitarbeiter / Geräte"><button type="button" class="primary" data-resource-add>+</button></div></label>
      </div>
    </section>
    <section class="pb-lower-card">
      <div class="pb-lower-title"><h3>⌕ Kommentare</h3></div>
      <div class="pb-comment-entry"><textarea rows="3" data-project-comment placeholder="Kommentar eingeben und zum Abschliessen Button rechts drücken"></textarea><button type="button" class="primary" data-comment-add>⌕</button></div>
      <div class="pb-comment-list">${(project.comments||[]).map((x,i)=>`<div><strong>${esc(x.user||'Projekt Bau')}</strong><span>${esc(new Date(x.at).toLocaleString('de-CH'))}</span><p>${esc(x.text||'')}</p><button type="button" data-comment-delete="${i}">×</button></div>`).join('')}</div>
    </section>
    <section class="pb-lower-card">
      <div class="pb-lower-title"><h3>↻ Verlauf</h3></div>
      <div class="pb-history-filter"><select data-history-kind><option value="">Objektstatusfilter</option><option>Änderung</option><option>Status</option><option>Datei</option><option>Termin</option><option>Kommentar</option><option>Aufgabe</option></select><input data-history-search placeholder="Verlauf durchsuchen …"></div>
      <div class="pb-history-list" data-history-list>${pbProjectHistoryHtml(project)}</div>
    </section>
    <section class="pb-lower-card">
      <div class="pb-lower-title"><h3>☷ Projekt Aufgaben</h3><button type="button" class="pb-round primary" data-task-add>+</button></div>
      <div class="pb-project-task-list">${pbProjectTasksHtml(project)}</div>
    </section>
    ${pbProjectRelationsHtml(project)}
    <section class="pb-lower-card pb-files-placeholder"><div class="pb-lower-title"><h3>▱ Dateien</h3></div><div data-project-management-files></div></section>
    <section class="pb-lower-card pb-project-footer-history">
      <div class="pb-lower-title"><h3>↻ Verlauf</h3></div>
      <div class="pb-project-meta-grid"><div><small>Erstellt am</small><strong>${esc(created)}</strong><span>${esc(project.createdBy||'-')}</span></div><div><small>Geändert am</small><strong>${esc(updated)}</strong><span>${esc(project.updatedBy||'-')}</span></div><div><small>Gelöscht am</small><strong>-</strong><span>-</span></div></div>
    </section>
  </div>`;
}


function pbCustomerAddressSuggestions(project){
  const customer=(S.customers||[]).find(x=>x.id===project.customerId),set=new Set(),add=v=>{v=String(v||'').trim();if(v)set.add(v)};
  add(project.address);add(customer?.fullAddress);
  (customer?.addresses||[]).forEach(x=>add(x.fullAddress||[x.street,x.houseNo].filter(Boolean).join(' ')+(x.zip||x.city?`, ${[x.zip,x.city].filter(Boolean).join(' ')}`:'')));
  if(customer)add([customer.street,customer.houseNo].filter(Boolean).join(' ')+(customer.zip||customer.city?`, ${[customer.zip,customer.city].filter(Boolean).join(' ')}`:''));
  (S.projects||[]).filter(p=>p.customerId===project.customerId).forEach(p=>add(p.address));
  return [...set];
}
function pbOpenProjectAddressPicker(workspace,project){
  document.querySelector('.pb-address-picker-modal')?.remove();const items=pbCustomerAddressSuggestions(project),modal=document.createElement('div');modal.className='pb-relation-modal pb-address-picker-modal';
  modal.innerHTML=`<div class="pb-relation-dialog pb-address-picker-dialog"><div class="pb-relation-dialog-head"><div><small>OBJEKTADRESSE</small><h2>Adresse auswählen</h2></div><button type="button" data-address-close>×</button></div><div class="pb-address-picker-list">${items.map(x=>`<button type="button" data-address-value="${esc(x)}">${esc(x)}</button>`).join('')||'<p>Für diesen Kunden ist noch keine Adresse gespeichert.</p>'}</div><div class="pb-address-manual"><label>Adresse manuell eingeben<input data-address-manual placeholder="Strasse Hausnummer, PLZ Ort"></label><button type="button" class="primary" data-address-use>Übernehmen</button></div></div>`;
  document.body.appendChild(modal);const close=()=>modal.remove();modal.querySelectorAll('[data-address-close]').forEach(b=>b.addEventListener('click',close));
  modal.querySelectorAll('[data-address-value]').forEach(b=>b.addEventListener('click',()=>{const input=workspace.querySelector('[data-pd="address"]');if(input)input.value=b.dataset.addressValue;close()}));
  modal.querySelector('[data-address-use]')?.addEventListener('click',()=>{const v=modal.querySelector('[data-address-manual]')?.value.trim();if(v){const input=workspace.querySelector('[data-pd="address"]');if(input)input.value=v;close()}});
}
function pbProjectRelationCounts(project){
  const c=ensurePlattenlegerProject(project),d=c.phaseData||{};
  return {Termine:(project.appointments||[]).length,Aufmass:d.aufmass?1:0,Offerten:d.offerte?1:0,Regieberichte:(project.regieReports||[]).length,Aufträge:(project.orders||[]).length,Lieferscheine:(project.deliveryNotes||[]).length,Abnahmeprotokolle:(project.acceptanceProtocols||[]).length,Rechnungen:(project.invoices||[]).length,Zahlungen:(project.payments||[]).length,Mahnungen:(project.reminders||[]).length,Formulare:(project.forms||[]).length,Verträge:(project.contracts||[]).length};
}
function pbProjectCalendarSync(project){
  if(!project)return;
  project.calendarEntries=Array.isArray(project.calendarEntries)?project.calendarEntries:[];
  project.appointments=Array.isArray(project.appointments)?project.appointments:[];
  project.projectTasks=Array.isArray(project.projectTasks)?project.projectTasks:[];

  /* Projektmanagement: Start-/Enddatum als durchgehender Kalendereintrag. */
  project.calendarEntries=project.calendarEntries.filter(x=>x.source!=='project-period');
  if(project.startDate){
    project.calendarEntries.push({
  ...
  /* Projektaufgaben / Neue Aufgabe: je Aufgabe genau ein Termin im Projektkalender. */
  const validTaskIds=new Set(project.projectTasks.map(t=>t.id).filter(Boolean));
  project.appointments=project.appointments.filter(x=>x.source!=='project-task'||validTaskIds.has(x.taskId));
  project.projectTasks.forEach(task=>{
    if(!task.id)task.id=u();
    const existing=project.appointments.find(x=>x.source==='project-task'&&x.taskId===task.id);
    if(!task.date){
      if(existing)project.appointments=project.appointments.filter(x=>x!==existing);
      return;
    }
    ...
    Object.assign(event,{
      projectId:project.id,
      customerId:project.customerId||'',
      title:task.title||'Projektaufgabe',
      date:task.date,
      startTime:task.time||'',
      endTime:task.endTime||'',
      type:'Aufgabe',
      status:task.done?'Erledigt':'Geplant',
      notes:task.notes||'',
      priority:task.priority||'Normal',
      employee:task.employee||project.owner||'',
      updatedAt:new Date().toISOString()
    });
    if(!existing)project.appointments.push(event);
  });
}
function pbImportLegacyProjectTasksForCalendar(project){
  if(!project)return;
  project.projectTasks=Array.isArray(project.projectTasks)?project.projectTasks:[];
  (Array.isArray(project.tasks)?project.tasks:[]).forEach(x=>{
    const id=x.id||`legacy-task-${String(x.title||x.name||'task').replace(/\W+/g,'-')}-${x.date||x.dueDate||x.deadline||''}`;
    if(project.projectTasks.some(t=>t.id===id))return;
    project.projectTasks.push({
      id,
      title:x.title||x.name||x.description||'Projektaufgabe',
      date:x.date||x.dueDate||x.deadline||'',
      time:x.time||x.startTime||'',
      endTime:x.endTime||'',
      priority:x.priority||'Normal',
      notes:x.notes||x.description||'',
      done:!!(x.done||x.completed||x.status==='Erledigt'),
      employee:x.employee||x.owner||project.owner||''
    });
  });
}
function pbSyncAllProjectCalendarData(){
  (S.projects||[]).forEach(project=>{
    pbImportLegacyProjectTasksForCalendar(project);
    pbProjectCalendarSync(project);
  });
}

function pbProjectDetailHtml(project){
  const customer=(S.customers||[]).find(x=>x.id===project.customerId);
  const f=pbProjectFinancials(project),statuses=pbProjectStatusSteps(),status=String(project.status||'neu');
  const address=project.address||customer?.fullAddress||'';
  return `<div class="pb-project-detail">
    <div class="pb-project-detail-head">
      <div><span class="pb-project-detail-icon">⌘</span><h2>Projekt</h2></div>
      <div class="pb-project-detail-actions"><button type="button" class="secondary" data-project-detail-back>Zurück</button><button type="button" class="primary" data-project-detail-save>Speichern</button></div>
    </div>
    <div class="pb-project-statusbar">${statuses.map(x=>`<button type="button" data-project-status="${esc(x)}" class="${x===status?'active':''}">${esc(x)}</button>`).join('')}</div>
    <div class="pb-project-customer-block">
      <button type="button" class="pb-project-customer-link" data-project-customer="${esc(customer?.id||'')}"><strong>${esc(pbCustomerName(customer)||project.customer||'Kunde')}</strong><span>${esc(address)}</span></button>
    </div>
    <div class="pb-project-number">${esc(project.projectNo||project.id||'')}</div>
    <div class="pb-project-detail-form">
      <label>Bezeichnung<input data-pd="name" value="${esc(project.name||'')}"></label>
      <label>Status<select data-pd="status">${statuses.map(x=>`<option ${x===status?'selected':''}>${esc(x)}</option>`).join('')}</select></label>
      <label>Objektadresse<div class="pb-project-address-control"><input data-pd="address" list="pbProjectAddressList" value="${esc(address)}"><datalist id="pbProjectAddressList">${pbCustomerAddressSuggestions(project).map(x=>`<option value="${esc(x)}"></option>`).join('')}</datalist><button type="button" class="primary" data-project-address-add>+</button></div></label>
      <label>Projektleiter<input data-pd="owner" value="${esc(project.owner||'')}"></label>
      <label>Kundenreferenz<input data-pd="customerReference" value="${esc(project.customerReference||'')}"></label>
      <label class="wide">Mitarbeiter<input data-pd="employees" value="${esc((project.employees||[]).join(', '))}" placeholder="z. B. Vakkas Oruc"></label>
      <label class="wide">Beschreibung<textarea data-pd="description" rows="5">${esc(project.description||'')}</textarea></label>
    </div>
    <div class="pb-project-finance-grid">
      ${(()=>{const rc=pbProjectRelationCounts(project);return `
      <article><span>▦</span><div><small>Offerten · ${rc.Offerten}</small><strong>CHF ${formatCHNumber(f.offer,2)}</strong></div></article>
      <article><span>↔</span><div><small>Aufträge · ${rc.Aufträge}</small><strong>CHF ${formatCHNumber(f.orders,2)}</strong></div></article>
      <article><span>▤</span><div><small>Rechnungen · ${rc.Rechnungen}</small><strong>CHF ${formatCHNumber(f.invoices,2)}</strong></div></article>
      <article><span>▧</span><div><small>Zahlungen · ${rc.Zahlungen}</small><strong>CHF ${formatCHNumber(f.payments,2)}</strong></div></article>
      <article><span>!</span><div><small>Forderungen</small><strong>CHF ${formatCHNumber(f.receivable,2)}</strong></div></article>
      <article><span>▣</span><div><small>offene Forderung</small><strong>CHF ${formatCHNumber(f.open,2)}</strong></div></article>`})()}
    </div>
    ${pbProjectLowerHtml(project)}
    <section class="pb-project-detail-files pb-project-detail-files-legacy" data-project-detail-files></section>
  </div>`;
}
function pbBindProjectDetail(workspace,project){
  workspace.querySelector('[data-project-detail-back]')?.addEventListener('click',()=>history.back());
  workspace.querySelectorAll('[data-project-status]').forEach(btn=>btn.addEventListener('click',()=>{
    const oldStatus=project.status||'neu';project.status=btn.dataset.projectStatus;
    if(oldStatus!==project.status)pbProjectHistoryAdd(project,'Projektstatus geändert',`${oldStatus} → ${project.status}`,'Status');
    workspace.querySelectorAll('[data-project-status]').forEach(x=>x.classList.toggle('active',x===btn));
    const select=workspace.querySelector('[data-pd="status"]');if(select)select.value=project.status;
    save();
  }));
  workspace.querySelector('[data-project-customer]')?.addEventListener('click',()=>{
    const id=workspace.querySelector('[data-project-customer]')?.dataset.projectCustomer;if(!id)return;
    document.querySelector('.cad-nav-item[data-module="Kunden"]')?.click();
    setTimeout(()=>{const w=document.getElementById('pbModuleWorkspace');const c=(S.customers||[]).find(x=>x.id===id);if(c&&w)pbRenderCustomerEditorV2958(w,c)},80);
  });
  workspace.querySelector('[data-project-address-add]')?.addEventListener('click',()=>pbOpenProjectAddressPicker(workspace,project));
  workspace.querySelector('[data-project-detail-save]')?.addEventListener('click',async()=>{
    const v=k=>workspace.querySelector(`[data-pd="${k}"]`)?.value||'';
    const before={name:project.name||'',status:project.status||'',address:project.address||'',owner:project.owner||'',customerReference:project.customerReference||'',description:project.description||'',employees:(project.employees||[]).join(', ')};
    project.name=v('name').trim();project.status=v('status');project.address=v('address').trim();project.owner=v('owner').trim();
    project.customerReference=v('customerReference').trim();project.description=v('description');
    project.employees=v('employees').split(',').map(x=>x.trim()).filter(Boolean);
    const changes=[];const labels={name:'Bezeichnung',status:'Status',address:'Objektadresse',owner:'Projektleiter',customerReference:'Kundenreferenz',description:'Beschreibung',employees:'Mitarbeiter'};
    const after={name:project.name,status:project.status,address:project.address,owner:project.owner,customerReference:project.customerReference,description:project.description,employees:project.employees.join(', ')};
    Object.keys(before).forEach(k=>{if(String(before[k])!==String(after[k]))changes.push(`${labels[k]}: ${before[k]||'-'} → ${after[k]||'-'}`)});
    if(changes.length)pbProjectHistoryAdd(project,'Projektdaten geändert',changes.join(' | '),'Änderung');
    const pmv=k=>workspace.querySelector(`[data-pm="${k}"]`)?.value||'';
    const oldStart=project.startDate||'',oldEnd=project.endDate||'',oldRes=(project.resources||[]).map(x=>x.name||x).join(', ');
    project.startDate=pmv('startDate');project.endDate=pmv('endDate');project.resources=pmv('resources').split(',').map(x=>x.trim()).filter(Boolean).map(name=>({name}));
    const pmChanges=[];if(oldStart!==project.startDate)pmChanges.push(`Startdatum: ${oldStart||'-'} → ${project.startDate||'-'}`);if(oldEnd!==project.endDate)pmChanges.push(`Enddatum: ${oldEnd||'-'} → ${project.endDate||'-'}`);const newRes=project.resources.map(x=>x.name).join(', ');if(oldRes!==newRes)pmChanges.push(`Ressourcen: ${oldRes||'-'} → ${newRes||'-'}`);if(pmChanges.length)pbProjectHistoryAdd(project,'Projektmanagement geändert',pmChanges.join(' | '),'Änderung');
    pbProjectCalendarSync(project);
    project.updatedAt=new Date().toISOString();project.updatedBy=window.ProjectBauAuth?.current?.()?.name||window.ProjectBauAuth?.current?.()?.email||'Projekt Bau';save();
    try{await window.PBStorage?.saveState?.(S,{reason:'project-detail-save-2980'})}catch(_){}
    try{window.ProjectBauOneDrive?.scheduleAutoSync?.('project-detail-save')}catch(_){}
    pbActionToast('Projekt gespeichert.');
  });
  project.comments=Array.isArray(project.comments)?project.comments:[];
  workspace.querySelector('[data-comment-add]')?.addEventListener('click',()=>{
    const box=workspace.querySelector('[data-project-comment]'),text=String(box?.value||'').trim();if(!text)return;
    const user=window.ProjectBauAuth?.current?.()?.name||window.ProjectBauAuth?.current?.()?.email||'Projekt Bau';
    project.comments.unshift({id:u(),text,at:new Date().toISOString(),user});pbProjectHistoryAdd(project,'Kommentar hinzugefügt',text,'Kommentar');save();pbNavigateToProject(project.id);
  });
  workspace.querySelectorAll('[data-comment-delete]').forEach(btn=>btn.addEventListener('click',()=>{
    const i=Number(btn.dataset.commentDelete);const old=project.comments[i];if(!old)return;if(!confirm('Kommentar wirklich löschen?'))return;project.comments.splice(i,1);pbProjectHistoryAdd(project,'Kommentar gelöscht',old.text||'','Kommentar');save();pbNavigateToProject(project.id);
  }));
  workspace.querySelector('[data-task-add]')?.addEventListener('click',()=>{
    document.querySelector('.pb-project-task-modal')?.remove();const modal=document.createElement('div');modal.className='pb-relation-modal pb-project-task-modal';
    modal.innerHTML=`<div class="pb-relation-dialog"><div class="pb-relation-dialog-head"><div><small>PROJEKTAUFGABE</small><h2>Aufgabe erfassen</h2></div><button type="button" data-task-modal-close>×</button></div><div class="pb-task-modal-form"><label>Aufgabe<input data-task-new-title placeholder="Aufgabe"></label><label>Termin<input type="datetime-local" data-task-new-date></label><label>Priorität<select data-task-new-priority><option>Niedrig</option><option selected>Normal</option><option>Hoch</option><option>Dringend</option></select></label><label class="wide">Beschreibung<textarea data-task-new-notes rows="4"></textarea></label></div><div class="pb-relation-dialog-actions"><button type="button" class="secondary" data-task-modal-close>Abbrechen</button><button type="button" class="primary" data-task-modal-save>Aufgabe speichern</button></div></div>`;
    document.body.appendChild(modal);modal.querySelectorAll('[data-task-modal-close]').forEach(b=>b.addEventListener('click',()=>modal.remove()));
    modal.querySelector('[data-task-modal-save]')?.addEventListener('click',()=>{const title=modal.querySelector('[data-task-new-title]').value.trim(),dt=modal.querySelector('[data-task-new-date]').value;if(!title){alert('Bitte eine Aufgabe eingeben.');return}project.projectTasks=Array.isArray(project.projectTasks)?project.projectTasks:[];const task={id:u(),title,date:dt?dt.slice(0,10):'',time:dt?dt.slice(11):'',priority:modal.querySelector('[data-task-new-priority]').value,notes:modal.querySelector('[data-task-new-notes]').value,done:false,createdAt:new Date().toISOString()};project.projectTasks.push(task);pbProjectCalendarSync(project);pbProjectHistoryAdd(project,'Projektaufgabe erstellt',`${task.title}${task.date?' · '+task.date+' '+task.time:''}`,'Aufgabe');save();modal.remove();pbNavigateToProject(project.id)});
  });
  workspace.querySelectorAll('[data-task-index]').forEach(row=>{
    const i=Number(row.dataset.taskIndex),task=project.projectTasks?.[i];if(!task)return;
    row.querySelector('[data-task-done]')?.addEventListener('change',e=>{task.done=e.target.checked;pbProjectCalendarSync(project);pbProjectHistoryAdd(project,'Aufgabenstatus geändert',`${task.title}: ${task.done?'Erledigt':'Offen'}`,'Aufgabe');save()});
    row.querySelector('[data-task-title]')?.addEventListener('change',e=>{const old=task.title;task.title=e.target.value;pbProjectCalendarSync(project);pbProjectHistoryAdd(project,'Projektaufgabe geändert',`${old} → ${task.title}`,'Aufgabe');save()});
    row.querySelector('[data-task-date]')?.addEventListener('change',e=>{task.date=e.target.value;pbProjectCalendarSync(project);pbProjectHistoryAdd(project,'Aufgabentermin geändert',`${task.title}: ${task.date}`,'Aufgabe');save()});
    row.querySelector('[data-task-priority]')?.addEventListener('change',e=>{task.priority=e.target.value;pbProjectCalendarSync(project);pbProjectHistoryAdd(project,'Aufgabenpriorität geändert',`${task.title}: ${task.priority}`,'Aufgabe');save()});
    row.querySelector('[data-task-delete]')?.addEventListener('click',()=>{if(!confirm('Aufgabe wirklich löschen?'))return;const old=task.title;project.projectTasks.splice(i,1);pbProjectCalendarSync(project);pbProjectHistoryAdd(project,'Projektaufgabe gelöscht',old,'Aufgabe');save();pbNavigateToProject(project.id)});
  });
  const histFilter=()=>{const q=String(workspace.querySelector('[data-history-search]')?.value||'').toLowerCase(),kind=String(workspace.querySelector('[data-history-kind]')?.value||'').toLowerCase();workspace.querySelectorAll('[data-history-search]').forEach(r=>{if(r.tagName==='INPUT')return;const txt=r.dataset.historySearch||'';r.hidden=(q&&!txt.includes(q))||(kind&&!txt.includes(kind))})};
  workspace.querySelector('[data-history-search]')?.addEventListener('input',histFilter);workspace.querySelector('[data-history-kind]')?.addEventListener('change',histFilter);
  workspace.querySelector('[data-doc-search]')?.addEventListener('input',e=>{const q=e.target.value.toLowerCase();workspace.querySelectorAll('[data-doc-search]').forEach(r=>{if(r.tagName==='INPUT')return;r.hidden=q&&!String(r.dataset.docSearch||'').includes(q)})});
  pbBindProjectRelations(workspace,project);
  workspace.querySelectorAll('[data-pm="startDate"],[data-pm="endDate"],[data-pm="resources"]').forEach(el=>el.addEventListener('change',async()=>{
    const val=k=>workspace.querySelector(`[data-pm="${k}"]`)?.value||'';
    project.startDate=val('startDate');
    project.endDate=val('endDate');
    project.resources=val('resources').split(',').map(x=>x.trim()).filter(Boolean).map(name=>({name}));
    pbProjectCalendarSync(project);
    project.updatedAt=new Date().toISOString();
    save();
    try{await window.PBStorage?.saveState?.(S,{reason:'projectmanagement-calendar-sync-2984'})}catch(_){}
    try{window.ProjectBauOneDrive?.scheduleAutoSync?.('projectmanagement-calendar-sync')}catch(_){}
  }));
  workspace.querySelector('[data-resource-add]')?.addEventListener('click',()=>workspace.querySelector('[data-pm="resources"]')?.focus());
  pbMountOwnerFiles(workspace.querySelector('[data-project-management-files]'),'project',project.id,'Dateien');
  workspace.querySelector('[data-project-detail-files]')?.classList.add('hidden');
}
function pbNavigateToProject(projectId,phase=''){
  const project=S.projects.find(p=>p.id===projectId);if(!project)return false;
  pbImportLegacyProjectTasksForCalendar(project);pbProjectCalendarSync(project);
  A=project.id;if(phase)ensurePlattenlegerProject(project).phase=phase;
  const workspace=document.getElementById('pbModuleWorkspace');if(!workspace)return false;
  document.body.classList.remove('pb-customer-mode','pb-shell-focus');
  document.body.classList.add('pb-module-mode');
  document.querySelectorAll('.cad-main>section').forEach(x=>x.classList.remove('pb-shell-visible'));
  workspace.innerHTML=pbProjectDetailHtml(project);workspace.classList.remove('hidden');pbBindProjectDetail(workspace,project);
  workspace.scrollIntoView({behavior:'smooth',block:'start'});
  if(!window.__pbHistoryReplay)pbPushHistory({type:'project',projectId:project.id});
  return true;
}
...
pbBindTap($('closeBaudokumentation'),pbCloseBaudokumentation);
document.querySelectorAll('[data-baudoc-target]').forEach(btn=>pbBindTap(btn,()=>{pbCloseBaudokumentation();const nav=[...document.querySelectorAll('.cad-nav-item')].find(x=>x.textContent.trim()===btn.dataset.baudocTarget);nav?.click()}));
...
pbBindTap($('openProjectCalendar'),()=>{
  ...
  document.querySelectorAll('.cad-main>section').forEach(x=>x.classList.remove('pb-shell-visible'));
  if(typeof window.PBRenderProjectCalendarWorkspace!=='function'){document.querySelector('[data-module="Projektkalender"]')?.click();return}
  window.PBRenderProjectCalendarWorkspace(workspace,p.id);workspace.classList.remove('hidden');workspace.scrollIntoView({behavior:'smooth',block:'start'});
});
...
function render(){
  ...
  S.projects.forEach(p=>{
    ...
    // Normaler Klick: Maus sowie der von Samsung/Android nach Touch erzeugte Klick.
    d.addEventListener('click',ev=>{
      if(Date.now()-touchOpenedAt < 700) return; // Ghost-Click nach touchend
      ev.preventDefault();
      ev.stopPropagation();
      openProject();
    });

    // Tablet-Fallback: echte Touch-Ereignisse unabhängig vom PointerEvent-Verhalten.
    ...
    d.addEventListener('touchend',ev=>{
      if(touchMoved) return; // Beim Scrollen das Projekt nicht öffnen
...
const PL_PHASE_CONTENT={
  anfrage:{title:'Kundenanfrage',desc:'Kundenanfrage erfassen und direkt mit dem Kundenstamm verbinden.',fields:[
    ['requestNo','Anfrage-Nr.','text'],['requestDate','Eingangsdatum','date'],
    ['source','Anfragequelle','select',['Telefon','E-Mail','Webseite','Empfehlung','Persönlich','Hornbach','Architekt / Planer','Sonstiges']],
    ['requestType','Anfrageart','select',['Badumbau','Plattenarbeiten','Reparatur','Sanierung','Neubau','Dusche / WC','Küche','Naturstein','Epoxidharz','Sonstiges']],
    ['desiredDate','Gewünschter Ausführungstermin','date'],
    ['visitRequired','Besichtigung erforderlich','select',['Ja','Nein']],['visitDate','Besichtigungstermin','datetime-local'],
    ['responsible','Verantwortlicher Mitarbeiter','text'],['priority','Priorität','select',['Niedrig','Normal','Hoch','Dringend']],
    ['requestStatus','Status','select',['Neu','In Prüfung','Besichtigung geplant','Offerte erstellen','Abgelehnt','Gewonnen']],
    ['budget','Budget / Kostenvorstellung (CHF)','number'],
    ['requestNotes','Kurzbeschreibung / Kundenwunsch / Ausgangslage','textarea','wide'],
    ['internalNotes','Interne Notizen','textarea','wide']
  ],checks:[]},
...
function aufmassRepeat(name,data){
  ...
  const field=(row,key,placeholder,type='text')=>{
    if(key==='unit')return `<select data-array-field="${key}" aria-label="${esc(placeholder)}">${pbUnitOptions(row[key]||'m²')}</select>`;
    return `<input data-array-field="${key}" type="${type}" value="${esc(row[key]??'')}" placeholder="${esc(placeholder)}"${type==='number'?' step="0.01"':''}>`;
  };
  return `<div class="aufmass-repeat"><div class="aufmass-repeat-head"><strong>${esc(cfg.title)}</strong><button type="button" data-aufmass-add="${name}">${esc(cfg.add)}</button></div><div data-aufmass-list="${name}">`+
    rows.map((row,index)=>`<div class="aufmass-repeat-row" data-aufmass-array="${name}" style="--cols:${cfg.cols.length}">${cfg.cols.map(([key,placeholder,type='text'])=>field(row,key,placeholder,type)).join('')}<button type="button" data-aufmass-delete="${name}" data-index="${index}">×</button></div>`).join('')+'</div></div>';
}
/* Schweiz / SIA-orientierte Einheiten.
   Primär SI/SIA-kompatible Einheiten; zusätzlich gebräuchliche Schweizer
   Abrechnungseinheiten (Stk., Paar, Set, pauschal, GL, Packung usw.). */
const PB_MEASURE_UNITS=[
  'mm','cm','m','km',
  'mm²','cm²','m²',
  'cm³','dm³','m³',
  'Stk.','St.','Paar','Set','Satz',
  'Rolle','Packung','Sack','Gebinde','Palette',
  'g','kg','t',
  'ml','cl','dl','l',
  's','min','h','Std.','Tag','Wo.','Monat',
  '%','‰',
  'N','kN','N/m','kN/m','N/m²','kN/m²',
  'Pa','kPa','MPa',
  'W','kW','Wh','kWh',
  '°C','K',
  'm/s','m²/s','l/s','m³/h',
  'kg/m','kg/m²','kg/m³',
  'W/m','W/m²','W/(m·K)',
  'CHF','CHF/m','CHF/m²','CHF/m³','CHF/Stk.','CHF/h',
  'pauschal','GL',
  'm1'
];
function pbUnitOptions(value=''){
  const selected=String(value||'');
  const list=PB_MEASURE_UNITS.includes(selected)||!selected?PB_MEASURE_UNITS:[selected,...PB_MEASURE_UNITS];
  return list.map(u=>`<option value="${esc(u)}" ${u===selected?'selected':''}>${esc(u)}</option>`).join('');
}
function pbUnitSelect(attrs='',value='',fallback='m²'){
  const v=String(value||fallback);
  return `<select ${attrs}>${pbUnitOptions(v)}</select>`;
}

const PB_MEASURE_ITEM_CATALOG=[
  'Abbrucharbeiten','Abdeckarbeiten','Abdichtung','Abdichtung Boden','Abdichtung Wand',
  'Anschlussfugen','Ausgleichsspachtelung','Ausschnitte erstellen','Bauendreinigung',
  'Baustelleneinrichtung','Betonarbeiten','Boden ausgleichen','Bodenbelag entfernen',
  'Bodenplatten verlegen','Boden vorbereiten','Bohrungen erstellen','Dehnfugen',
  'Demontagearbeiten','Dichtband einbauen','Dichtmanschetten einbauen',
  'Eckschutzprofile montieren','Entsorgung & Recycling','Epoxidharzfugen',
  'Epoxidharzgrundierung','Epoxidharzbelag','Feinsteinzeug verlegen','Fenstersims',
  'Fliesen verlegen','Fugen auskratzen','Fugen erneuern','Gefälle ausbilden',
  'Gehrungsschnitte','Gerüst / Arbeitsbühne','Grundierung','Haftbrücke',
  'Hohlstellen sanieren','Kernbohrung','Kleber entfernen','Lieferung',
  'Mauerwerk erstellen','Mosaik verlegen','Naturstein verlegen','Nivelliersystem',
  'Platten schneiden','Platten verfugen','Plattenbelag entfernen','Plattenbelag verlegen',
  'Randabschlüsse','Reparaturarbeiten','Risssanierung','Schutzabdeckung',
  'Schützen von bestehenden Bauteilen','Silikonfugen','Sockelplatten verlegen',
  'Spachtelarbeiten','Spitzarbeiten','Stundenlohnarbeiten','Untergrund schleifen',
  'Untergrund vorbereiten','Verlegearbeiten','Wand ausgleichen','Wandplatten verlegen',
  'Wand vorbereiten','Wasserabdichtung','Zementfugen','Zusatzarbeiten'
];

function pbMeasureSuggestionSource(){
  const set=new Set(PB_MEASURE_ITEM_CATALOG);
  (S.projects||[]).forEach(p=>{
    const groups=p?.plattenleger?.phaseData?.aufmass?.measureGroups||[];
    groups.forEach(g=>(g.rows||[]).forEach(r=>{if(String(r.item||'').trim())set.add(String(r.item).trim())}));
    (p.tileMaterials||[]).forEach(m=>{if(String(m.designation||m.model||'').trim())set.add(String(m.designation||m.model).trim())});
  });
  (S.materialCatalog||[]).forEach(m=>{
    if(String(m.designation||'').trim())set.add(String(m.designation).trim());
    if(String(m.serviceText||'').trim()){
      String(m.serviceText).split(/\r?\n/).map(x=>x.trim()).filter(Boolean).slice(0,20).forEach(x=>set.add(x));
    }
  });
  return [...set].sort((x,y)=>x.localeCompare(y,'de-CH',{sensitivity:'base'}));
}
function pbMeasureSuggestions(query){
  const q=String(query||'').trim().toLocaleLowerCase('de-CH');
  const all=pbMeasureSuggestionSource();
  const result=q?all.filter(x=>x.toLocaleLowerCase('de-CH').includes(q)):all;
  return result.slice(0,10);
}
function pbCloseMeasureSuggest(except=null){
  document.querySelectorAll('.pb-measure-suggest').forEach(n=>{if(n!==except)n.remove()});
}
function pbOpenMeasureSuggest(input){
  pbCloseMeasureSuggest();
  const items=pbMeasureSuggestions(input.value);if(!items.length)return;
  const wrap=document.createElement('div');wrap.className='pb-measure-suggest';wrap.setAttribute('role','listbox');
  wrap.innerHTML=items.map((x,i)=>`<button type="button" role="option" data-suggest-index="${i}">${esc(x)}</button>`).join('');
  const host=input.closest('.pb-measure-item-cell')||input.parentElement;host.style.position='relative';host.appendChild(wrap);
  let active=-1;
  const choose=i=>{const b=wrap.querySelector(`[data-suggest-index="${i}"]`);if(!b)return;input.value=b.textContent;input.dispatchEvent(new Event('change',{bubbles:true}));pbCloseMeasureSuggest();input.focus()};
  const mark=i=>{const bs=[...wrap.querySelectorAll('button')];bs.forEach((b,k)=>b.classList.toggle('active',k===i));if(bs[i])bs[i].scrollIntoView({block:'nearest'})};
  wrap.addEventListener('pointerdown',e=>{const b=e.target.closest('button');if(!b)return;e.preventDefault();choose(Number(b.dataset.suggestIndex))});
  input._pbSuggestKey=e=>{
    const max=items.length-1;
    if(e.key==='ArrowDown'){e.preventDefault();active=Math.min(max,active+1);mark(active)}
    else if(e.key==='ArrowUp'){e.preventDefault();active=Math.max(0,active-1);mark(active)}
    else if(e.key==='Enter'&&active>=0){e.preventDefault();choose(active)}
    else if(e.key==='Escape'){pbCloseMeasureSuggest()}
  };
  input.addEventListener('keydown',input._pbSuggestKey,{once:true});
}
...
function pbRenderMeasureGroups(data){
  ...
  data.measureGroups=groups;data.selectedMaterials=Array.isArray(data.selectedMaterials)?data.selectedMaterials:[];
  const groupHtml=groups.map((g,gi)=>{const subtotal=pbMeasureSummary([g]);return `<section class="pb-measure-group" data-measure-group="${gi}"><div class="pb-measure-group-title"><input data-group-title value="${esc(g.title||'Raum')}" aria-label="Bereichsname"><span><button type="button" data-measure-row-add="${gi}">+</button><button type="button" class="danger" data-measure-group-delete="${gi}">×</button></span></div><div class="pb-measure-table-head"><span>Gegenstand</span><span>Berechnung</span><span>Formel</span><span>Anmerkung</span><span>Summe / Einheit</span><span></span></div><div class="pb-measure-rows">${(g.rows||[]).map((r,ri)=>{const total=pbMeasureNumber(r.factor||1)*pbMeasureFormula(r.formula);return `<div class="pb-measure-row" data-measure-row="${ri}"><div class="pb-measure-item-cell"><input data-mr="item" autocomplete="off" value="${esc(r.item||'')}" placeholder="Gegenstand"></div><input data-mr="factor" type="hidden" value="${esc(r.factor??'1')}"><select class="pb-formula-kind" aria-label="Berechnung"><option>Freie Formel</option></select><input data-mr="formula" value="${esc(r.formula||'')}" placeholder="z. B. 2,40 × 1,20"><input data-mr="note" value="${esc(r.note||'')}" placeholder="Anmerkung"><div class="pb-measure-result"><output>${formatCHNumber(total,3)}</output><select data-mr="unit">${pbUnitOptions(r.unit||'m²')}</select></div><button type="button" class="danger" data-measure-row-delete="${gi}:${ri}">×</button></div>`}).join('')}</div><textarea data-group-notes rows="3" placeholder="Notizen">${esc(g.notes||'')}</textarea><div class="pb-measure-subtotal"><strong>Zwischensumme</strong>${[...subtotal].map(([k,v])=>`<span>${esc(k.split('|||')[0])} <b>${formatCHNumber(v,2)} ${esc(k.split('|||')[1])}</b></span>`).join('')}</div></section>`}).join('');
  const p=cur()||{};
  const materials=data.selectedMaterials;
  const materialRows=materials.length?`<div class="pb90-mat-head"><span>Artikelnummer</span><span>Bezeichnung</span><span>Lieferant</span><span>Menge</span><span>Einheit</span><span>Angebotspreis</span><span></span></div>${materials.map((x,i)=>`<div class="pb90-mat-row" data-selected-material="${i}"><span>${pbMaterialEscape(x.articleNo)}</span><span><strong>${pbMaterialEscape(x.designation)}</strong></span><span>${pbMaterialEscape(x.supplier)}</span><input data-sm-qty value="${pbMaterialEscape(x.qty||'1')}" inputmode="decimal"><select data-sm-unit>${pbUnitOptions(x.unit||'m²')}</select><span>CHF ${pbMaterialMoney(x.offerPrice||0)}</span><button type="button" class="danger" data-sm-delete="${i}">×</button></div>`).join('')}`:'<div class="pb-material-empty">Noch kein Material ausgewählt.</div>';
  return `<div class="pb90-aufmass-shell">
    <div class="pb90-tabs"><button type="button" class="active">Neues Aufmass</button><button type="button" data-pb90-list>Aufmass-Liste</button><button type="button" data-pb90-eval>Auswertungen</button></div>
    <section class="pb90-position-capture"><div class="pb90-section-title"><h3>Positionen erfassen</h3><p>Wählen Sie die gewünschte Materialerfassung:</p></div>
      <div class="pb-material-choice-grid pb90-choice-grid">
        <article><span class="pb-choice-icon">▥</span><div><h4>Materialien auswählen</h4><p>Materialien aus der Materialdatenbank wählen.</p></div><button type="button" class="primary" data-aufmass-material-action="list">Materialien</button></article>
        <article><span class="pb-choice-icon">▦</span><div><h4>Materialien mit Kalkulation</h4><p>Materialien erfassen und mit Kalkulation.</p></div><button type="button" class="primary" data-aufmass-material-action="calc">Material erfassen (mit Kalkulation)</button></article>
        <article><span class="pb-choice-icon">▤</span><div><h4>Materialien ohne Kalkulation</h4><p>Materialien erfassen ohne Kalkulation.</p></div><button type="button" class="primary" data-aufmass-material-action="plain">Material erfassen (ohne Kalkulation)</button></article>
      </div>
    </section>
    <section class="pb90-selected"><div class="pb90-section-title"><h3>Ausgewählte Materialien</h3><p>Materialdaten werden aus den Stammdaten übernommen. Im Aufmass wird nur die benötigte Menge angepasst.</p></div><div class="pb90-mat-table">${materialRows}</div></section>
    <div class="pb-measure-document"><div class="pb-measure-topline"><strong>${esc(data.measureNo||'AUF-2026-00001')}</strong><label>Datum <input type="date" data-pl-phase-field="measureDate" value="${esc(data.measureDate||'')}"></label></div><div class="pb-measure-meta"><label>Bezeichnung<input data-pl-phase-field="measureTitle" value="${esc(data.measureTitle||'Sanierung Nasszellen')}"></label><label>Projekt<input value="${esc(`${p.name||''}${p.projectNo?' ('+p.projectNo+')':''}`)}" readonly></label><label>Objektadresse<input value="${esc(p.address||'')}" readonly></label><label>Status<select data-pl-phase-field="measureStatus"><option>${esc(data.measureStatus||'Entwurf')}</option><option>Entwurf</option><option>Aufmass abgeschlossen</option><option>Offerte erstellt</option></select></label><label class="wide">Mitarbeiter<input data-pl-phase-field="measurer" value="${esc(data.measurer||p.owner||'')}"></label><label class="wide">Anmerkung<textarea rows="3" data-pl-phase-field="generalNotes">${esc(data.generalNotes||'')}</textarea></label><input type="hidden" data-pl-phase-field="measureNo" value="${esc(data.measureNo||'')}"></div>
      <div class="pb90-flowbar"><div><strong>Aufmass</strong><span>Mengen erfassen → Material zuordnen → Offerte erstellen</span></div><button type="button" class="primary" data-open-offer>Aufmass in Offerte übernehmen →</button></div>
      ${groupHtml}<button type="button" class="primary pb-add-room" data-measure-group-add>+ Neuer Raum / Bereich</button>
      <section class="pb-measure-total"><h3>Zusammenfassung</h3>${[...summary].map(([k,v])=>`<div><span>${esc(k.split('|||')[0])}</span><strong>${formatCHNumber(v,2)} ${esc(k.split('|||')[1])}</strong></div>`).join('')||'<p>Noch keine Positionen vorhanden.</p>'}</section>
    </div>
  </div>`;
}
function pbOfferDraft(c){
  ...
  if(!Array.isArray(data.sections)){
    const auf=c.phaseData.aufmass||{},source=auf.measureGroups||[];
    data.sections=source.map(g=>({title:g.title||'Leistungen',rows:(g.rows||[]).map(r=>({title:r.item||'',description:r.note||'',qty:String(pbMeasureNumber(r.factor||1)*pbMeasureFormula(r.formula)),unit:r.unit||'m²',unitPrice:'0',discount:'0',sourceType:'aufmass'}))}));
    const mats=Array.isArray(auf.selectedMaterials)?auf.selectedMaterials:[];
    if(mats.length)data.sections.push({title:'Materialien',rows:mats.map(m=>({title:m.designation||'Material',description:[m.articleNo?`Art.-Nr. ${m.articleNo}`:'',m.supplier?`Lieferant: ${m.supplier}`:''].filter(Boolean).join(' | '),qty:String(m.qty||'1'),unit:m.unit||'m²',unitPrice:String(m.offerPrice||'0'),discount:'0',materialId:m.materialId||'',sourceType:'material'}))});
  }
...
async function pbBrandLogoData(){
  ...
  try{const blob=await fetch('./logo_ceramica.jpg?v=299101').then(r=>r.blob());pbBrandLogoCache=await new Promise((resolve,reject)=>{const fr=new FileReader();fr.onload=()=>resolve(fr.result);fr.onerror=reject;fr.readAsDataURL(blob)});return pbBrandLogoCache}catch(_){return null}
}
...
function pbCompanySettings(){
  S.settings=S.settings&&typeof S.settings==='object'?S.settings:{};
  const d={
    companyName:'Ceramica Naturo GmbH',legalForm:'GmbH',managingDirector:'Oruc Vakkas',
    street:'Seebergstrasse',houseNo:'1',zip:'8103',city:'Unterengstringen',country:'Schweiz',canton:'Zürich',
    phone:'+41 56 525 65 23',mobile:'+41 78 408 45 12',website:'ceramica-naturo.ch',email:'info@ceramica-naturo.ch',
    bankName:'Zürcher Kantonalbank',bankAccountName:'Ceramica Naturo GmbH',iban:'CH43 0070 0110 0069 3815 0',bic:'ZKBKCHZZ80A',
    uid:'CHE-320.887.348',vatNo:'CHE-320.887.348',mwst:'8.1',
    laborHourly:'119',laborOverhead:'53.3',laborProfit:'105',materialOverhead:'10',materialProfit:'25',
    offerValidity:'10',payment1:'40',payment2:'50',payment3:'10',
    timeStart:'07:00',timeEnd:'20:00',timePause:'00:30',
    aiEnabled:true,datevEnabled:false,sepaMode:'alle Zahlungen einzeln',
    offerTerms:'Sämtliche Arbeiten werden fachgerecht nach den anerkannten Regeln der Baukunde, den geltenden SIA-Normen sowie den Richtlinien des Schweizerischen Plattenverbandes (SPV) ausgeführt.',
    guarantee:'Die Garantieleistungen richten sich nach den Bestimmungen der SIA 118:\\n• 2 Jahre für offene (erkennbare) Mängel\\n• 5 Jahre für verdeckte Mängel',
    thanks:'Vielen Dank für Ihr Interesse und das entgegengebrachte Vertrauen.'
  };
  S.settings.company={...d,...(S.settings.company||{})};
  return S.settings.company;
}
function pbOfferPdfNumber(n){return formatCHNumber(Number(n)||0,2)}
function pbOfferPdfDate(v){
  const d=v?new Date(v):new Date();if(Number.isNaN(d.getTime()))return String(v||'');
  return new Intl.DateTimeFormat('de-CH',{day:'2-digit',month:'2-digit',year:'numeric'}).format(d);
}
async function pbOfferPdfLikeA2026134(p){
  ...
  const c=ensurePlattenlegerProject(p),d=pbOfferDraft(c),tot=pbOfferTotals(d),s=pbCompanySettings(),logo=await pbBrandLogoData();
  const doc=new JsPdf({unit:'mm',format:'a4',orientation:'portrait',compress:true});
  const L=18,R=192,W=174,footerTop=267,bodyBottom=258;
  const safe=v=>String(v??'').replace(/[\u2010-\u2015]/g,'-').replace(/×/g,'x');
  const lines=(v,w)=>doc.splitTextToSize(safe(v||''),w);
  const money=v=>pbOfferPdfNumber(v);
  const addrCompany=`${s.companyName} - ${s.street} ${s.houseNo} - ${s.zip} ${s.city}`;
  let page=1,y=0,positionNo=0;
  const sections=d.sections||[];
  const sectionTotals=sections.map(sec=>(sec.rows||[]).reduce((sum,r)=>sum+pbMeasureNumber(r.qty)*pbMeasureNumber(r.unitPrice)*(1-pbMeasureNumber(r.discount)/100),0));

  function logoHeader(){
    if(logo)pbAddBrandLogo(doc,logo,L,8,58);
  }
  function footer(){
    doc.setDrawColor(85);doc.setLineWidth(.18);doc.line(L,footerTop,R,footerTop);
    doc.setFont('helvetica','normal');doc.setTextColor(105);doc.setFontSize(6.3);
    const cols=[L,62,106,150];
    const c1=[s.companyName,`Geschäftsführung ${s.managingDirector}`,`${s.street} ${s.houseNo}`,`${s.zip} ${s.city}`];
    const c2=[`Telefon: ${s.phone}`,`Mobil: ${s.mobile}`,`E-Mail: ${s.email}`,`Web: ${s.website}`];
    const c3=[`Bank: ${s.bankName}`,`Inhaber: ${s.bankAccountName}`,`IBAN: ${s.iban}`,`BIC: ${s.bic}`];
    const c4=[`USt.Id.: ${s.uid}`];
    [c1,c2,c3,c4].forEach((arr,i)=>doc.text(arr,cols[i],footerTop+4,{lineHeightFactor:1.35}));
    doc.setTextColor(110);doc.text(`Seite ${page}`,105,289,{align:'center'});
    doc.setTextColor(55);
  }
  function newPage(){
    footer();doc.addPage();page++;logoHeader();y=35;
  }
  function ensure(h){
    if(y+h>bodyBottom)newPage();
  }
  function bodyText(v,x=L,w=W,size=9.5,bold=false,gap=2.3){
    const ls=lines(v,w);const h=ls.length*(size*.43)+gap;ensure(h);
    doc.setFont('helvetica',bold?'bold':'normal');doc.setFontSize(size);doc.text(ls,x,y,{lineHeightFactor:1.35});y+=h;
  }
  function firstPageHeader(){
    logoHeader();
    doc.setFont('helvetica','normal');doc.setFontSize(6.5);doc.setTextColor(70);
    doc.text(addrCompany,L,51);
    doc.setDrawColor(90);doc.setLineWidth(.15);doc.line(L,52,80,52);
    doc.setTextColor(55);doc.setFontSize(10.5);
    doc.text(lines(p.customer||'Kunde',74),L,58,{lineHeightFactor:1.4});
    const recLines=lines(p.customerAddress||p.address||'',74);doc.text(recLines,L,64,{lineHeightFactor:1.4});
    doc.setFontSize(9.5);
    const meta=[
      `Datum: ${pbOfferPdfDate(d.offerDate)}`,
      `Kundennummer: ${p.customerNo||p.customerId||'-'}`,
      `Projekt: ${p.projectNo||p.id||'-'}`,
      `Bearbeiter: ${p.owner||s.managingDirector||'-'}`
    ];
    doc.text(meta,R,90,{align:'right',lineHeightFactor:1.55});
    y=111;
    bodyText(`Offerte ${d.offerNo||''} - ${d.title||p.name||'Projekt'}`,L,W,10.4,true,3);
    bodyText(`Objektadresse: ${p.address||'-'}`,L,W,9.5,false,7);
    bodyText(`Voraussichtlicher Ausführungstermin: ${d.execution||'Nach Vereinbarung'}`,L,W,9.5,false,8);
    bodyText(`Sehr geehrte Damen und Herren,`,L,W,9.5,false,6);
    bodyText(d.intro||'Vielen Dank für Ihr Interesse. Gerne unterbreiten wir Ihnen mit dieser Offerte unser Angebot für Ihr Projekt.',L,W,9.5,false,5);
    bodyText('Mit uns entscheiden Sie sich für einen zuverlässigen Fachbetrieb, der grossen Wert auf Qualität, Präzision und Termintreue legt. Sämtliche Arbeiten werden fachgerecht nach den geltenden SIA-Normen sowie den Herstellervorgaben ausgeführt.',L,W,9.5,false,4);
    bodyText('Sie profitieren von einer transparenten Beratung, einer sauberen Arbeitsweise und hochwertigen Materialien. Unser Anspruch ist es, Ihr Projekt sorgfältig, effizient und mit höchster handwerklicher Qualität umzusetzen.',L,W,9.5,false,6);
  }
  function tableHead(){
    ensure(12);doc.setDrawColor(95);doc.setLineWidth(.15);doc.line(L,y,R,y);y+=4;
    doc.setFont('helvetica','normal');doc.setFontSize(8.2);
    doc.text('Position',L,y);doc.text('Leistung/Material',30,y);doc.text('Menge',140,y,{align:'right'});doc.text('Einzelpreis',170,y,{align:'right'});doc.text('Gesamtpreis',R,y,{align:'right'});
    y+=3;doc.line(L,y,R,y);y+=7;
  }
  function sectionTitle(si,sec){
    ensure(10);doc.setFont('helvetica','bold');doc.setFontSize(9.5);
    doc.text(String(si+1),L,y);doc.text(safe(sec.title||`Titel ${si+1}`),30,y);y+=7;
  }
  function position(si,ri,row){
    positionNo++;
    const qty=pbMeasureNumber(row.qty),price=pbMeasureNumber(row.unitPrice),sum=qty*price*(1-pbMeasureNumber(row.discount)/100);
    const title=lines(row.title||'Position',88),desc=lines(row.description||'',92);
    const h=Math.max(7,title.length*4.2+desc.length*4.0+(desc.length?2:0));
    ensure(h+4);
    doc.setFont('helvetica','normal');doc.setFontSize(8.8);
    doc.text(`${si+1}.${ri+1}`,L,y);
    doc.text(title,30,y,{lineHeightFactor:1.25});
    const unit=safe(row.unit||'');
    doc.text(`${String(row.qty||'')} ${unit}`,140,y,{align:'right'});
    doc.text(money(price),170,y,{align:'right'});doc.text(money(sum),R,y,{align:'right'});
    let dy=y+title.length*4.1+1;
    if(desc.length){doc.setFontSize(8.2);doc.text(desc,30,dy,{lineHeightFactor:1.28})}
    y+=h;
  }
  function sectionSum(value){
    ensure(8);doc.setFont('helvetica','normal');doc.setFontSize(8.5);
    doc.text('Titelsumme',142,y);doc.text(`${money(value)} CHF`,R,y,{align:'right'});y+=8;
  }

  firstPageHeader();tableHead();
  sections.forEach((sec,si)=>{
    sectionTitle(si,sec);
    (sec.rows||[]).forEach((r,ri)=>position(si,ri,r));
    sectionSum(sectionTotals[si]);
  });

  // Final page follows the same structure as the supplied A-2026-134.
  newPage();y=42;
  const summaryX=106,labelX=106,valueX=R;
  doc.setFontSize(9.2);doc.setFont('helvetica','normal');
  sections.forEach((sec,si)=>{doc.text(safe(sec.title||`Titel ${si+1}`),labelX,y);doc.text(`${money(sectionTotals[si])} CHF`,valueX,y,{align:'right'});y+=6});
  doc.setDrawColor(80);doc.line(166,y-2,R,y-2);
  doc.text('Netto',labelX,y+2);doc.text(`${money(tot.net)} CHF`,valueX,y+2,{align:'right'});y+=8;
  doc.text(`${String(d.vat||s.mwst).replace('.',',')}% MwSt.`,labelX,y);doc.text(`${money(tot.vat)} CHF`,valueX,y,{align:'right'});y+=7;
  doc.setFont('helvetica','bold');doc.text('Brutto',labelX,y);doc.text(`${money(tot.gross)} CHF`,valueX,y,{align:'right'});y+=12;
  doc.setFont('helvetica','normal');doc.setFontSize(8.8);
  bodyText(`Die Offerte ist gültig bis zum ${pbOfferPdfDate(d.validUntil||new Date(Date.now()+(Number(s.offerValidity)||10)*86400000))}`,L,W,8.8,false,6);
  bodyText('Ausführungsgrundlagen',L,W,9.2,true,5);
  bodyText(s.offerTerms,L,W,8.7,false,6);
  bodyText('Zahlungsbedingungen',L,W,9.2,true,4);
  bodyText(`• ${s.payment1||40} % innerhalb von 5 Tagen nach Vertragsabschluss\n• ${s.payment2||50} % innerhalb von 5 Tagen nach Baubeginn\n• ${s.payment3||10} % innerhalb von 5 Tagen nach Bauabnahme\nAlle Rechnungen sind ohne Abzug zahlbar.`,L,W,8.7,false,6);
  bodyText('Garantieleistungen',L,W,9.2,true,4);
  bodyText(s.guarantee,L,W,8.7,false,6);
  bodyText('Vielen Dank',L,W,9.2,true,4);
  bodyText(`${s.thanks}\n\nHaben Sie Fragen oder wünschen Sie Anpassungen an dieser Offerte? Gerne beraten wir Sie persönlich. Sie erreichen uns telefonisch unter ${s.phone} oder per E-Mail an ${s.email}.\n\nWir freuen uns darauf, Ihr Projekt fachgerecht, zuverlässig und termingerecht für Sie umzusetzen.`,L,W,8.7,false,6);
  bodyText(`Der Auftrag für die Offerte ${d.offerNo||''} (Brutto ${money(tot.gross)} CHF) wird hiermit erteilt.`,L,W,8.7,false,13);
  ensure(24);doc.setDrawColor(80);
  [L,79,140].forEach((x,i)=>doc.line(x,y,x+52,y));
  doc.setFontSize(8);doc.text('Name',L,y+5);doc.text('Ort, Datum',79,y+5);doc.text('Unterschrift',140,y+5);y+=15;

  // Correct total page count and footer on every page.
  const totalPages=doc.getNumberOfPages();
  for(let i=1;i<=totalPages;i++){
    doc.setPage(i);page=i;
    footer();
    doc.setFontSize(6.3);doc.setTextColor(110);doc.text(`Seite ${i} / ${totalPages}`,105,289,{align:'center'});
  }
  const slug=String(d.offerNo||'Offerte').replace(/[^A-Za-z0-9_-]+/g,'_');
  doc.save(`${slug}_${String(d.title||p.name||'Projekt').replace(/[^A-Za-z0-9ÄÖÜäöüß_-]+/g,'_')}.pdf`);
}

async function pbPhasePdfExport(p,requested){
  if(Array.isArray(requested)&&requested.length===1&&requested[0]==='offerte'){await pbOfferPdfLikeA2026134(p);return}
  const JsPdf=window.jspdf?.jsPDF;if(!JsPdf){alert('PDF-Modul ist noch nicht geladen. Bitte kurz warten und erneut versuchen.');return}
  const logo=await pbBrandLogoData(),doc=new JsPdf({unit:'mm',format:'a4'}),margin=16,pageW=210,maxW=178;let y=31,pageNo=1;pbAddBrandLogo(doc,logo,margin,7,52);
  const safe=v=>String(v??'').replace(/[\u2010-\u2015]/g,'-').replace(/\u00d7/g,'x')
    .replace(/Ã„/g,'Ä').replace(/Ã–/g,'Ö').replace(/Ãœ/g,'Ü').replace(/Ã¤/g,'ä').replace(/Ã¶/g,'ö').replace(/Ã¼/g,'ü').replace(/ÃŸ/g,'ß').replace(/Â·/g,'|').replace(/Â/g,'');
  const dateCH=v=>{if(!v)return'';const d=v instanceof Date?v:new Date(v);if(Number.isNaN(d.getTime()))return safe(v);return new Intl.DateTimeFormat('de-CH',{day:'2-digit',month:'2-digit',year:'numeric'}).format(d)};
  const footer=()=>{doc.setDrawColor(190);doc.line(margin,278,pageW-margin,278);doc.setFontSize(7);doc.setFont('helvetica','normal');doc.setTextColor(100);doc.text('Ceramica Naturo GmbH | Seebergstrasse 1 | 8103 Unterengstringen | info@ceramica-naturo.ch',margin,284);doc.text(`Seite ${pageNo}`,pageW-margin,289,{align:'right'});doc.setTextColor(20)};
  const newPage=()=>{footer();doc.addPage();pageNo++;y=31;pbAddBrandLogo(doc,logo,margin,7,52)};
  ...
  const kv=(k,v)=>{if(v===undefined||v===null||v===''||v===false)return;ensure(8);doc.setFontSize(9);doc.setFont('helvetica','bold');doc.text(safe(k),margin,y);doc.setFont('helvetica','normal');const lines=doc.splitTextToSize(safe(v===true?'Ja':v),116);doc.text(lines,76,y);y+=Math.max(6,lines.length*4)};
  const heading=(name)=>{if(y>245)newPage();doc.setTextColor(7,94,168);text(name,18,true);doc.setTextColor(20);doc.setDrawColor(7,94,168);doc.line(margin,y-1,pageW-margin,y-1);y+=5};
  const tableHeader=()=>{ensure(12);doc.setDrawColor(150);doc.line(margin,y,pageW-margin,y);y+=4;doc.setFontSize(7.6);doc.setFont('helvetica','bold');[['Gegenstand',margin],['Faktor',79],['Formel',98],['Anmerkung',116],['Summe',180],['Einheit',184]].forEach(([v,x])=>doc.text(v,x,y,x===180?{align:'right'}:undefined));y+=3;doc.line(margin,y,pageW-margin,y);y+=5;};
  const phaseData=p.plattenleger?.phaseData||{};
  requested.forEach((phase,idx)=>{
    const label=PB_PDF_PHASES.find(x=>x[0]===phase)?.[1]||phase;heading(label);
    kv('Projekt',p.name);kv('Kunde / Firma',p.customer);kv('Adresse',p.address);kv('Verantwortlich',p.owner);kv('Exportdatum',dateCH(new Date()));
    const d=phaseData[phase]||{};
    if(phase==='aufmass'){
      kv('Aufmass-Nr.',d.measureNo);kv('Bezeichnung',d.measureTitle);kv('Aufmassdatum',dateCH(d.measureDate));kv('Bearbeiter',d.measurer);kv('Anmerkung',d.generalNotes);
      (d.measureGroups||[]).forEach(g=>{
        ensure(22);text(g.title||'Bereich',12,true);tableHeader();
        (g.rows||[]).forEach(r=>{
          const sum=pbMeasureNumber(r.factor||1)*pbMeasureFormula(r.formula);
          const item=doc.splitTextToSize(safe(r.item||'Position'),58),note=doc.splitTextToSize(safe(r.note||''),44);
          const h=Math.max(6,item.length*3.7,note.length*3.7);ensure(h+3);
          doc.setFontSize(8);doc.setFont('helvetica','normal');
          doc.text(item,margin,y);doc.text(safe(r.factor||1),79,y);doc.text(safe(r.formula||'-'),98,y);
          if(r.note)doc.text(note,116,y);doc.text(formatCHNumber(sum,3),180,y,{align:'right'});doc.text(safe(r.unit||''),184,y);y+=h;
        });
        doc.setDrawColor(170);doc.line(margin,y,pageW-margin,y);y+=5;if(g.notes)kv('Notizen',g.notes)
      });
    }else if(phase==='offerte'){
      const od=pbOfferDraft(p.plattenleger),tot=pbOfferTotals(od);kv('Offerte-Nr.',od.offerNo);kv('Datum',dateCH(od.offerDate));kv('Bezeichnung',od.title);kv('Ausführung',od.execution);kv('Einleitung',od.intro);
...
function pbAnfrageCustomerCard(data){
  const val=k=>esc(data[k]??'');
  const customerNo=esc(data.customerNo||'wird beim Speichern erstellt');
  const contact=data.customerContact||{};
  const bank=data.customerBank||{};
  return `<section class="pb-anfrage-customer-card">
    <div class="pb-customer-card-title"><div><span>▣</span><h3>Kunde</h3></div></div>
    <div class="pb-customer-master-grid">
      <label>Typ<select data-pl-phase-field="customerType"><option ${data.customerType==='Privat'?'selected':''}>Privat</option><option ${data.customerType==='Firma'?'selected':''}>Firma</option></select></label>
      <label>Kundennummer<div class="pb-input-icon"><span>▣</span><input value="${customerNo}" readonly></div></label>
      <label>Anrede<select data-pl-phase-field="customerSalutation">${['Herr','Frau','Firma'].map(x=>`<option ${data.customerSalutation===x?'selected':''}>${x}</option>`).join('')}</select></label>
      <label>Briefanrede<input data-pl-phase-field="customerLetterSalutation" value="${val('customerLetterSalutation')}"></label>
      <label>Vorname<input data-pl-phase-field="customerFirstName" value="${val('customerFirstName')}"></label>
      <label>Nachname<input data-pl-phase-field="customerLastName" value="${val('customerLastName')}"></label>
      <label>E-Mail<input type="email" data-pl-phase-field="contactEmail" value="${val('contactEmail')}"></label>
      <label>Fax<input data-pl-phase-field="customerFax" value="${val('customerFax')}"></label>
      <label>Telefon<input data-pl-phase-field="contactPhone" value="${val('contactPhone')}"></label>
      <label>Mobil<input data-pl-phase-field="customerMobile" value="${val('customerMobile')}"></label>
    </div>
    <div class="pb-customer-tabs" role="tablist">
      <button type="button" class="active" data-anfrage-customer-tab="address">⌂ Adressen</button>
      <button type="button" data-anfrage-customer-tab="contact">♙ Ansprechpartner</button>
      <button type="button" data-anfrage-customer-tab="bank">▦ Zahlung &amp; Bank</button>
      <button type="button" data-anfrage-customer-tab="notes">▢ Anmerkung</button>
    </div>
    <div class="pb-customer-tabpanel active" data-anfrage-customer-panel="address">
      <div class="pb-address-form">
        <label class="wide">Adresse / Bezeichnung<input data-pl-phase-field="customerAddressLabel" value="${val('customerAddressLabel')}"></label>
        <label>Strasse<input data-pl-phase-field="customerStreet" value="${val('customerStreet')}"></label>
        <label>Hausnummer<input data-pl-phase-field="customerHouseNo" value="${val('customerHouseNo')}"></label>
        <label>PLZ<input data-pl-phase-field="customerZip" value="${val('customerZip')}"></label>
        <label>Stadt<input data-pl-phase-field="customerCity" value="${val('customerCity')}"></label>
      </div>
    </div>
    <div class="pb-customer-tabpanel" data-anfrage-customer-panel="contact">
      <div class="pb-panel-actions"><select data-anfrage-contact="selection"><option>Ansprechpartner 1</option></select><button type="button" class="pb-round primary">+</button><button type="button" class="pb-round danger">×</button></div>
      <div class="pb-contact-form">
        <label>Anrede<select data-anfrage-contact="salutation"><option></option><option ${contact.salutation==='Herr'?'selected':''}>Herr</option><option ${contact.salutation==='Frau'?'selected':''}>Frau</option></select></label><label>Briefanrede<input data-anfrage-contact="letterSalutation" value="${esc(contact.letterSalutation||'')}"></label>
        <label>Titel<input data-anfrage-contact="title" value="${esc(contact.title||'')}"></label><span></span>
        <label>Vorname<input data-anfrage-contact="firstName" value="${esc(contact.firstName||'')}"></label><label>Nachname<input data-anfrage-contact="lastName" value="${esc(contact.lastName||'')}"></label>
        <label>Geburtsdatum<input type="date" data-anfrage-contact="birthDate" value="${esc(contact.birthDate||'')}"></label><label>Rolle<input data-anfrage-contact="role" value="${esc(contact.role||'')}"></label>
        <label>E-Mail<input type="email" data-anfrage-contact="email" value="${esc(contact.email||'')}"></label><label>Fax<input data-anfrage-contact="fax" value="${esc(contact.fax||'')}"></label>
        <label>Telefon<input data-anfrage-contact="phone" value="${esc(contact.phone||'')}"></label><label>Mobil<input data-anfrage-contact="mobile" value="${esc(contact.mobile||'')}"></label>
        <label>Telefon privat<input data-anfrage-contact="privatePhone" value="${esc(contact.privatePhone||'')}"></label><span></span>
        <label class="wide">Anmerkung<textarea rows="4" data-anfrage-contact="notes">${esc(contact.notes||'')}</textarea></label>
      </div>
    </div>
    <div class="pb-customer-tabpanel" data-anfrage-customer-panel="bank">
      <div class="pb-bank-form">
        <label>Kontoinhaber<input data-anfrage-bank="accountHolder" value="${esc(bank.accountHolder||'')}"></label><label>Bank Name<input data-anfrage-bank="bankName" value="${esc(bank.bankName||'')}"></label>
        <label>IBAN<input data-anfrage-bank="iban" value="${esc(bank.iban||'')}"></label><label>BIC<input data-anfrage-bank="bic" value="${esc(bank.bic||'')}"></label>
        <h3 class="wide">Zahlungsbedingungen</h3>
        <label>Zahlungsziel (in Tagen)<input type="number" min="0" step="1" data-anfrage-bank="paymentDays" value="${esc(bank.paymentDays||'')}"></label><span></span>
        <label class="check wide"><input type="checkbox" data-anfrage-bank-check="discount" ${bank.discount?'checked':''}> Skonto</label>
        <label class="check wide"><input type="checkbox" data-anfrage-bank-check="reverseCharge" ${bank.reverseCharge?'checked':''}> Steuerschuldumkehr (Reverse Charge)</label>
      </div>
    </div>
    <div class="pb-customer-tabpanel" data-anfrage-customer-panel="notes">
      <div class="pb-notes-form"><label class="wide">Anmerkung<textarea rows="6" data-pl-phase-field="customerNotes">${val('customerNotes')}</textarea></label><label>Akquisekanal<input data-pl-phase-field="customerAcquisitionChannel" value="${val('customerAcquisitionChannel')}"></label><label>Sprache<select data-pl-phase-field="customerLanguage">${['Deutsch','Französisch','Italienisch','Englisch'].map(x=>`<option ${data.customerLanguage===x?'selected':''}>${x}</option>`).join('')}</select></label></div>
    </div>
  </section>`;
}

function pbEnsureAnfrageDefaults(p,c,data){
  if(!data.requestNo){
    const year=new Date().getFullYear();
    const nums=(S.projects||[]).map(x=>x.plattenleger?.phaseData?.anfrage?.requestNo||'')
      .map(x=>Number(String(x).match(/(\d+)$/)?.[1]||0));
    data.requestNo=`AN-${year}-${String(Math.max(0,...nums)+1).padStart(3,'0')}`;
  }
  data.requestDate=data.requestDate||new Date().toISOString().slice(0,10);
  const cust=(S.customers||[]).find(x=>x.id===p.customerId);
  data.customerCompany=data.customerCompany||cust?.company||p.customer||'';
  data.customerNo=data.customerNo||cust?.number||'';
  data.customerType=data.customerType||cust?.type||(cust?.company?'Firma':'Privat');
  data.customerLetterSalutation=data.customerLetterSalutation||cust?.letterSalutation||'';
  data.customerContact=data.customerContact||structuredClone(cust?.contacts?.[0]||{});
  data.customerBank=data.customerBank||structuredClone(cust?.bank||{});
  data.customerNotes=data.customerNotes||cust?.notes||'';
  data.customerAcquisitionChannel=data.customerAcquisitionChannel||cust?.acquisitionChannel||'';
  data.customerLanguage=data.customerLanguage||cust?.language||'Deutsch';
  data.customerSalutation=data.customerSalutation||cust?.salutation||(cust?.company?'Firma':'Herr');
  data.customerFirstName=data.customerFirstName||cust?.firstName||'';
  data.customerLastName=data.customerLastName||cust?.lastName||'';
  data.contactEmail=data.contactEmail||cust?.email||'';
  data.contactPhone=data.contactPhone||cust?.phone||p.phone||'';
  data.customerMobile=data.customerMobile||cust?.mobile||'';
  data.customerStreet=data.customerStreet||cust?.street||'';
  data.customerHouseNo=data.customerHouseNo||cust?.houseNo||'';
  data.customerZip=data.customerZip||cust?.zip||'';
  data.customerCity=data.customerCity||cust?.city||'';
  if(!data.customerStreet && p.address){
    const m=String(p.address).match(/^(.*?)(?:\s+)(\d+[A-Za-z]?)\s*,?\s*(\d{4})\s+(.+)$/);
    if(m){data.customerStreet=m[1];data.customerHouseNo=m[2];data.customerZip=m[3];data.customerCity=m[4]}
  }
  data.responsible=data.responsible||p.owner||'';
  data.priority=data.priority||'Normal';
  data.requestStatus=data.requestStatus||'Neu';
}

async function pbSaveAnfrageCustomer(c,data){
  const p=cur();if(!p)return;
  readPlattenlegerPhaseData(c,'anfrage');
  const host=document.getElementById('plattenlegerPhaseContent');
  data.customerContact=data.customerContact||{};host?.querySelectorAll('[data-anfrage-contact]').forEach(el=>{if(el.dataset.anfrageContact!=='selection')data.customerContact[el.dataset.anfrageContact]=el.value});
  data.customerBank=data.customerBank||{};host?.querySelectorAll('[data-anfrage-bank]').forEach(el=>data.customerBank[el.dataset.anfrageBank]=el.value);host?.querySelectorAll('[data-anfrage-bank-check]').forEach(el=>data.customerBank[el.dataset.anfrageBankCheck]=el.checked);
  const company=String(data.customerCompany||'').trim();
  ...
  if(!company && !firstName && !lastName){
    alert('Bitte Kunde / Firma oder einen Kundennamen eingeben.');
    return;
  }
  S.customers=Array.isArray(S.customers)?S.customers:[];
  let customer=p.customerId?S.customers.find(x=>x.id===p.customerId):null;
  if(!customer && data.contactEmail){
    customer=S.customers.find(x=>String(x.email||'').toLowerCase()===String(data.contactEmail).toLowerCase());
  }
  if(!customer){
    customer={id:u(),number:pbNextNumber('K'),createdAt:new Date().toISOString()};
    S.customers.unshift(customer);
  }
  customer.type=data.customerType|| (company?'Firma':'Privat');
  customer.letterSalutation=String(data.customerLetterSalutation||'').trim();
  customer.fax=String(data.customerFax||'').trim();
  customer.addressLabel=String(data.customerAddressLabel||'').trim();
  customer.addressType=String(data.customerAddressType||'Standard').trim();
  customer.altRecipient=String(data.customerAltRecipient||'').trim();
  customer.addressExtra=String(data.customerAddressExtra||'').trim();
  customer.company=company;
  customer.salutation=data.customerSalutation|| (company?'Firma':'Herr');
  customer.firstName=firstName;
  customer.lastName=lastName;
  customer.email=String(data.contactEmail||'').trim();
  customer.phone=String(data.contactPhone||'').trim();
  customer.mobile=String(data.customerMobile||'').trim();
  customer.street=String(data.customerStreet||'').trim();
  customer.houseNo=String(data.customerHouseNo||'').trim();
  customer.zip=String(data.customerZip||'').trim();
  customer.city=String(data.customerCity||'').trim();
  customer.fullAddress=[customer.street,customer.houseNo].filter(Boolean).join(' ')+(customer.zip||customer.city?`, ${[customer.zip,customer.city].filter(Boolean).join(' ')}`:'');
  customer.contacts=[structuredClone(data.customerContact||{})];
  customer.bank=structuredClone(data.customerBank||{});
  customer.notes=String(data.customerNotes||'').trim();
  customer.acquisitionChannel=String(data.customerAcquisitionChannel||'').trim();
  customer.language=data.customerLanguage||'Deutsch';
  customer.updatedAt=new Date().toISOString();
  pbSynchronizeAllData();
  data.customerNo=customer.number||data.customerNo||'';

  p.customerId=customer.id;
  p.customer=pbCustomerName(customer);
  p.phone=customer.phone||customer.mobile||'';
  if(customer.fullAddress)p.address=customer.fullAddress;
  pbApplyCustomerToProject(p,customer);

  save();
  try{await Promise.resolve(window.PBStorage?.saveState?.(S,{reason:'anfrage-customer-save-2976'}))}catch(_){}
  try{window.ProjectBauOneDrive?.scheduleAutoSync?.('anfrage-customer-save')}catch(_){}
  pbActionToast('Kunde gespeichert und in der Kundenliste eingetragen.');
}

async function pbAnfrageAction(action,c,data){
  const p=cur();if(!p)return;
  readPlattenlegerPhaseData(c,'anfrage');
  if(action==='visit'){
    data.visitRequired='Ja';data.requestStatus='Besichtigung geplant';
    save();renderPlattenlegerPhaseContent(c);pbActionToast('Besichtigung als nächster Schritt markiert.');return;
  }
  if(action==='reject'){
    if(!confirm('Anfrage wirklich als abgelehnt markieren?'))return;
    data.requestStatus='Abgelehnt';p.status='Anfrage abgelehnt';save();renderPlattenlegerPhaseContent(c);pbActionToast('Anfrage abgelehnt.');return;
  }
  if(action==='project'){
    p.status='Aktiv';data.requestStatus='Gewonnen';
    // Das Projekt ist bereits der zentrale Datensatz. Sicherstellen, dass es in S.projects vorhanden ist.
    if(!(S.projects||[]).some(x=>x.id===p.id))S.projects.unshift(p);
    save();pbActionToast('Projekt gespeichert und in der Projekte-Maske verfügbar.');return;
  }
  if(action==='offer'){
    data.requestStatus='Offerte erstellen';c.phase='offerte';
    c.phaseData=c.phaseData||{};c.phaseData.offerte=c.phaseData.offerte||{};
    const off=c.phaseData.offerte;
    off.title=off.title||p.name||'Offerte';off.offerDate=off.offerDate||new Date().toISOString().slice(0,10);
    save();renderPlattenlegerCockpit(p);pbActionToast('Offerte aus der Anfrage vorbereitet.');return;
  }
}
function pbOpenBaudokumentation(){
  const panel=$('projectEditFields');if(!panel)return;
  panel.classList.remove('hidden');panel.classList.add('pb-baudokumentation-open');
  panel.scrollIntoView({behavior:'smooth',block:'start'});
}
function pbCloseBaudokumentation(){
  const panel=$('projectEditFields');if(!panel)return;
  panel.classList.add('hidden');panel.classList.remove('pb-baudokumentation-open');
}

function renderPlattenlegerPhaseContent(c){
  document.body.classList.toggle('pb-anfrage-mode',c.phase==='anfrage');
  ...
  if(c.phase==='aufmass')host.innerHTML=`<div class="plattenleger-phase-head"><div><h4>Professionelles Bad-Aufmass</h4><p>Vollständige Aufnahme für Badumbau, Offerte, Material und Ausführung.</p></div><span class="plattenleger-phase-badge">Aufmass</span></div>${renderProfessionalAufmass(c,data)}`;
    ...
    if(c.phase==='anfrage')pbEnsureAnfrageDefaults(cur(),c,data);
    host.innerHTML=`<div class="plattenleger-phase-head"><div><h4>${esc(schema.title)}</h4><p>${esc(schema.desc)}</p></div><span class="plattenleger-phase-badge">${esc(PL_PHASES.find(x=>x[0]===c.phase)?.[1]||'Phase')}</span></div>`+
      `<div class="plattenleger-phase-fields">${schema.fields.map(f=>phaseFieldHtml(f,data)).join('')}</div>`+
      (c.phase==='anfrage'?pbAnfrageCustomerCard(data)+`<div class="pb-anfrage-savebar"><button type="button" class="primary" data-anfrage-save>Kunde und Anfrage speichern</button></div>`:
  ...
  host.insertAdjacentHTML('afterbegin',`<div class="pb-phase-pdfbar pb-phase-pdfbar-multi">
    <strong>PDF-Ausgabe</strong>
    <div class="pb-pdf-multiselect">
      <button type="button" class="secondary" data-pdf-menu>Phasen auswählen <span>⌄</span></button>
      <div class="pb-pdf-menu hidden">${PB_PDF_PHASES.map(([k,l])=>`<label><input type="checkbox" data-pdf-phase-check value="${k}" ${k===c.phase?'checked':''}> ${l}</label>`).join('')}</div>
    </div>
    <button type="button" class="primary" data-pdf-selected>Ausgewählte Phasen als PDF</button>
  </div>`);
  host.querySelector('[data-open-baudokumentation]')?.addEventListener('click',pbOpenBaudokumentation);

  const pdfMenu=host.querySelector('.pb-pdf-menu');
  host.querySelector('[data-pdf-menu]')?.addEventListener('click',e=>{e.stopPropagation();pdfMenu?.classList.toggle('hidden')});
  host.querySelector('[data-pdf-selected]')?.addEventListener('click',async()=>{
    const selected=[...host.querySelectorAll('[data-pdf-phase-check]:checked')].map(x=>x.value);
    if(!selected.length){alert('Bitte mindestens eine Phase auswählen.');return}
    await pbPhasePdfExport(cur(),selected);
  });

  host.querySelectorAll('[data-anfrage-customer-tab]').forEach(btn=>btn.addEventListener('click',()=>{
    host.querySelectorAll('[data-anfrage-customer-tab]').forEach(x=>x.classList.toggle('active',x===btn));
    host.querySelectorAll('[data-anfrage-customer-panel]').forEach(panel=>panel.classList.toggle('active',panel.dataset.anfrageCustomerPanel===btn.dataset.anfrageCustomerTab));
  }));
  host.querySelector('[data-anfrage-save]')?.addEventListener('click',()=>pbSaveAnfrageCustomer(c,data));
  host.querySelector('[data-pb90-list]')?.addEventListener('click',()=>{
    const p=cur();if(!p)return;const d=ensurePlattenlegerProject(p).phaseData.aufmass||{};
    alert(`Aufmass-Liste\n\n${d.measureNo||'-'}  ${d.measureTitle||p.name||''}\nStatus: ${d.measureStatus||'Entwurf'}`);
  });
  host.querySelector('[data-pb90-eval]')?.addEventListener('click',()=>{
    const sum=pbMeasureSummary(data.measureGroups||[]),mats=(data.selectedMaterials||[]).length;
    alert(`Auswertungen\n\nRäume / Bereiche: ${(data.measureGroups||[]).length}\nMaterialpositionen: ${mats}\nAufmasspositionen: ${(data.measureGroups||[]).reduce((n,g)=>n+(g.rows||[]).length,0)}\n\n${[...sum].map(([k,v])=>`${k.split('|||')[0]}: ${formatCHNumber(v,2)} ${k.split('|||')[1]}`).join('\n')}`);
  });
  host.querySelectorAll('[data-aufmass-material-action]').forEach(btn=>pbBindTap(btn,()=>{
    readPlattenlegerPhaseData(c,'aufmass');
    const mode=btn.dataset.aufmassMaterialAction;
    pbOpenMaterialManager(mode==='list'?'list':mode==='calc'?'calc':'plain',{pickContext:'aufmass'});
  }));
  host.querySelectorAll('[data-selected-material]').forEach(row=>{
    const i=Number(row.dataset.selectedMaterial);
    row.querySelector('[data-sm-qty]')?.addEventListener('change',e=>{data.selectedMaterials[i].qty=e.target.value;save()});
    row.querySelector('[data-sm-unit]')?.addEventListener('change',e=>{data.selectedMaterials[i].unit=e.target.value;save()});
  });
  host.querySelectorAll('[data-sm-delete]').forEach(btn=>pbBindTap(btn,()=>{data.selectedMaterials.splice(Number(btn.dataset.smDelete),1);save();renderPlattenlegerPhaseContent(c)}));
  ...
  host.querySelectorAll('.pb-measure-item-cell [data-mr="item"]').forEach(input=>{
    input.addEventListener('input',()=>pbOpenMeasureSuggest(input));
    input.addEventListener('focus',()=>pbOpenMeasureSuggest(input));
    input.addEventListener('blur',()=>setTimeout(()=>pbCloseMeasureSuggest(),140));
  });
...
function ensurePlattenlegerProject(p){
  if(!p.projectNo)p.projectNo=pbNextNumber('PRO');
  ...
  c.phaseData.anfrage=c.phaseData.anfrage||{};if(!c.phaseData.anfrage.requestNo)c.phaseData.anfrage.requestNo=pbNextNumber('AN');
  c.phaseData.aufmass=c.phaseData.aufmass||{};if(!c.phaseData.aufmass.measureNo)c.phaseData.aufmass.measureNo=pbNextNumber('AUF');
  c.phaseData.offerte=c.phaseData.offerte||{};if(!c.phaseData.offerte.offerNo)c.phaseData.offerte.offerNo=pbNextNumber('OF');
...
function renderPlattenlegerCockpit(p){
  ...
  if(phasebar){
    phasebar.innerHTML=PL_PHASES.map((x,i)=>{
      const phaseBtn=`<button type="button" data-pl-phase="${x[0]}" class="plattenleger-phase ${i<phaseIndex?'done':''} ${i===phaseIndex?'active':''}">${esc(x[1])}</button>`;
      return x[0]==='anfrage'?phaseBtn+`<button type="button" class="plattenleger-phase pb-baudokumentation-tab" data-open-baudokumentation>Baudokumentation</button>`:phaseBtn;
    }).join('');
    phasebar.querySelector('[data-open-baudokumentation]')?.addEventListener('click',pbOpenBaudokumentation);
...
function renderP(){
  ...
  if(pe){
    if(!pe.classList.contains('pb-baudokumentation-open'))pe.classList.add('hidden');
    $('editProjectName').value=p.name||''; $('editProjectAddress').value=p.address||'';
    $('editProjectCustomer').innerHTML=pbCustomerOptions(p.customerId||''); $('editProjectPhone').value=p.phone||'';
    $('editProjectCustomer').onchange=()=>{const c=S.customers.find(x=>x.id===$('editProjectCustomer').value);if(c){$('editProjectPhone').value=c.phone||c.mobile||'';if(c.fullAddress)$('editProjectAddress').value=c.fullAddress}};
  ...
  renderFloorplans(p);
  let projectFiles=$('pbProjectFiles');
  if(!projectFiles){projectFiles=document.createElement('section');projectFiles.id='pbProjectFiles';projectFiles.className='editor pb-project-files';$('panel')?.appendChild(projectFiles)}
  pbMountOwnerFiles(projectFiles,'project',p.id,'Projektdateien');
}
function area(p,a){
...
function photoCard(a,ph,i){
  ...
  <div class=photo-fields>
    <label>Titel / Position<input class=photoTitle placeholder="z. B. Badezimmer – Duschwand"></label>
...
async function generateDirectPDFReport(){
  const p=cur();
  if(!p)return;
  ...
  try{
    ...
    const brandLogo=await pbBrandLogoData();pbAddBrandLogo(doc,brandLogo,12,7,52);
    ...
    doc.setTextColor(20);doc.setFontSize(17);
    const projectTitleLines=doc.splitTextToSize(String(p.name||'Projekt'),186);
    doc.text(projectTitleLines,12,32);
    doc.setFont('helvetica','normal');doc.setFontSize(9);
    doc.text(`Seite 1 / ${totalPages}`,198,12,{align:'right'});
    doc.text(`Berichtsdatum: ${reportDate}`,198,17,{align:'right'});

    let y=32+projectTitleLines.length*6+5;
    const rows=[
    ...
    for(const line of infoLines){
      if(y>280){
        ...
        y=18;
        doc.setTextColor(90);doc.setFont('helvetica','bold');doc.setFontSize(8);
        doc.text('PROJEKT BAU | PROJEKTINFORMATIONEN',72,12);
    ...
    for(let pageIndex=0;pageIndex<photoPages;pageIndex++){
      doc.addPage('a4','portrait');
      pbAddBrandLogo(doc,brandLogo,12,7,52);

      doc.setTextColor(90);doc.setFont('helvetica','bold');doc.setFontSize(8);
      doc.text('CERAMICA NATURO | BAUDOKUMENTATION',72,12);
      doc.setTextColor(20);doc.setFontSize(17);
      const photoProjectTitle=doc.splitTextToSize(String(p.name||'Projekt'),186);
      doc.text(photoProjectTitle,12,32);

      let hy=32+photoProjectTitle.length*6+4;
      rows.forEach(([label,value])=>{
      ...
      doc.text(`Berichtsdatum: ${reportDate}`,198,17,{align:'right'});
      const photoContentTop=Math.max(58,hy+3);doc.setLineWidth(.5);doc.line(12,photoContentTop-4,198,photoContentTop-4);

      for(let slot=0;slot<2;slot++){
        const item=items[pageIndex*2+slot];
        if(!item)continue;
        const py=photoContentTop+slot*110,h=104;
        doc.setDrawColor(170);doc.setLineWidth(.25);
        doc.roundedRect(12,py,186,h,2,2,'S');
        doc.line(98,py,98,py+h);
        addImageContain(doc,item.photo.data,13,py+1,84,102);
...
/* ============================================================
   2.9.68 – MATERIALIEN / KALKULATION / AUFMASS
   Zentrale Materialdatenbank mit zwei Erfassungsmasken:
   "mit Kalkulation" und "ohne Kalkulation".
   ============================================================ */
let pbMaterialMode='list';
let pbMaterialEditingId=null;
let pbMaterialPickContext=null;
let pbMaterialPhotoData='';

function pbMaterialDb(){
  S.materialCatalog=Array.isArray(S.materialCatalog)?S.materialCatalog:[];
  return S.materialCatalog;
}
function pbMaterialId(){return 'mat_'+Date.now().toString(36)+'_'+Math.random().toString(36).slice(2,8)}
function pbMaterialMoney(v){return formatCHNumber(Number(v)||0,2)}
function pbMaterialEscape(v){return esc(v??'')}
function pbMaterialCalc(m){
  const purchase=Math.max(0,Number(m.purchasePrice)||0);
  const overheadPct=Math.max(0,Number(m.overheadPct)||0);
  const profitPct=Math.max(0,Number(m.profitPct)||0);
  const materialCost=purchase;
  const overhead=materialCost*overheadPct/100;
  const selfCost=materialCost+overhead;
  const profit=selfCost*profitPct/100;
  const calculated=selfCost+profit;
  const offer=m.priceType==='manuell'?Math.max(0,Number(m.offerPrice)||0):calculated;
  return {purchase,materialCost,overhead,selfCost,profit,calculated,offer};
}
function pbMaterialNormalise(m={}){
  return {
    id:m.id||pbMaterialId(),
    articleNo:m.articleNo||m.article||'',
    designation:m.designation||[m.brand,m.model].filter(Boolean).join(' ')||'',
    ean:m.ean||'',
    supplier:m.supplier||'',
    serviceText:m.serviceText||m.notes||'',
    comments:m.comments||'',
    photo:m.photo||'',
    calcMode:m.calcMode||'mit Kalkulation',
    type:m.type||'Material',
    purchasePrice:String(m.purchasePrice??m.price??''),
    overheadPct:String(m.overheadPct??'10'),
    profitPct:String(m.profitPct??''),
    overrideProfit:String(m.overrideProfit??''),
    bomRule:String(m.bomRule??''),
    priceType:m.priceType||'manuell',
    offerPrice:String(m.offerPrice??m.price??''),
    packageSize:String(m.packageSize??'1'),
    packageUnit:m.packageUnit||'Packung',
    offerUnit:m.offerUnit||'m²',
    vat:m.vat||'normal',
    createdAt:m.createdAt||new Date().toISOString(),
    updatedAt:new Date().toISOString()
  };
}
function pbMaterialSeedFromTiles(){
  const db=pbMaterialDb();if(db.length)return;
  const seen=new Set();
  (S.projects||[]).forEach(p=>(p.tileMaterials||[]).forEach(x=>{
    const n=pbMaterialNormalise(x),key=(n.articleNo+'|'+n.designation).toLowerCase();
    if(!n.designation||seen.has(key))return;seen.add(key);db.push(n);
  }));
  if(db.length)save();
}
function pbMaterialTopTitle(text){const n=$('pbMaterialTopTitle');if(n)n.textContent=text}
function pbOpenMaterialManager(mode='list',opts={}){
  pbMaterialSeedFromTiles();
  pbMaterialMode=mode;pbMaterialPickContext=opts.pickContext||null;
  const workspace=$('pbMaterialManager'),main=document.querySelector('.cad-main');if(!workspace||!main)return;
  main.classList.add('pb-material-mode');
  workspace.classList.remove('hidden');workspace.setAttribute('aria-hidden','false');
  document.querySelectorAll('.cad-nav-item').forEach(x=>x.classList.toggle('active',x.id==='openMaterialManager'));
  const u=window.ProjectBauAuth?.current?.();const greet=$('pbMaterialGreeting');if(greet)greet.textContent=u?.name?`Grüezi ${u.name.split(' ')[0]}`:'Projekt Bau';
  if(mode==='list')pbRenderMaterialList();
  else pbOpenMaterialEditor(null,mode==='calc'?'mit Kalkulation':'ohne Kalkulation');
  window.scrollTo({top:0,behavior:'instant'});
}
function pbCloseMaterialManager(){
  const workspace=$('pbMaterialManager'),main=document.querySelector('.cad-main');
  if(workspace){workspace.classList.add('hidden');workspace.setAttribute('aria-hidden','true')}
  main?.classList.remove('pb-material-mode');
  document.querySelectorAll('.cad-nav-item').forEach(x=>x.classList.remove('active'));
  pbMaterialPickContext=null;pbMaterialEditingId=null;pbMaterialPhotoData='';
}
function pbMaterialListRows(){
  const q=String($('pbMatFilter')?.value||'').trim().toLowerCase();
  const supplier=String($('pbMatSupplierFilter')?.value||'').trim().toLowerCase();
  let rows=[...pbMaterialDb()];
  if(q)rows=rows.filter(m=>[m.articleNo,m.designation,m.ean,m.supplier,m.serviceText].some(v=>String(v||'').toLowerCase().includes(q)));
  if(supplier)rows=rows.filter(m=>String(m.supplier||'').toLowerCase().includes(supplier));
  return rows.sort((x,y)=>String(x.designation||'').localeCompare(String(y.designation||''),'de-CH'));
}
function pbRenderMaterialList(){
  pbMaterialTopTitle('Materialien');
  const body=$('pbMaterialManagerBody');if(!body)return;
  const rows=pbMaterialListRows();
  body.innerHTML=`<section class="pb-material-page pb-material-list-page">
    <div class="pb-material-heading"><div><span class="pb-material-barcode dark">▥</span><h1>Materialien</h1></div><div class="pb-material-actions"><button type="button" data-mat-import title="Import">↥</button><button type="button" class="primary round" data-mat-new title="Neues Material">+</button></div></div>
    <div class="pb-material-table-wrap">
      <div class="pb-material-grid pb-material-grid-head"><span>↕ ARTIKELNUMMER</span><span>↕ BEZEICHNUNG</span><span>↕ LIEFERANT</span><span>↕ ANGEBOTSPREIS</span><span>⚙</span></div>
      <div class="pb-material-grid pb-material-filter-row">
        <input id="pbMatFilter" placeholder="⌕ Filtern" value="${pbMaterialEscape($('pbMatFilter')?.value||'')}">
        <span></span><input id="pbMatSupplierFilter" placeholder="alle" value="${pbMaterialEscape($('pbMatSupplierFilter')?.value||'')}"><span></span><span></span>
      </div>
      <div class="pb-material-table-body">${rows.length?rows.map(m=>{const c=pbMaterialCalc(m);return `<button type="button" class="pb-material-grid pb-material-row" data-mat-row="${m.id}">
        <span>${pbMaterialEscape(m.articleNo||'')}</span><span>${pbMaterialEscape(m.designation||'')}</span><span>${pbMaterialEscape(m.supplier||'')}</span><span>CHF ${pbMaterialMoney(c.offer)}</span><span>›</span>
      </button>`}).join(''):'<div class="pb-material-empty">Noch keine Materialien vorhanden.</div>'}</div>
    </div>
  </section>`;
  body.querySelector('[data-mat-new]')?.addEventListener('click',()=>pbOpenMaterialEditor(null,'mit Kalkulation'));
  body.querySelector('[data-mat-import]')?.addEventListener('click',()=>pbOpenMaterialEditor(null,'ohne Kalkulation'));
  body.querySelector('#pbMatFilter')?.addEventListener('input',()=>pbRenderMaterialList());
  body.querySelector('#pbMatSupplierFilter')?.addEventListener('input',()=>pbRenderMaterialList());
  body.querySelectorAll('[data-mat-row]').forEach(row=>row.addEventListener('click',()=>{
    const m=pbMaterialDb().find(x=>x.id===row.dataset.matRow);if(!m)return;
    if(pbMaterialPickContext){pbAddMaterialToAufmass(m);pbCloseMaterialManager();return}
    pbOpenMaterialEditor(m,m.calcMode||'mit Kalkulation');
  }));
}
function pbMaterialField(label,id,value='',type='text',extra=''){
  return `<label>${label}<input id="${id}" type="${type}" value="${pbMaterialEscape(value)}" ${extra}></label>`;
}
function pbOpenMaterialEditor(item=null,calcMode='mit Kalkulation'){
  const m=pbMaterialNormalise(item||{calcMode});m.calcMode=calcMode;
  pbMaterialEditingId=item?.id||null;pbMaterialPhotoData=m.photo||'';
  pbMaterialTopTitle(`Material: ${m.designation||'Neu'}`);
  const body=$('pbMaterialManagerBody');if(!body)return;
  const c=pbMaterialCalc(m),withCalc=calcMode==='mit Kalkulation';
  body.innerHTML=`<section class="pb-material-page pb-material-edit-page">
    <div class="pb-material-heading"><div><span class="pb-material-barcode dark">▥</span><h1>Material</h1></div><div class="pb-material-actions"><button type="button" class="warning round" data-mat-back title="Zurück">↶</button>${item?'<button type="button" class="danger round" data-mat-delete title="Löschen">▣</button>':''}</div></div>
    <h2>Basisinformationen</h2>
    <div class="pb-material-base-grid">
      ${pbMaterialField('Artikelnummer','pbMatArticle',m.articleNo)}
      ${pbMaterialField('Bezeichnung','pbMatDesignation',m.designation)}
      ${pbMaterialField('EAN','pbMatEan',m.ean)}
      <label>Foto hochladen<div class="pb-material-file"><input id="pbMatPhotoName" value="${m.photo?'Foto vorhanden':''}" readonly><button type="button" id="pbMatPhotoButton">↥</button><input id="pbMatPhotoInput" class="hidden" type="file" accept="image/*"></div></label>
      ${pbMaterialField('Lieferant','pbMatSupplier',m.supplier)}
      <div></div>
      <label class="wide">Leistungstext<textarea id="pbMatService" rows="8">${pbMaterialEscape(m.serviceText)}</textarea></label>
      <label class="wide">Kommentare<textarea id="pbMatComments" rows="4">${pbMaterialEscape(m.comments)}</textarea></label>
    </div>
    <h2>${withCalc?'Kalkulation und Preis':'Preis (ohne Kalkulation)'}</h2>
    <div class="pb-material-calc-grid ${withCalc?'':'without'}">
      <label>Kalkulation<select id="pbMatCalcMode"><option ${withCalc?'selected':''}>mit Kalkulation</option><option ${!withCalc?'selected':''}>ohne Kalkulation</option></select></label>
      <label>Typ<select id="pbMatType"><option ${m.type==='Material'?'selected':''}>Material</option><option ${m.type==='Leistung'?'selected':''}>Leistung</option></select></label>
      ${withCalc?`
      ${pbMaterialField('Einkaufspreis (Nettopreis)','pbMatPurchase',m.purchasePrice,'number','step="0.01" min="0"')}
      <div class="pb-calc-summary">
        <label class="pb-calc-readonly"><span>Materialeinzelkosten</span><input id="pbCalcMaterialCost" value="CHF ${pbMaterialMoney(c.materialCost)}" readonly></label>
        <label><span>Materialgemeinkosten</span><div class="pb-percent-control"><input id="pbMatOverhead" value="${pbMaterialEscape(m.overheadPct)}" inputmode="decimal"><em>%</em></div></label>
        <label class="pb-calc-readonly"><span>Materialgemeinkosten Betrag</span><input id="pbCalcOverhead" value="CHF ${pbMaterialMoney(c.overhead)}" readonly></label>
        <label class="pb-calc-readonly"><span>Selbstkosten</span><input id="pbCalcSelf" value="CHF ${pbMaterialMoney(c.selfCost)}" readonly></label>
        <label><span>Gewinnzuschlag</span><div class="pb-percent-control"><input id="pbMatProfit" value="${pbMaterialEscape(m.profitPct)}" inputmode="decimal"><em>%</em></div></label>
        <label class="pb-calc-readonly"><span>Gewinnzuschlag Betrag</span><input id="pbCalcProfit" value="CHF ${pbMaterialMoney(c.profit)}" readonly></label>
      </div>
      ${pbMaterialField('Gewinnzuschlag Material überschreiben','pbMatOverride',m.overrideProfit)}
      ${pbMaterialField('Vorschrift für Stückliste','pbMatBomRule',m.bomRule)}
      `:''}
      <label>Angebotspreistyp<select id="pbMatPriceType"><option ${m.priceType==='manuell'?'selected':''}>manuell</option><option ${m.priceType==='kalkuliert'?'selected':''}>kalkuliert</option></select></label>
      ${pbMaterialField('Angebotspreis','pbMatOfferPrice',m.offerPrice,'number','step="0.01" min="0"')}
      ${pbMaterialField('Gebindegrösse','pbMatPackageSize',m.packageSize,'number','step="0.01" min="0"')}
      <label>Einheit Gebinde<select id="pbMatPackageUnit">${pbUnitOptions(m.packageUnit||'Packung')}</select></label>
      <label>Angebotseinheit<select id="pbMatOfferUnit">${pbUnitOptions(m.offerUnit||'m²')}</select></label>
      <label>Mehrwertsteuer<select id="pbMatVat"><option ${m.vat==='normal'?'selected':''}>normal</option><option ${m.vat==='reduziert'?'selected':''}>reduziert</option><option ${m.vat==='keine'?'selected':''}>keine</option></select></label>
    </div>
    <div class="pb-material-savebar"><button type="button" class="secondary" data-mat-cancel>Abbrechen</button><button type="button" class="primary" data-mat-save>${pbMaterialPickContext?'Speichern und ins Aufmass übernehmen':'Material speichern'}</button></div>
  </section>`;
  const update=()=>{
    const tmp=pbReadMaterialForm(false),cc=pbMaterialCalc(tmp);
    if($('pbCalcMaterialCost'))$('pbCalcMaterialCost').value='CHF '+pbMaterialMoney(cc.materialCost);
    if($('pbCalcOverhead'))$('pbCalcOverhead').value='CHF '+pbMaterialMoney(cc.overhead);
    if($('pbCalcSelf'))$('pbCalcSelf').value='CHF '+pbMaterialMoney(cc.selfCost);
    if($('pbCalcProfit'))$('pbCalcProfit').value='CHF '+pbMaterialMoney(cc.profit);
    if($('pbMatPriceType')?.value==='kalkuliert'&&$('pbMatOfferPrice'))$('pbMatOfferPrice').value=cc.calculated.toFixed(2);
  };
  body.querySelectorAll('#pbMatPurchase,#pbMatOverhead,#pbMatProfit,#pbMatPriceType').forEach(x=>x?.addEventListener('input',update));
  $('pbMatCalcMode')?.addEventListener('change',()=>pbOpenMaterialEditor(pbReadMaterialForm(false),$('pbMatCalcMode').value));
  $('pbMatPhotoButton')?.addEventListener('click',()=>$('pbMatPhotoInput')?.click());
  $('pbMatPhotoInput')?.addEventListener('change',async e=>{const f=e.target.files?.[0];if(f){pbMaterialPhotoData=await img(f);if($('pbMatPhotoName'))$('pbMatPhotoName').value=f.name}});
  body.querySelector('[data-mat-back]')?.addEventListener('click',pbRenderMaterialList);
  body.querySelector('[data-mat-cancel]')?.addEventListener('click',pbRenderMaterialList);
  body.querySelector('[data-mat-save]')?.addEventListener('click',pbSaveMaterialForm);
  body.querySelector('[data-mat-delete]')?.addEventListener('click',()=>{if(item&&confirm('Material wirklich löschen?')){S.materialCatalog=pbMaterialDb().filter(x=>x.id!==item.id);save();pbRenderMaterialList()}});
}
function pbReadMaterialForm(validate=true){
  const val=id=>$(id)?.value??'';
  const m=pbMaterialNormalise({
    id:pbMaterialEditingId||undefined,
    articleNo:val('pbMatArticle'),designation:val('pbMatDesignation'),ean:val('pbMatEan'),supplier:val('pbMatSupplier'),
    serviceText:val('pbMatService'),comments:val('pbMatComments'),photo:pbMaterialPhotoData,
    calcMode:val('pbMatCalcMode')||'ohne Kalkulation',type:val('pbMatType')||'Material',
    purchasePrice:val('pbMatPurchase'),overheadPct:val('pbMatOverhead'),profitPct:val('pbMatProfit'),overrideProfit:val('pbMatOverride'),bomRule:val('pbMatBomRule'),
    priceType:val('pbMatPriceType')||'manuell',offerPrice:val('pbMatOfferPrice'),packageSize:val('pbMatPackageSize'),packageUnit:val('pbMatPackageUnit'),offerUnit:val('pbMatOfferUnit'),vat:val('pbMatVat')
  });
  if(validate&&!m.designation){alert('Bitte eine Bezeichnung eingeben.');return null}
  return m;
}
function pbSaveMaterialForm(){
  const m=pbReadMaterialForm(true);if(!m)return;
  const db=pbMaterialDb(),i=db.findIndex(x=>x.id===m.id);
  if(i>=0)db[i]=m;else db.push(m);
  save();
  if(pbMaterialPickContext){pbAddMaterialToAufmass(m);pbCloseMaterialManager()}
  else pbRenderMaterialList();
}
function pbAddMaterialToAufmass(m){
  const p=cur();if(!p)return;
  const c=ensurePlattenlegerProject(p),data=c.phaseData.aufmass=c.phaseData.aufmass||{};
  data.selectedMaterials=Array.isArray(data.selectedMaterials)?data.selectedMaterials:[];
  const calc=pbMaterialCalc(m);
  data.selectedMaterials.push({
    id:'sel_'+Date.now().toString(36),materialId:m.id,articleNo:m.articleNo,designation:m.designation,supplier:m.supplier,
    qty:'1',unit:m.offerUnit||'m²',offerPrice:calc.offer,calcMode:m.calcMode
  });
  save();if(c.phase==='aufmass')renderPlattenlegerPhaseContent(c);
  pbActionToast('Material wurde ins Aufmass übernommen.');
}
function pbAufmassMaterialChooser(data){
  const selected=Array.isArray(data.selectedMaterials)?data.selectedMaterials:[];
  return `<section class="pb-aufmass-materials">
    <div class="pb-aufmass-materials-head"><div><h3>Materialien</h3><p>Material aus der Datenbank wählen oder direkt neu erfassen.</p></div></div>
    <div class="pb-material-choice-grid">
      <article><span class="pb-choice-icon">▥</span><h4>Materialien auswählen</h4><p>Materialien aus der Materialdatenbank wählen.</p><button type="button" class="primary" data-aufmass-material-action="list">Materialien</button></article>
      <article><span class="pb-choice-icon">▦</span><h4>Materialien mit Kalkulation</h4><p>Materialien erfassen und mit Kalkulation.</p><button type="button" class="primary" data-aufmass-material-action="calc">Material erfassen (mit Kalkulation)</button></article>
      <article><span class="pb-choice-icon">▤</span><h4>Materialien ohne Kalkulation</h4><p>Materialien erfassen ohne Kalkulation.</p><button type="button" class="primary" data-aufmass-material-action="plain">Material erfassen (ohne Kalkulation)</button></article>
    </div>
    <div class="pb-selected-materials">${selected.length?`<div class="pb-selected-material-head"><span>Artikelnummer</span><span>Bezeichnung</span><span>Lieferant</span><span>Menge</span><span>Einheit</span><span></span></div>${selected.map((x,i)=>`<div class="pb-selected-material-row" data-selected-material="${i}"><span>${pbMaterialEscape(x.articleNo)}</span><span>${pbMaterialEscape(x.designation)}</span><span>${pbMaterialEscape(x.supplier)}</span><input data-sm-qty value="${pbMaterialEscape(x.qty||'1')}" inputmode="decimal"><select data-sm-unit>${pbUnitOptions(x.unit||'m²')}</select><button type="button" class="danger" data-sm-delete="${i}">×</button></div>`).join('')}`:'<div class="pb-material-empty">Noch kein Material für dieses Aufmass ausgewählt.</div>'}</div>
  </section>`;
}
...
function initTileTools(){
  $('openMaterialManager')?.addEventListener('click',ev=>{ev.preventDefault();ev.stopPropagation();pbOpenMaterialManager('list')});
  $('pbMaterialHome')?.addEventListener('click',pbCloseMaterialManager);
  document.querySelectorAll('.cad-nav-item').forEach(btn=>{
    if(btn.id==='openMaterialManager')return;
    btn.addEventListener('click',()=>{if(document.querySelector('.cad-main')?.classList.contains('pb-material-mode'))pbCloseMaterialManager()});
  });
...
function drawLiveWallDimension(preview){
  ...
  if(preview.snapTarget){
    ...
    fpCtx.fillText(
      preview.snappedToTarget ? 'An Wand eingerastet' : `Gegenüberliegende Wand: ${Math.round(t.distance)} cm`,
...
function drawFloorplan(preview=null){
  ...
  try{
    ...
    try{fpDrawOuterCornerMarkers2927()}catch(e){console.error('Eckmarkierung',e)}
    try{fpDrawInnerDimensionPoints2929()}catch(e){console.error('Innenmass-Markierung',e)}
...
function drawAllProfessionalWallDimensions(){
  ...
  for(const wall of walls){
    ...
    if(fpDimensionLayoutMap.has(wall.id)){
      drawProfessionalWallDimension(wall);          // Aussenmass
      drawInnerEffectiveWallDimension2935(wall);    // effektives Innenmass
      rendered++;
    }
  }
...
function fpObjectWallDistance(o){
  ...
  // Platzierungsmass ab linker Seite bzw. ab Wandanfang.
...
function drawFpObject(o,preview=false){
  ...
  // Masse innerhalb bzw. im Zentrum der Objektzeichnung
...
async function generateFloorplan2DPDF(){
  ...
  try{
    ...
    const brandLogo=await pbBrandLogoData();pbAddBrandLogo(doc,brandLogo,12,5,48);
...
function initCadShell(){
  ...
  if(resetBtn&&!resetBtn.dataset.module)resetBtn.onclick=()=>window.scrollTo({top:0,behavior:'smooth'});
...
/* ============================================================
   2.9.79 – DATEIEN FÜR KUNDEN UND PROJEKTE
   Binärdaten liegen separat in IndexedDB und blähen den Projekt-State
   bzw. localStorage nicht auf. Alle Dateiformate sind zulässig.
   ============================================================ */
const PB_FILE_DB='projekt-bau-owner-files-v1',PB_FILE_STORE='files';
let pbFileDbPromise=null;
function pbOpenFileDb(){
  if(pbFileDbPromise)return pbFileDbPromise;
  pbFileDbPromise=new Promise((resolve,reject)=>{
    const r=indexedDB.open(PB_FILE_DB,1);
    r.onupgradeneeded=()=>{const db=r.result;if(!db.objectStoreNames.contains(PB_FILE_STORE)){const s=db.createObjectStore(PB_FILE_STORE,{keyPath:'id'});s.createIndex('owner','owner',{unique:false})}};
    r.onsuccess=()=>resolve(r.result);r.onerror=()=>reject(r.error);
  });return pbFileDbPromise;
}
async function pbOwnerFiles(ownerType,ownerId){
  const db=await pbOpenFileDb(),owner=`${ownerType}:${ownerId}`;
  return new Promise((resolve,reject)=>{const tx=db.transaction(PB_FILE_STORE,'readonly'),idx=tx.objectStore(PB_FILE_STORE).index('owner'),r=idx.getAll(owner);r.onsuccess=()=>resolve((r.result||[]).sort((x,y)=>String(y.updatedAt||'').localeCompare(String(x.updatedAt||''))));r.onerror=()=>reject(r.error)});
}
async function pbPutOwnerFile(record){
  const db=await pbOpenFileDb();return new Promise((resolve,reject)=>{const tx=db.transaction(PB_FILE_STORE,'readwrite');tx.objectStore(PB_FILE_STORE).put(record);tx.oncomplete=()=>resolve(record);tx.onerror=()=>reject(tx.error)});
}
async function pbDeleteOwnerFile(id){
  const db=await pbOpenFileDb();return new Promise((resolve,reject)=>{const tx=db.transaction(PB_FILE_STORE,'readwrite');tx.objectStore(PB_FILE_STORE).delete(id);tx.oncomplete=()=>resolve();tx.onerror=()=>reject(tx.error)});
}
function pbFileSize(n){n=Number(n)||0;return n<1024?`${n} B`:n<1048576?`${(n/1024).toFixed(1)} KB`:n<1073741824?`${(n/1048576).toFixed(1)} MB`:`${(n/1073741824).toFixed(2)} GB`}
async function pbDownloadOwnerFile(id){
  const db=await pbOpenFileDb();const rec=await new Promise((resolve,reject)=>{const r=db.transaction(PB_FILE_STORE,'readonly').objectStore(PB_FILE_STORE).get(id);r.onsuccess=()=>resolve(r.result);r.onerror=()=>reject(r.error)});
  if(!rec?.blob)return;const url=URL.createObjectURL(rec.blob),x=document.createElement('a');x.href=url;x.download=rec.name||'Datei';x.click();setTimeout(()=>URL.revokeObjectURL(url),1500);
}
async function pbMountOwnerFiles(host,ownerType,ownerId,title='Dateien'){
  if(!host||!ownerId)return;
  host.classList.add('pb-owner-files');host.dataset.ownerType=ownerType;host.dataset.ownerId=ownerId;
  const draw=async()=>{
    const files=await pbOwnerFiles(ownerType,ownerId);
    host.innerHTML=`<div class="pb-files-head"><div><span class="pb-files-icon">▱</span><h3>${esc(title)}</h3></div><div class="pb-files-actions"><input data-owner-file-search placeholder="Datei suchen …"><button type="button" class="pb-round secondary" data-owner-file-folder title="Datei auswählen">▣</button><button type="button" class="pb-round primary" data-owner-file-add title="Datei hinzufügen">+</button><input type="file" data-owner-file-input multiple hidden></div></div>
      <div class="pb-file-breadcrumb">▱ Start</div>
      <div class="pb-file-drop" data-owner-file-drop><strong>Dateien hierher ziehen oder klicken</strong><span>Alle Dateiformate werden unterstützt.</span></div>
      <div class="pb-file-list">${files.map(f=>`<div class="pb-file-row" data-owner-file-row data-search="${esc(String(f.name||'').toLowerCase())}">
        <button type="button" class="pb-file-open" data-owner-file-open="${esc(f.id)}"><span>▤</span><strong>${esc(f.name)}</strong><small>${esc(f.type||'Datei')} · ${pbFileSize(f.size)} · ${fmtDate(String(f.updatedAt||'').slice(0,10))}</small></button>
        <button type="button" class="secondary" data-owner-file-edit="${esc(f.id)}">Bearbeiten</button>
        <button type="button" class="danger" data-owner-file-delete="${esc(f.id)}">Löschen</button>
      </div>`).join('')||'<p class="pb-files-empty">Noch keine Dateien vorhanden.</p>'}</div>`;
    const input=host.querySelector('[data-owner-file-input]'),drop=host.querySelector('[data-owner-file-drop]');
    const add=async fileList=>{
      for(const file of [...(fileList||[])]){await pbPutOwnerFile({id:u(),owner:`${ownerType}:${ownerId}`,ownerType,ownerId,name:file.name,type:file.type||'application/octet-stream',size:file.size,blob:file,lastModified:file.lastModified||Date.now(),createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()});if(ownerType==='project'){const p=S.projects.find(x=>x.id===ownerId);pbProjectHistoryAdd(p,'Datei hinzugefügt',file.name,'Datei');save()}}
      await draw();pbActionToast('Datei gespeichert.');
    };
    const choose=()=>input.click();
    host.querySelector('[data-owner-file-add]')?.addEventListener('click',choose);host.querySelector('[data-owner-file-folder]')?.addEventListener('click',choose);drop?.addEventListener('click',choose);
    input?.addEventListener('change',async()=>{await add(input.files);input.value=''});
    ['dragenter','dragover'].forEach(ev=>drop?.addEventListener(ev,e=>{e.preventDefault();drop.classList.add('dragover')}));
    ['dragleave','drop'].forEach(ev=>drop?.addEventListener(ev,e=>{e.preventDefault();drop.classList.remove('dragover')}));
    drop?.addEventListener('drop',e=>add(e.dataTransfer?.files));
    host.querySelector('[data-owner-file-search]')?.addEventListener('input',e=>{const q=e.target.value.trim().toLowerCase();host.querySelectorAll('[data-owner-file-row]').forEach(r=>r.hidden=q&&!r.dataset.search.includes(q))});
    host.querySelectorAll('[data-owner-file-open]').forEach(b=>b.addEventListener('click',()=>pbDownloadOwnerFile(b.dataset.ownerFileOpen)));
    host.querySelectorAll('[data-owner-file-edit]').forEach(b=>b.addEventListener('click',async()=>{
      const db=await pbOpenFileDb();const rec=await new Promise((resolve,reject)=>{const r=db.transaction(PB_FILE_STORE,'readonly').objectStore(PB_FILE_STORE).get(b.dataset.ownerFileEdit);r.onsuccess=()=>resolve(r.result);r.onerror=()=>reject(r.error)});
      if(!rec)return;const oldName=rec.name;const name=prompt('Dateiname bearbeiten:',rec.name);if(name===null)return;const clean=name.trim();if(!clean)return;rec.name=clean;rec.updatedAt=new Date().toISOString();await pbPutOwnerFile(rec);if(ownerType==='project'){const p=S.projects.find(x=>x.id===ownerId);pbProjectHistoryAdd(p,'Datei umbenannt',`${oldName} → ${clean}`,'Datei');save()}await draw();
    }));
    host.querySelectorAll('[data-owner-file-delete]').forEach(b=>b.addEventListener('click',async()=>{if(!confirm('Datei wirklich löschen?'))return;const db=await pbOpenFileDb();const rec=await new Promise((resolve,reject)=>{const r=db.transaction(PB_FILE_STORE,'readonly').objectStore(PB_FILE_STORE).get(b.dataset.ownerFileDelete);r.onsuccess=()=>resolve(r.result);r.onerror=()=>reject(r.error)});await pbDeleteOwnerFile(b.dataset.ownerFileDelete);if(ownerType==='project'){const p=S.projects.find(x=>x.id===ownerId);pbProjectHistoryAdd(p,'Datei gelöscht',rec?.name||'Datei','Datei');save()}await draw();pbActionToast('Datei gelöscht.')}))
  };
  await draw();
}

function pbRenderCustomersWorkspace(workspace,editId=null){
...
const PB_EMPLOYEE_COLORS=['#2563eb','#dc2626','#16a34a','#9333ea','#ea580c','#0891b2','#ca8a04','#db2777','#4f46e5','#059669'];
function pbEmployeeNames(){const set=new Set();(S.employees||[]).forEach(x=>set.add(String(typeof x==='string'?x:x.name||[x.firstName,x.lastName].filter(Boolean).join(' ')).trim()));(S.projects||[]).forEach(p=>{if(p.owner)set.add(p.owner);(p.areas||[]).forEach(a=>{if(a.worker)set.add(a.worker)})});return [...set].filter(Boolean).sort((a,b)=>a.localeCompare(b,'de-CH'))}
function pbEmployeeColor(name='Nicht zugewiesen'){S.meta=S.meta||{};S.meta.employeeColors=S.meta.employeeColors||{};if(!S.meta.employeeColors[name]){let h=0;for(const c of name)h=(h*31+c.charCodeAt(0))>>>0;S.meta.employeeColors[name]=PB_EMPLOYEE_COLORS[h%PB_EMPLOYEE_COLORS.length]}return S.meta.employeeColors[name]}
function pbCalendarEvents(customerId=''){
  pbSyncAllProjectCalendarData();
  const projects=(S.projects||[]).filter(p=>!customerId||p.customerId===customerId),ids=new Set(projects.map(p=>p.id)),events=[];
  (S.projectCalendarEvents||[]).filter(e=>ids.has(e.projectId)).forEach(e=>events.push({...e,source:'project'}));
  projects.forEach(p=>{(p.appointments||p.dates||[]).forEach(a=>events.push({...a,id:a.id||u(),projectId:p.id,projectName:p.name,employee:a.employee||p.owner||'Nicht zugewiesen',source:'appointment'}));(p.calendarEntries||[]).forEach(x=>events.push({...x,projectId:p.id,projectName:p.name,employee:x.employee||p.owner||'Nicht zugewiesen'}))});
  return events.sort((a,b)=>`${a.date||''}${a.startTime||''}`.localeCompare(`${b.date||''}${b.startTime||''}`));
}
function pbIsoWeek(date){
  const d=new Date(Date.UTC(date.getFullYear(),date.getMonth(),date.getDate()));d.setUTCDate(d.getUTCDate()+4-(d.getUTCDay()||7));const y=new Date(Date.UTC(d.getUTCFullYear(),0,1));return Math.ceil((((d-y)/86400000)+1)/7);
}
function pbMountMonthCalendar(host,events,initialMonth=new Date()){
  if(!host)return;let view=new Date(initialMonth.getFullYear(),initialMonth.getMonth(),1);
  const draw=()=>{
    const y=view.getFullYear(),m=view.getMonth(),today=new Date().toISOString().slice(0,10),monthLabel=new Intl.DateTimeFormat('de-CH',{month:'long',year:'numeric'}).format(view);
    const first=new Date(y,m,1),monday=new Date(first);monday.setDate(1-((first.getDay()+6)%7));
    let rows='';
    for(let w=0;w<6;w++){
      const ws=new Date(monday);ws.setDate(monday.getDate()+w*7);
      let days=`<div class="pb-cal-kw">KW ${pbIsoWeek(ws)}</div>`;
      for(let d=0;d<7;d++){
        const day=new Date(ws);day.setDate(ws.getDate()+d);const iso=`${day.getFullYear()}-${String(day.getMonth()+1).padStart(2,'0')}-${String(day.getDate()).padStart(2,'0')}`,outside=day.getMonth()!==m,dayEvents=events.filter(e=>e.date===iso || (e.date&&e.endDate&&iso>=e.date&&iso<=e.endDate));
        days+=`<div class="pb-cal-day ${outside?'muted outside':''} ${iso===today?'today':''}"><b>${day.getDate()}</b>${dayEvents.map(e=>`<button type="button" data-cal-project="${esc(e.projectId||'')}" style="--event-color:${pbEmployeeColor(e.employee||'Nicht zugewiesen')}"><span>${esc(e.startTime||'')}</span>${esc(e.title||e.number||'Termin')}</button>`).join('')}</div>`;
      }
      rows+=`<div class="pb-cal-week-row">${days}</div>`;
    }
    host.innerHTML=`<div class="pb-cal-toolbar"><button type="button" data-cal-prev>‹</button><strong>${esc(monthLabel)}</strong><button type="button" data-cal-today>Heute</button><button type="button" data-cal-next>›</button></div><div class="pb-cal-week pb-cal-week-with-kw"><span>KW</span><span>Mo</span><span>Di</span><span>Mi</span><span>Do</span><span>Fr</span><span>Sa</span><span>So</span></div><div class="pb-cal-grid pb-cal-grid-weeks">${rows}</div>`;
    pbBindTap(host.querySelector('[data-cal-prev]'),()=>{view=new Date(y,m-1,1);draw()});pbBindTap(host.querySelector('[data-cal-next]'),()=>{view=new Date(y,m+1,1);draw()});pbBindTap(host.querySelector('[data-cal-today]'),()=>{view=new Date();draw()});host.querySelectorAll('[data-cal-project]').forEach(btn=>pbBindTap(btn,()=>pbNavigateToProject(btn.dataset.calProject)));
  };draw();
}
...
const PB_CUSTOMER_RELATIONS=[
  ['projects','⌘','Projekte'],['appointments','▣','Termine'],['baudoku','▧','Baudoku'],['measurements','⌑','Aufmass'],['offers','▤','Offerten'],
...
function pbCustomerRelationItems(customer,type){
  pbSynchronizeAllData();
  const projects=pbCustomerProjects(customer);if(type==='projects')return projects;
  if(type==='appointments')return projects.flatMap(p=>(p.appointments||p.dates||[]).map(x=>({...x,_project:p})));
  if(type==='baudoku')return [];
  const phaseKey={measurements:'aufmass',offers:'offerte',acceptances:'abnahme'}[type];
  if(phaseKey)return projects.flatMap(p=>{const x=p.plattenleger?.phaseData?.[phaseKey];return x?[{...x,_project:p}]:[]});
  if(type==='orders')return projects.flatMap(p=>(p.orders||[]).map(x=>({...x,_project:p})));
  const map={dayworks:'dayworks',deliveries:'deliveries',invoices:'invoices',payments:'payments',reminders:'reminders',forms:'forms',contracts:'contracts',emails:'sentEmails',letters:'letters'};
  return projects.flatMap(p=>(p[map[type]]||[]).map(x=>({...x,_project:p})));
}
function pbCustomerRelationsHtml(customer){
  return `<section class="pb-customer-relations"><div class="pb-customer-relation-tabs">${PB_CUSTOMER_RELATIONS.map(([key,icon,label],i)=>{const count=pbCustomerRelationItems(customer,key).length;return `<button type="button" class="${i?'':'active'}" data-customer-relation="${key}"><i>${icon}</i>${label}${count?` <b>${count}</b>`:''}</button>`}).join('')}</div><div class="pb-customer-relation-content" data-customer-relation-content></div></section>`;
}
function pbCustomerRelationPrefix(type){return{projects:'PRO',appointments:'TER',baudoku:'BAU',measurements:'AUF',offers:'OFF',dayworks:'REG',orders:'AUFTR',deliveries:'LIE',acceptances:'ABN',invoices:'RE',payments:'ZA',reminders:'MAH',forms:'FOR',contracts:'VER',emails:'MAIL',letters:'BR'}[type]||'DOK'}
...
function pbOpenCustomerRelationCreate(workspace,customer,type){
  ...
  pbBindTap(modal.querySelector('[data-modal-save]'),async()=>{
    if(directPhase){pbOpenExistingProjectPhase(workspace,S.projects.find(x=>x.id===modal.querySelector('[data-new-project]')?.value),directPhase);return}
    const title=modal.querySelector('[data-new-title]')?.value.trim();if(!title){alert('Bitte eine Bezeichnung eingeben.');modal.querySelector('[data-new-title]')?.focus();return}
    const number=modal.querySelector('[data-new-number]')?.value,date=modal.querySelector('[data-new-date]')?.value,notes=modal.querySelector('[data-new-notes]')?.value||'',now=new Date().toISOString();
    if(isProject){const p={id:u(),projectNo:number,name:title,address:modal.querySelector('[data-new-address]')?.value||'',customerId:customer.id,customer:pbCustomerName(customer),phone:customer.phone||customer.mobile||'',startDate:date,owner:modal.querySelector('[data-new-owner]')?.value||'',description:notes,status:'Neu',createdAt:now,updatedAt:now,areas:[]};S.projects.unshift(p);pbApplyCustomerToProject(p,customer)}
    else{
      const p=S.projects.find(x=>x.id===modal.querySelector('[data-new-project]')?.value);if(!p)return;const item={id:u(),number,title,date,startTime:modal.querySelector('[data-new-start-time]')?.value||'',endTime:modal.querySelector('[data-new-end-time]')?.value||'',employee:modal.querySelector('[data-new-employee]')?.value||p.owner||'Nicht zugewiesen',status:modal.querySelector('[data-new-status]')?.value||'Entwurf',notes,customerId:customer.id,projectId:p.id,createdAt:now,updatedAt:now};
      const phaseKey={measurements:'aufmass',offers:'offerte',acceptances:'abnahme'}[type];if(phaseKey){p.plattenleger=p.plattenleger||{};p.plattenleger.phaseData=p.plattenleger.phaseData||{};p.plattenleger.phaseData[phaseKey]={...(p.plattenleger.phaseData[phaseKey]||{}),...item,[phaseKey==='aufmass'?'measureNo':phaseKey==='offerte'?'offerNo':'acceptanceNo']:number}}
      else if(type==='orders'){p.orders=p.orders||[];p.orders.push({...item,orderNo:number})}
      else{const map={appointments:'appointments',dayworks:'dayworks',deliveries:'deliveries',invoices:'invoices',payments:'payments',reminders:'reminders',forms:'forms',contracts:'contracts',emails:'sentEmails',letters:'letters'};const key=map[type];p[key]=p[key]||[];p[key].push(item)}
    }
    localStorage.setItem(K3,JSON.stringify(S));await Promise.resolve(window.PBStorage?.saveState?.(S,{reason:`customer-${type}-create-296101`}));try{window.ProjectBauOneDrive?.scheduleAutoSync?.(`customer-${type}-create`)}catch(_){ }modal.remove();pbRenderCustomerRelation(workspace,customer,type);pbActionToast(`${label} gespeichert.`);
  });
}

function pbOpenAppointmentEditor(workspace,customer,projectId='',appointmentId=''){
  const projects=pbCustomerProjects(customer);if(!projects.length){alert('Bitte zuerst ein Projekt für diesen Kunden erstellen.');return}
  let project=projects.find(p=>p.id===projectId)||projects[0];
  let appointment=(project.appointments||[]).find(x=>x.id===appointmentId)||null;
  document.querySelector('.pb-appointment-modal')?.remove();
  const modal=document.createElement('div');modal.className='pb-relation-modal pb-appointment-modal';
  const from=appointment?.date&&appointment?.startTime?`${appointment.date}T${appointment.startTime}`:(appointment?.from||'');
  const to=appointment?.date&&appointment?.endTime?`${appointment.date}T${appointment.endTime}`:(appointment?.to||'');
  modal.innerHTML=`<div class="pb-relation-dialog pb-appointment-dialog" role="dialog" aria-modal="true">
    <div class="pb-relation-dialog-head"><div><small>TERMIN</small><h2>▣ Termin</h2></div><button type="button" data-appointment-close>×</button></div>
    <div class="pb-appointment-customer"><strong>${esc(pbCustomerName(customer))}</strong><span>${esc(customer.fullAddress||'')}</span></div>
    <div class="pb-appointment-form">
      <label>Projekt<select data-ap-project>${projects.map(p=>`<option value="${esc(p.id)}" ${p.id===project.id?'selected':''}>${esc(p.name)} (${esc(p.projectNo||'')})</option>`).join('')}</select></label>
      <label>Objektadresse<input data-ap-address value="${esc(appointment?.address||project.address||'')}"></label>
      <label>von<input type="datetime-local" data-ap-from value="${esc(from)}"></label>
      <label>bis<input type="datetime-local" data-ap-to value="${esc(to)}"></label>
      <label>wiederholen<select data-ap-repeat><option>Nie</option><option>Täglich</option><option>Wöchentlich</option><option>Monatlich</option></select></label>
      <label>Bezeichnung<input data-ap-title value="${esc(appointment?.title||project.name||'')}"></label>
      <label>Typ<select data-ap-type><option>Termin</option><option>Besichtigung</option><option>Ausführung</option><option>Abnahme</option><option>Besprechung</option></select></label>
      <label>Status<select data-ap-status><option>Geplant</option><option>Bestätigt</option><option>Erledigt</option><option>Abgesagt</option></select></label>
      <label>Verknüpfung<select data-ap-link><option>Projekt</option><option>Kunde</option><option>Aufmass</option><option>Offerte</option></select></label>
      <label class="wide">Beschreibung<textarea rows="5" data-ap-notes>${esc(appointment?.notes||'')}</textarea></label>
      <label class="check wide"><input type="checkbox" data-ap-alt ${appointment?.alternateAddress?'checked':''}> Abweichende Terminadresse</label>
    </div>
    <div class="pb-relation-dialog-actions">${appointment?'<button type="button" class="danger" data-ap-delete>Löschen</button>':''}<button type="button" class="secondary" data-appointment-close>Abbrechen</button><button type="button" class="primary" data-ap-save>Termin speichern</button></div>
  </div>`;
  document.body.appendChild(modal);
  modal.querySelectorAll('[data-appointment-close]').forEach(b=>pbBindTap(b,()=>modal.remove()));
  const projectSel=modal.querySelector('[data-ap-project]');
  projectSel.addEventListener('change',()=>{project=projects.find(p=>p.id===projectSel.value)||projects[0];modal.querySelector('[data-ap-address]').value=project.address||''});
  pbBindTap(modal.querySelector('[data-ap-save]'),async()=>{
    project=projects.find(p=>p.id===projectSel.value)||projects[0];project.appointments=project.appointments||[];
    const fromVal=modal.querySelector('[data-ap-from]').value,toVal=modal.querySelector('[data-ap-to]').value;
    if(!fromVal){alert('Bitte Startdatum und Uhrzeit eingeben.');return}
    const [date,startTime]=fromVal.split('T'),endTime=toVal?.split('T')[1]||'';
    const obj=appointment||{id:u(),createdAt:new Date().toISOString()};
    Object.assign(obj,{customerId:customer.id,projectId:project.id,date,startTime,endTime,from:fromVal,to:toVal,address:modal.querySelector('[data-ap-address]').value,title:modal.querySelector('[data-ap-title]').value.trim()||'Termin',repeat:modal.querySelector('[data-ap-repeat]').value,type:modal.querySelector('[data-ap-type]').value,status:modal.querySelector('[data-ap-status]').value,link:modal.querySelector('[data-ap-link]').value,notes:modal.querySelector('[data-ap-notes]').value,alternateAddress:modal.querySelector('[data-ap-alt]').checked,employee:project.owner||'',updatedAt:new Date().toISOString()});
    if(!appointment)project.appointments.push(obj);
    pbProjectCalendarSync(project);
    pbProjectHistoryAdd(project,appointment?'Termin geändert':'Termin erstellt',`${obj.title} · ${obj.date} ${obj.startTime||''}`,'Termin');
    save();await window.PBStorage?.saveState?.(S,{reason:'appointment-save-2984'});try{window.ProjectBauOneDrive?.scheduleAutoSync?.('appointment-save')}catch(_){}
    modal.remove();pbRenderCustomerRelation(workspace,customer,'appointments');pbActionToast('Termin gespeichert.');
  });
  if(appointment)pbBindTap(modal.querySelector('[data-ap-delete]'),()=>{if(!confirm('Termin wirklich löschen?'))return;project.appointments=(project.appointments||[]).filter(x=>x.id!==appointment.id);pbProjectCalendarSync(project);pbProjectHistoryAdd(project,'Termin gelöscht',appointment.title||'Termin','Termin');save();modal.remove();pbRenderCustomerRelation(workspace,customer,'appointments')});
}
...
function pbRenderCustomerRelation(workspace,customer,type='projects'){
  ...
  else if(type==='appointments')target.innerHTML=`<div class="pb-relation-tools"><strong>Termine</strong><button type="button" class="primary" data-appointment-new>+</button></div><div class="pb-relation-cards">${items.map(x=>`<button type="button" data-appointment-edit="${esc(x.id||'')}" data-appointment-project="${esc(x._project?.id||x.projectId||'')}"><strong>${esc(x.title||x.number||'Termin')}</strong><span>${esc(x._project?.name||'')}</span><small>${fmtDate(x.date||'')} · ${esc(x.startTime||'')}${x.endTime?'–'+esc(x.endTime):''}</small></button>`).join('')||'<p class="pb-relation-empty">Für diesen Kunden sind noch keine Termine vorhanden.</p>'}</div>`;
  else if(type==='baudoku')pbRenderBaudoku(target,p);
  ...
  pbBindTap(target.querySelector('[data-appointment-new]'),()=>pbOpenAppointmentEditor(workspace,customer));
  target.querySelectorAll('[data-appointment-edit]').forEach(btn=>pbBindTap(btn,()=>pbOpenAppointmentEditor(workspace,customer,btn.dataset.appointmentProject,btn.dataset.appointmentEdit)));
}

function pbRenderCustomerEditorV2958(workspace,editId){
  const customer=editId&&editId!=='new'?S.customers.find(c=>c.id===editId):null;
  if(!customer&&editId!=='new'){pbRenderCustomersWorkspace(workspace);return}
  ...
  if(customer)pbMountOwnerFiles(workspace.querySelector('[data-customer-files]'),'customer',customer.id,'Dateien');
...
/* Projekt Bau 2.9.91: non-destructive ERP navigation shell. */
(()=>{
  ...
  ready(()=>{
    ...
    const roadmap={
      ...
      Hilfe:'Projekt Bau 2.9.91 PRO CLEAN · Lokale Daten, OneDrive, PDF, Aufmass sowie 2D/3D CAD bleiben unverändert verfügbar.'
    };
    document.querySelectorAll('.cad-nav-item[data-module]').forEach(btn=>btn.addEventListener('click',()=>{
      activate(btn);
      const name=btn.dataset.module;
      document.body.classList.remove('pb-shell-focus');document.querySelectorAll('.cad-main>section').forEach(x=>x.classList.remove('pb-shell-visible'));document.body.classList.add('pb-module-mode');
      ...
      workspace.innerHTML=`<div class="pb-module-head"><div><span class="eyebrow">MODUL</span><h2>${name}</h2></div><button type="button" class="secondary" data-close-module>Schliessen</button></div><div class="pb-module-content-empty"></div>`;
    ...
    document.querySelectorAll('.cad-nav-item[data-scroll]').forEach(btn=>btn.addEventListener('click',()=>{
      activate(btn);document.body.classList.remove('pb-customer-mode','pb-module-mode');workspace?.classList.add('hidden');
      const target=document.querySelector(btn.dataset.scroll),section=target?.closest('section');document.querySelectorAll('.cad-main>section').forEach(x=>x.classList.toggle('pb-shell-visible',x===section));document.body.classList.toggle('pb-shell-focus',!!section);
    }));
...
(()=>{
  const VERSION='2.9.91 PRO CLEAN';
...
/* ============================================================
   2.9.71 – UNTERNEHMENSEINSTELLUNGEN
   Struktur und Arbeitsweise nach der gelieferten Referenz:
   Firma, Kontakt, Bank, Steuer, Funktionen, Kalkulation,
   Offerte, Rechnungen, Zeiterfassung, HR, Einheiten, Aufmass,
   DATEV, SEPA und weitere Einstellungen.
   ============================================================ */
function pbSettingsVal(key,fallback=''){const s=pbCompanySettings();return s[key]??fallback}
function pbSettingsInput(key,label,type='text',extra=''){
  const v=pbSettingsVal(key,'');
  if(type==='checkbox')return `<label class="pb-setting-check"><input type="checkbox" data-setting="${key}" ${v?'checked':''}><span>${label}</span></label>`;
  if(type==='textarea')return `<label>${label}<textarea data-setting="${key}" rows="4">${esc(v)}</textarea></label>`;
  return `<label>${label}<input type="${type}" data-setting="${key}" value="${esc(v)}" ${extra}></label>`;
}
function pbSettingsSelect(key,label,opts){
  const v=String(pbSettingsVal(key,opts[0]||''));return `<label>${label}<select data-setting="${key}">${opts.map(x=>`<option ${String(x)===v?'selected':''}>${esc(x)}</option>`).join('')}</select></label>`;
}
function pbSettingsSection(title,body,sub=''){
  return `<section class="pb-settings-card"><header><div><h3>${title}</h3>${sub?`<p>${sub}</p>`:''}</div></header><div class="pb-settings-grid">${body}</div></section>`;
}
function pbRenderSettingsModule(){
  const s=pbCompanySettings();
  const functions=['Angebotserstellung','Materialien','Projekte / Aufgaben','Mitarbeiter','Regie','Lager','Verträge','Aufmass','Lieferanten','Zahlungen','Rechnungen','Formulare','Buchhaltung'];
  const units=PB_MEASURE_UNITS;
  return `<div class="pb-settings-page">
    <div class="pb-settings-title"><div><span>⚙</span><h2>Unternehmenseinstellungen</h2></div><button type="button" class="primary" data-settings-save>Einstellungen speichern</button></div>
    ${pbSettingsSection('Unternehmensangaben',
      pbSettingsInput('companyName','Name')+
      pbSettingsSelect('legalForm','Firmierung',['GmbH','AG','Einzelfirma','Kollektivgesellschaft','Andere'])+
      pbSettingsInput('managingDirector','Geschäftsführung')+
      pbSettingsInput('registerNo','Registernummer')+
      pbSettingsInput('registerCourt','Registergericht')
    )}
    ${pbSettingsSection('Adresse',
      pbSettingsSelect('salutation','Anrede/Firmierung',['Firma','Herr','Frau'])+
      pbSettingsInput('street','Strasse')+
      pbSettingsInput('houseNo','Hausnummer')+
      pbSettingsInput('zip','PLZ')+
      pbSettingsInput('city','Stadt')+
      pbSettingsSelect('country','Land / Firmensitz',['Schweiz','Deutschland','Österreich','Liechtenstein'])+
      pbSettingsInput('canton','Kanton')
    )}
    ${pbSettingsSection('Kontaktdaten',
      pbSettingsInput('phone','Telefon')+pbSettingsInput('mobile','Mobil')+
      pbSettingsInput('website','Webseite')+pbSettingsInput('email','E-Mail','email')
    )}
    ${pbSettingsSection('Bankverbindung',
      pbSettingsInput('bankAccountName','Kontoinhaber')+pbSettingsInput('bankName','Bankname')+
      pbSettingsInput('iban','IBAN')+pbSettingsInput('bic','BIC')
    )}
    ${pbSettingsSection('Steuer / Finanzamt',
      pbSettingsInput('vatNo','Mehrwertsteuernummer')+pbSettingsInput('uid','Steuernummer / UID')+
      pbSettingsInput('taxOffice','Finanzamt')+pbSettingsInput('taxNote','Steuerhinweis')
    )}
    ${pbSettingsSection('Verwendete Funktionen',
      `<div class="pb-setting-functions">${functions.map(x=>`<label><input type="checkbox" checked disabled><span>${x}</span></label>`).join('')}</div>`,
      'Die aktivierten Module werden im Programm verwendet.'
    )}
    ${pbSettingsSection('Künstliche Intelligenz Funktionen',
      pbSettingsInput('aiEnabled','KI-Funktionen aktiv','checkbox')+pbSettingsInput('aiContext','Bereich / Kontext')
    )}
    ${pbSettingsSection('Kalkulation',
      `<div class="pb-settings-subtitle">Lohnkalkulation</div>`+
      pbSettingsInput('laborHourly','Stundenlohn (CHF/h)','number','step="0.01"')+
      pbSettingsInput('laborOverhead','Gemeinkostenzuschlag Lohn (%)','number','step="0.01"')+
      pbSettingsInput('laborProfit','Verrechnungszuschlag Lohn (%)','number','step="0.01"')+
      `<div class="pb-settings-subtitle">Materialkalkulation</div>`+
      pbSettingsInput('materialOverhead','Gemeinkostenzuschlag Material (%)','number','step="0.01"')+
      pbSettingsInput('materialProfit','Gewinnzuschlag Material (%)','number','step="0.01"')+
      `<div class="pb-settings-subtitle">Aufschlag</div>`+
      pbSettingsInput('offerSurcharge','Aufschlag (%)','number','step="0.01"')+
      pbSettingsInput('orderSurcharge','Auftragssumme / Zuschlag (%)','number','step="0.01"')
    )}
    ${pbSettingsSection('Stückliste / Tiefenkalkulation',
      pbSettingsInput('costCenterEnabled','Kostenstellen verwenden','checkbox')+
      pbSettingsInput('materialGroupEnabled','Materialgruppen verwenden','checkbox')+
      pbSettingsInput('externalCostEnabled','Fremdleistungen verwenden','checkbox')
    )}
    ${pbSettingsSection('EFB Preisblätter',
      pbSettingsInput('efbBaseWage','Mittellohn ML (CHF/h)','number','step="0.01"')+
      pbSettingsInput('efbWageOverhead','Lohngebundene Kosten (%)','number','step="0.01"')+
      pbSettingsInput('efbWageAncillary','Lohnnebenkosten (%)','number','step="0.01"')+
      pbSettingsInput('efbCalcWage','Kalkulationslohn (CHF/h)','number','step="0.01"')
    )}
    ${pbSettingsSection('E-Mail',
      pbSettingsInput('mailStart','E-Mail-Funktion aktiv','checkbox')+
      pbSettingsInput('mailSignature','E-Mail Signatur','textarea')
    )}
    ${pbSettingsSection('Offerte',
      pbSettingsInput('offerValidity','Gültigkeit Offerte (Tage)','number','min="1"')+
      pbSettingsInput('payment1','1. Akonto (%)','number','step="0.01"')+
      pbSettingsInput('payment2','2. Akonto (%)','number','step="0.01"')+
      pbSettingsInput('payment3','Schlusszahlung (%)','number','step="0.01"')+
      pbSettingsInput('offerTerms','Allgemeine Geschäftsbedingungen / Ausführungsgrundlagen','textarea')+
      pbSettingsInput('guarantee','Garantieleistungen','textarea')+
      pbSettingsInput('thanks','Schlusstext','textarea')
    )}
    ${pbSettingsSection('Rechnungen',
      pbSettingsInput('invoiceManual','Rechnungsnummer manuell erfassen','checkbox')+
      pbSettingsInput('invoiceQr','QR-Rechnung erzeugen','checkbox')+
      pbSettingsInput('invoiceEmail','Rechnung per E-Mail versenden','checkbox')+
      pbSettingsInput('invoicePaymentTerm','Zahlungsziel (Tage)','number','min="0"')
    )}
    ${pbSettingsSection('Zeiterfassung',
      pbSettingsInput('timeStart','Start','time')+pbSettingsInput('timeEnd','Ende','time')+
      pbSettingsInput('timePause','Pause','time')+
      pbSettingsInput('timeCustomer','Kundenauswahl erlauben','checkbox')+
      pbSettingsInput('timeProject','Projektauswahl erlauben','checkbox')
    )}
    ${pbSettingsSection('Untertyp',
      pbSettingsInput('subtypes','Untertypen / Bereiche','textarea')
    )}
    ${pbSettingsSection('HR Schlüssel',
      pbSettingsInput('hrVacation','Ferien / Schlüssel')+
      pbSettingsInput('hrSickness','Krankheit / Schlüssel')+
      pbSettingsInput('hrOvertime','Überzeit / Schlüssel')+
      pbSettingsInput('hrSpecialLeave','Sonderurlaub / Schlüssel')
    )}
    ${pbSettingsSection('Materialien / Leistungen',
      `<div class="pb-setting-units"><div class="pb-setting-unit-head"><strong>Einheiten</strong><span>${units.length} Einheiten aktiv</span></div><div class="pb-setting-unit-grid">${units.map(u=>`<span>${esc(u)}</span>`).join('')}</div></div>`,
      'Diese Einheiten werden programmweit für Material, Leistungen, Aufmass und Offerten verwendet.'
    )}
    ${pbSettingsSection('Aufmass',
      pbSettingsInput('measureWall','Wand verwenden','checkbox')+
      pbSettingsInput('measureCeiling','Decke verwenden','checkbox')+
      pbSettingsInput('measureFloor','Boden verwenden','checkbox')+
      pbSettingsInput('measureWindows','Fenster verwenden','checkbox')+
      pbSettingsInput('measureDoors','Türen verwenden','checkbox')+
      pbSettingsInput('measureOpenings','Öffnungen berücksichtigen','checkbox')
    )}
    ${pbSettingsSection('DATEV Einstellungen',
      pbSettingsInput('datevEnabled','DATEV-Konfiguration aktiv','checkbox')+
      pbSettingsInput('datevConsultant','Beraternummer')+
      pbSettingsInput('datevClient','Mandantennummer')
    )}
    ${pbSettingsSection('SEPA',
      pbSettingsSelect('sepaMode','SEPA Modus',['alle Zahlungen einzeln','Sammelzahlung','deaktiviert'])+
      pbSettingsSelect('sepaVersion','SEPA Version',['pain.001.001.03','pain.001.001.09'])
    )}
    ${pbSettingsSection('Andere Einstellungen',
      pbSettingsSelect('csvExport','CSV Export für',['Excel','Standard CSV','Semikolon CSV'])+
      pbSettingsInput('pdfNewTab','PDFs in neuem Tab öffnen','checkbox')
    )}
  </div>`;
}
function pbBindSettingsWorkspace(w){
  w.querySelector('[data-settings-save]')?.addEventListener('click',()=>{
    const s=pbCompanySettings();
    w.querySelectorAll('[data-setting]').forEach(el=>{
      const k=el.dataset.setting;s[k]=el.type==='checkbox'?el.checked:el.value;
    });
    S.settings=S.settings||{};S.settings.company=s;save();pbActionToast('Einstellungen gespeichert.');
  });
}

/* Projekt Bau 2.9.65: ERP workspaces backed by existing project data. */
(()=>{
 ...
 function shell(title,body,sub='ERP / CRM'){return `<div class="pb-module-head"><div><span class="eyebrow">${e(sub)}</span><h2>${e(title)}</h2></div><button type="button" class="secondary" data-close-module>← Zurück</button></div>${body}`}

 function splitProjectAddress(value=''){
   const s=String(value||'').trim();
   let street=s,zip='',city='';
   // Schweizer Adressen: "Strasse 7, 8906 Bonstetten" / "Strasse 7 8906 Bonstetten"
   const m=s.match(/^(.*?)(?:,\s*|\s+)(\d{4})\s+(.+)$/);
   if(m){street=m[1].trim().replace(/,\s*$/,'');zip=m[2];city=m[3].trim()}
   return {street,zip,city};
 }
 function projectNo(p,index){
   if(p.projectNo)return p.projectNo;
   const year=new Date(p.createdAt||p.startDate||Date.now()).getFullYear();
   return `P-${year}-${String(Math.max(1,projects().length-index)).padStart(3,'0')}`;
 }
 function projectModule(){
  pbSynchronizeAllData();
   const ps=[...projects()].sort((x,y)=>String(projectNo(y,0)).localeCompare(String(projectNo(x,0)),'de-CH',{numeric:true}));
   const rows=ps.map((p,i)=>{
     const ad=splitProjectAddress(p.address);
     return `<button type="button" class="pb-projects-row" data-pb-project="${e(p.id)}"
       data-project-search="${e([projectNo(p,i),p.customer,p.name,ad.street,ad.zip,ad.city,p.status,p.owner].join(' ').toLowerCase())}">
       <span>${e(projectNo(p,i))}</span><span>${e(p.customer||'')}</span><span>${e(p.name||'')}</span>
       <span>${e(ad.street)}</span><span>${e(ad.zip)}</span><span>${e(ad.city)}</span>
       <span>${e(String(p.status||p.plattenleger?.phase||'neu').toLowerCase())}</span><span>${e(p.owner||'')}</span>
     </button>`;
   }).join('');
   return `<div class="pb-projects-module">
     <div class="pb-projects-titlebar">
       <div><span class="pb-projects-symbol">⌘</span><h2>Projekte</h2></div>
       <div class="pb-projects-title-actions">
         <button type="button" title="Ansicht">≡</button>
         <button type="button" title="Export">⇩</button>
         <button type="button" class="primary" data-projects-new title="Neues Projekt">+</button>
       </div>
     </div>
     <div class="pb-projects-table">
       <div class="pb-projects-head">
         <span>↕ NUMMER</span><span>↕ KUNDE</span><span>↕ BEZEICHNUNG</span><span>↕ STRASSE</span>
         <span>↕ ZIP</span><span>↕ STADT</span><span>↕ STATUS</span><span>↕ PROJEKTLEITER ⚙</span>
       </div>
       <div class="pb-projects-filter">
         <input data-pf="all" placeholder="⌕">
         <input data-pf="customer" placeholder="⌕">
         <input data-pf="name" placeholder="⌕">
         <input data-pf="street" placeholder="⌕">
         <input data-pf="zip" placeholder="⌕">
         <input data-pf="city" placeholder="⌕">
         <select data-pf="status"><option value="">alle</option>${[...new Set(ps.map(p=>String(p.status||p.plattenleger?.phase||'neu').toLowerCase()))].sort().map(x=>`<option>${e(x)}</option>`).join('')}</select>
         <select data-pf="owner"><option value="">alle</option>${[...new Set(ps.map(p=>p.owner).filter(Boolean))].sort((x,y)=>x.localeCompare(y,'de-CH')).map(x=>`<option>${e(x)}</option>`).join('')}</select>
       </div>
       <div class="pb-projects-body">${rows||'<div class="pb-projects-empty">Noch keine Projekte vorhanden.</div>'}</div>
     </div>
   </div>`;
 }
 function bindProjectModule(w){
   const filters=[...w.querySelectorAll('[data-pf]')];
   const run=()=>{
     const vals=Object.fromEntries(filters.map(x=>[x.dataset.pf,String(x.value||'').trim().toLowerCase()]));
     w.querySelectorAll('.pb-projects-row').forEach(row=>{
       const cells=[...row.children].map(x=>String(x.textContent||'').toLowerCase());
       const ok=(!vals.all||row.dataset.projectSearch.includes(vals.all))&&
         (!vals.customer||cells[1].includes(vals.customer))&&(!vals.name||cells[2].includes(vals.name))&&
         (!vals.street||cells[3].includes(vals.street))&&(!vals.zip||cells[4].includes(vals.zip))&&
         (!vals.city||cells[5].includes(vals.city))&&(!vals.status||cells[6]===vals.status)&&(!vals.owner||cells[7].toLowerCase()===vals.owner);
       row.hidden=!ok;
     });
   };
   filters.forEach(x=>x.addEventListener(x.tagName==='SELECT'?'change':'input',run));
   w.querySelectorAll('[data-pb-project]').forEach(row=>pbBindTap(row,()=>pbNavigateToProject(row.dataset.pbProject)));
   w.querySelector('[data-projects-new]')?.addEventListener('click',()=>{
     document.body.classList.remove('pb-module-mode');w.classList.add('hidden');
     document.querySelector('.card.editor')?.scrollIntoView({behavior:'smooth',block:'start'});
 ...
 function cockpit(){
   pbSynchronizeAllData();
   const ps=projects(),customers=Array.isArray(S?.customers)?S.customers:[],today=new Date().toISOString().slice(0,10);
   const active=ps.filter(p=>!['abgeschlossen','verloren'].includes(String(p.status||'').toLowerCase()));
   const appointments=ps.flatMap(p=>(p.appointments||[]).map(x=>({...x,_project:p})));
   const tasks=ps.flatMap(p=>(p.projectTasks||[]).map(x=>({...x,_project:p})));
   const invoices=ps.flatMap(p=>(p.invoices||[]).map(x=>({...x,_project:p})));
   const todayAppointments=appointments.filter(x=>x.date===today).sort((x,y)=>String(x.startTime||'').localeCompare(String(y.startTime||'')));
   const openTasks=tasks.filter(x=>!x.done);
   const overdueTasks=openTasks.filter(x=>x.date&&x.date<today);
   const dueToday=openTasks.filter(x=>x.date===today);
   const openInvoices=invoices.filter(x=>!['bezahlt','paid','erledigt'].includes(String(x.status||'').toLowerCase()));
   const openAmount=openInvoices.reduce((s,x)=>s+(Number(x.total||x.gross)||0),0);

   const nextAction=()=>{
     if(overdueTasks.length)return `<strong>${overdueTasks.length} überfällige Aufgabe${overdueTasks.length===1?'':'n'}</strong><span>Heute zuerst erledigen</span>`;
     if(todayAppointments.length)return `<strong>${todayAppointments.length} Termin${todayAppointments.length===1?'':'e'} heute</strong><span>Kalender prüfen</span>`;
     if(active.length)return `<strong>${active.length} aktive Projekte</strong><span>Aktuellen Projektstand prüfen</span>`;
     return `<strong>Alles bereit</strong><span>Neuen Kunden oder neues Projekt anlegen</span>`;
   };

   return shell('Cockpit',`
     <section class="pb87-welcome">
       <div><span class="eyebrow">HEUTE</span><h3>Was ist als Nächstes zu tun?</h3><p>Alle wichtigen Informationen an einem Ort. Kunde und Projekt bleiben die zentralen Datensätze.</p></div>
       <div class="pb87-next">${nextAction()}</div>
     </section>

     <div class="pb87-quick-actions">
       <button type="button" class="primary" data-pb-module="Kunden">＋ Kunde</button>
       <button type="button" class="primary" data-pb-module="Projekte">＋ Projekt</button>
       <button type="button" data-pb-module="Projektkalender">▣ Termin / Kalender</button>
       <button type="button" data-pb-module="Aufgaben">✓ Aufgabe</button>
       <button type="button" data-pb-module="Offerten">CHF Offerte</button>
       <button type="button" data-pb-module="Rechnungen">₣ Rechnung</button>
     </div>

     <div class="pb87-kpis">
       <button type="button" data-pb-module="Projekte"><small>Aktive Projekte</small><strong>${active.length}</strong><span>${ps.length} insgesamt</span></button>
       <button type="button" data-pb-module="Projektkalender"><small>Termine heute</small><strong>${todayAppointments.length}</strong><span>${appointments.length} gespeichert</span></button>
       <button type="button" data-pb-module="Aufgaben"><small>Offene Aufgaben</small><strong>${openTasks.length}</strong><span>${overdueTasks.length} überfällig</span></button>
       <button type="button" data-pb-module="Rechnungen"><small>Offene Rechnungen</small><strong>${openInvoices.length}</strong><span>${money(openAmount)}</span></button>
     </div>

     <div class="pb87-dashboard">
       <section class="pb87-card">
         <div class="pb87-card-head"><div><span class="eyebrow">HEUTE</span><h3>Termine & Aufgaben</h3></div><button data-pb-module="Projektkalender">Kalender öffnen</button></div>
         <div class="pb87-agenda">
           ${todayAppointments.slice(0,6).map(x=>`<button data-pb-project="${e(x._project.id)}"><time>${e(x.startTime||'–')}</time><div><strong>${e(x.title||'Termin')}</strong><span>${e(x._project.name||'')}</span></div><em>Termin</em></button>`).join('')}
           ${dueToday.slice(0,6).map(x=>`<button data-pb-project="${e(x._project.id)}"><time>${e(x.time||'–')}</time><div><strong>${e(x.title||'Aufgabe')}</strong><span>${e(x._project.name||'')}</span></div><em>Aufgabe</em></button>`).join('')}
           ${!todayAppointments.length&&!dueToday.length?'<p class="pb87-empty">Heute sind keine Termine oder Aufgaben eingetragen.</p>':''}
         </div>
       </section>

       <section class="pb87-card">
         <div class="pb87-card-head"><div><span class="eyebrow">PROJEKTE</span><h3>Zuletzt bearbeitet</h3></div><button data-pb-module="Projekte">Alle Projekte</button></div>
         <div class="pb87-project-list">
           ${[...ps].sort((x,y)=>String(y.updatedAt||y.createdAt||'').localeCompare(String(x.updatedAt||x.createdAt||''))).slice(0,7).map(p=>`<button data-pb-project="${e(p.id)}"><div><strong>${e(p.name||'Projekt')}</strong><span>${e(p.customer||p.address||'')}</span></div><em>${e(p.status||'Aktiv')}</em></button>`).join('')||'<p class="pb87-empty">Noch keine Projekte vorhanden.</p>'}
         </div>
       </section>
     </div>

     <section class="pb87-workflow">
       <div class="pb87-card-head"><div><span class="eyebrow">EINFACHER ARBEITSABLAUF</span><h3>Vom Kunden bis zur Zahlung</h3></div></div>
       <div class="pb87-flow">
         <button data-pb-module="Kunden"><b>1</b><strong>Kunde</strong><span>Stammdaten erfassen</span></button>
         <span>→</span>
         <button data-pb-module="Projekte"><b>2</b><strong>Projekt</strong><span>Projekt anlegen</span></button>
         <span>→</span>
         <button data-pb-scroll="#plattenlegerCockpit"><b>3</b><strong>Aufmass</strong><span>Leistung aufnehmen</span></button>
         <span>→</span>
         <button data-pb-module="Offerten"><b>4</b><strong>Offerte</strong><span>Angebot erstellen</span></button>
         <span>→</span>
         <button data-pb-module="Rechnungen"><b>5</b><strong>Rechnung</strong><span>Abrechnen & Zahlung</span></button>
       </div>
     </section>
   `,'ÜBERSICHT')
 }

 const PB91_STANDALONE_MODULES=[
   'Materialien','Leistungen','Mitarbeiter','Textbausteine','Formular Editor','Bestand','Verträge',
   'Arbeitszeiten','Meine Aufträge','Berichte','Fotoberichte','Baustellenfotos','2D / 3D / CAD',
   'Grundrisse','Bautagebücher','Regieberichte','Einsatzplanung','Projektaufgaben'
 ];
 const PB91_DEFS={
   'Leistungen':[['Bezeichnung','text'],['Einheit','unit'],['Preis CHF','number'],['Leistungstext','textarea']],
   'Mitarbeiter':[['Vorname','text'],['Nachname','text'],['Funktion','text'],['Telefon','text'],['E-Mail','email']],
   'Textbausteine':[['Titel','text'],['Kategorie','text'],['Text','textarea']],
   'Formular Editor':[['Formularname','text'],['Kategorie','text'],['Inhalt','textarea']],
   'Bestand':[['Artikel','text'],['Lagerort','text'],['Menge','number'],['Einheit','unit'],['Mindestbestand','number']],
   'Verträge':[['Vertragsnummer','text'],['Titel','text'],['Vertragspartner','text'],['Beginn','date'],['Ende','date'],['Status','status']],
   'Arbeitszeiten':[['Mitarbeiter','text'],['Datum','date'],['Von','time'],['Bis','time'],['Stunden','number'],['Tätigkeit','textarea']],
   'Meine Aufträge':[['Auftragsnummer','text'],['Auftraggeber','text'],['Bezeichnung','text'],['Datum','date'],['Status','status']],
   'Berichte':[['Titel','text'],['Typ','text'],['Datum','date'],['Beschreibung','textarea']],
   'Fotoberichte':[['Titel','text'],['Datum','date'],['Beschreibung','textarea']],
   'Baustellenfotos':[['Titel','text'],['Datum','date'],['Kategorie','text'],['Beschreibung','textarea']],
   '2D / 3D / CAD':[['Titel','text'],['Datum','date'],['Beschreibung','textarea']],
   'Grundrisse':[['Titel','text'],['Datum','date'],['Massstab','text'],['Beschreibung','textarea']],
   'Bautagebücher':[['Datum','date'],['Titel','text'],['Wetter','text'],['Ausgeführte Arbeiten','textarea']],
   'Regieberichte':[['Regie-Nr.','text'],['Datum','date'],['Auftraggeber','text'],['Stunden','number'],['Arbeiten','textarea']],
   'Einsatzplanung':[['Datum','date'],['Mitarbeiter','text'],['Einsatzort','text'],['Von','time'],['Bis','time']],
   'Projektaufgaben':[['Titel','text'],['Verantwortlich','text'],['Termin','date'],['Priorität','priority'],['Status','status'],['Beschreibung','textarea']]
 };
 function pb91Store(){
   S.standaloneModules=S.standaloneModules||{};
   PB91_STANDALONE_MODULES.forEach(n=>{if(!Array.isArray(S.standaloneModules[n]))S.standaloneModules[n]=[]});
   return S.standaloneModules;
 }
 function pb91Esc(v){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
 function pb91Field(label,type,value='',i=0){
   const k=`f${i}`,v=pb91Esc(value);
   if(type==='textarea')return `<label class="wide">${pb91Esc(label)}<textarea rows="4" data-pb91-field="${k}">${v}</textarea></label>`;
   if(type==='unit')return `<label>${pb91Esc(label)}<select data-pb91-field="${k}">${pbUnitOptions(value||'Stk.')}</select></label>`;
   if(type==='status')return `<label>${pb91Esc(label)}<select data-pb91-field="${k}"><option>Offen</option><option>In Bearbeitung</option><option>Erledigt</option><option>Archiviert</option></select></label>`;
   if(type==='priority')return `<label>${pb91Esc(label)}<select data-pb91-field="${k}"><option>Niedrig</option><option selected>Normal</option><option>Hoch</option><option>Dringend</option></select></label>`;
   return `<label>${pb91Esc(label)}<input type="${type}" data-pb91-field="${k}" value="${v}"></label>`;
 }
 function pb91Label(rec,defs){
   const vals=defs.map((d,i)=>rec[`f${i}`]).filter(Boolean);
   return vals[0]||'Eintrag';
 }
 async function pb91ImageFiles(moduleName){
   try{return await pbOwnerFiles('standalone',moduleName)}catch(_){return []}
 }
 async function pb91RenderIndependent(name,w,editId=null){
   pb91Store();
   if(name==='Materialien'){
     w.innerHTML=shell('Materialien',`<section class="pb91-independent-intro"><div><h3>Material-Stammdaten</h3><p>Materialien funktionieren unabhängig von Kunden und Projekten. Sie können später in Aufmass und Offerte verwendet werden.</p></div><div><button type="button" class="primary" data-pb91-material-list>Materialien öffnen</button><button type="button" class="secondary" data-pb91-material-new>+ Material erfassen</button></div></section><div class="pb91-info-card"><strong>Zentrale Materialdatenbank</strong><span>Artikelnummer, Bezeichnung, Lieferant, Leistungstext, Kalkulation, Angebotspreis und Foto bleiben zentral gespeichert.</span></div>`,'STAMMDATEN');
     w.querySelector('[data-pb91-material-list]')?.addEventListener('click',()=>pbOpenMaterialManager('list'));
     w.querySelector('[data-pb91-material-new]')?.addEventListener('click',()=>pbOpenMaterialManager('calc'));
     return true;
   }
   const defs=PB91_DEFS[name]||[['Titel','text'],['Beschreibung','textarea']],rows=pb91Store()[name];
   const imageModule=['Baustellenfotos','Fotoberichte','Grundrisse','2D / 3D / CAD'].includes(name);
   if(editId){
     const isNew=editId==='new',rec=isNew?{id:u(),createdAt:new Date().toISOString()}:rows.find(x=>x.id===editId);
     if(!rec)return pb91RenderIndependent(name,w);
     w.innerHTML=shell(name,`<div class="pb91-editor-head"><div><span class="eyebrow">UNABHÄNGIGER BEREICH</span><h3>${isNew?'Neuer Eintrag':pb91Esc(pb91Label(rec,defs))}</h3><p>Dieser Datensatz benötigt weder Kunde noch Projekt.</p></div><div><button type="button" class="secondary" data-pb91-back>← Zurück</button><button type="button" class="primary" data-pb91-save>Speichern</button>${isNew?'':'<button type="button" class="danger" data-pb91-delete>Löschen</button>'}</div></div><div class="pb91-form">${defs.map((d,i)=>pb91Field(d[0],d[1],rec[`f${i}`]||'',i)).join('')}</div>${imageModule?`<section class="pb91-pc-photo"><div><h3>Dateien / Fotos vom PC</h3><p>Fotos und Dateien können direkt vom Computer hinzugefügt werden.</p></div><div data-pb91-files></div></section>`:''}`,'VERWALTUNG');
     // correct selected values
     defs.forEach((d,i)=>{const el=w.querySelector(`[data-pb91-field="f${i}"]`);if(el&&['status','priority'].includes(d[1])&&rec[`f${i}`])el.value=rec[`f${i}`]});
     w.querySelector('[data-pb91-back]')?.addEventListener('click',()=>pb91RenderIndependent(name,w));
     w.querySelector('[data-pb91-save]')?.addEventListener('click',()=>{
       defs.forEach((d,i)=>{rec[`f${i}`]=w.querySelector(`[data-pb91-field="f${i}"]`)?.value||''});
       rec.updatedAt=new Date().toISOString();if(isNew)rows.unshift(rec);save();pbActionToast('Gespeichert.');pb91RenderIndependent(name,w,rec.id);
     });
     w.querySelector('[data-pb91-delete]')?.addEventListener('click',()=>{if(!confirm('Eintrag wirklich löschen?'))return;S.standaloneModules[name]=rows.filter(x=>x.id!==rec.id);save();pb91RenderIndependent(name,w)});
     if(imageModule){
       const host=w.querySelector('[data-pb91-files]');
       pbMountOwnerFiles(host,'standalone',`${name}:${rec.id}`,name==='Baustellenfotos'?'Fotos':'Dateien');
     }
     return true;
   }
   const files=imageModule?await pb91ImageFiles(name):[];
   w.innerHTML=shell(name,`<div class="pb91-list-head"><div><span class="eyebrow">UNABHÄNGIGER BEREICH</span><h3>${pb91Esc(name)}</h3><p>Dieser Bereich arbeitet unabhängig von Kunden und Projekten.</p></div><button type="button" class="primary" data-pb91-new>+ Neuer Eintrag</button></div><div class="pb91-list"><div class="pb91-list-row head"><span>Bezeichnung</span><span>Details</span><span>Geändert</span><span></span></div>${rows.map(r=>`<button type="button" class="pb91-list-row" data-pb91-open="${pb91Esc(r.id)}"><strong>${pb91Esc(pb91Label(r,defs))}</strong><span>${pb91Esc(defs.slice(1,3).map((d,i)=>r[`f${i+1}`]).filter(Boolean).join(' · '))}</span><span>${fmtDate(String(r.updatedAt||r.createdAt||'').slice(0,10))}</span><span>›</span></button>`).join('')||'<p class="pb91-empty">Noch keine Einträge vorhanden.</p>'}</div>${imageModule?`<section class="pb91-pc-upload"><div><h3>Direkt vom PC hinzufügen</h3><p>Foto oder Datei auswählen. Die Datei wird lokal und dauerhaft im Dateispeicher abgelegt.</p></div><button type="button" class="primary" data-pb91-direct-upload>＋ Datei / Foto auswählen</button><input type="file" data-pb91-direct-input ${name==='Baustellenfotos'||name==='Fotoberichte'?'accept="image/*"':''} multiple hidden></section>`:''}`,'VERWALTUNG');
   w.querySelector('[data-pb91-new]')?.addEventListener('click',()=>pb91RenderIndependent(name,w,'new'));
   w.querySelectorAll('[data-pb91-open]').forEach(b=>b.addEventListener('click',()=>pb91RenderIndependent(name,w,b.dataset.pb91Open)));
   if(imageModule){
     const input=w.querySelector('[data-pb91-direct-input]');
     w.querySelector('[data-pb91-direct-upload]')?.addEventListener('click',()=>input?.click());
     input?.addEventListener('change',async()=>{
       for(const file of [...input.files]){
         const id=u(),rec={id,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),f0:file.name,f1:new Date().toISOString().slice(0,10),f2:'',f3:''};
         rows.unshift(rec);
         await pbPutOwnerFile({id:u(),owner:`standalone:${name}:${id}`,ownerType:'standalone',ownerId:`${name}:${id}`,name:file.name,type:file.type||'application/octet-stream',size:file.size,blob:file,lastModified:file.lastModified||Date.now(),createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()});
       }
       save();input.value='';pbActionToast('Datei vom PC gespeichert.');pb91RenderIndependent(name,w);
     });
   }
   return true;
 }
 function render(name,w){
   if(PB91_STANDALONE_MODULES.includes(name)){pb91RenderIndependent(name,w);return true}
   if(name==='Cockpit')w.innerHTML=cockpit();
   else if(name==='Kunden'){document.body.classList.add('pb-customer-mode');pbEnsureCustomerRecords();pbRenderCustomersWorkspace(w)}
   else if(name==='Projektkalender'){document.body.classList.remove('pb-customer-mode');pbRenderProjectCalendarWorkspace(w)}
   else if(name==='Projekte'){w.innerHTML=projectModule();bindProjectModule(w)}
   else if(name==='Einstellungen'){w.innerHTML=shell('Einstellungen',pbRenderSettingsModule(),'UNTERNEHMEN');pbBindSettingsWorkspace(w)}
   else if(defs[name])w.innerHTML=tableModule(name);
   else{
     // Niemals eine leere Maske zeigen: auf die bestehende Hauptansicht zurückfallen.
     document.body.classList.remove('pb-module-mode','pb-customer-mode','pb-shell-focus');
     w.classList.add('hidden');w.innerHTML='';
     const target=document.querySelector(`[data-fallback-module="${name}"]`)||document.querySelector('#projects')||document.querySelector('.card.editor');
     target?.scrollIntoView({block:'start'});
     return false;
   }
   w.querySelector('[data-close-module]')?.addEventListener('click',()=>{document.body.classList.remove('pb-module-mode','pb-customer-mode');w.classList.add('hidden');history.back()});w.querySelectorAll('[data-pb-project]').forEach(b=>b.onclick=()=>{if(typeof pbNavigateToProject==='function')pbNavigateToProject(b.dataset.pbProject)});w.querySelectorAll('[data-pb-module]').forEach(b=>b.onclick=()=>document.querySelector(`[data-module="${b.dataset.pbModule}"]`)?.click());w.querySelectorAll('[data-pb-scroll]').forEach(b=>b.onclick=()=>{document.body.classList.remove('pb-module-mode');w.classList.add('hidden');document.querySelector(b.dataset.pbScroll)?.scrollIntoView({behavior:'smooth'})});return true}
 window.PBERP={render,projectModule,tableModule,cockpit};
 const bindErpNavigation=()=>{
   const w=document.getElementById('pbModuleWorkspace');if(!w||w.dataset.erpBound==='1')return;
   w.dataset.erpBound='1';
   document.querySelectorAll('.cad-nav-item[data-module]').forEach(b=>b.addEventListener('click',ev=>{
     const name=b.dataset.module;
     const ok=render(name,w);
     ev.preventDefault();ev.stopImmediatePropagation();
     if(ok){
       document.querySelectorAll('.cad-nav-item').forEach(x=>x.classList.toggle('active',x===b));
       document.body.classList.remove('pb-customer-mode','pb-shell-focus');
       document.body.classList.add('pb-module-mode');
       w.classList.remove('hidden');
       w.scrollIntoView({block:'start'});
       if(!window.__pbHistoryReplay)pbPushHistory({type:'module',name});
     }else{
       pbShowMainView(false);
     }
   },true));
   const c=document.querySelector('.cad-nav-item.active[data-scroll="#projects"]');
   if(c)c.addEventListener('click',ev=>{
     ev.preventDefault();ev.stopImmediatePropagation();
     document.querySelectorAll('.cad-nav-item').forEach(x=>x.classList.toggle('active',x===c));
     document.body.classList.remove('pb-customer-mode','pb-shell-focus');
     document.body.classList.add('pb-module-mode');
     render('Cockpit',w);w.classList.remove('hidden');
   },true);
 };

 function pbBaseUrl(){
   return location.pathname+(location.search&&location.search.startsWith('?v=')?'':location.search);
 }
 function pbPushHistory(state){
   try{
     const tag=state.type==='module'?`#${encodeURIComponent(state.name)}`:
               state.type==='project'?`#Projekt-${encodeURIComponent(state.projectId||'')}`:
               state.type==='scroll'?`#${encodeURIComponent(state.target||'')}`:'#cockpit';
     history.pushState({pb:true,...state},'',location.pathname+location.search+tag);
   }catch(_){}
 }
 function pbShowMainView(push=false){
   const w=document.getElementById('pbModuleWorkspace');
   document.body.classList.remove('pb-module-mode','pb-customer-mode','pb-shell-focus');
   if(w){w.classList.add('hidden');w.innerHTML=''}
   document.querySelectorAll('.cad-nav-item').forEach(x=>x.classList.remove('active'));
   const cockpit=document.querySelector('.cad-nav-item[data-scroll="#projects"]');
   cockpit?.classList.add('active');
   document.querySelector('#projects')?.scrollIntoView({block:'start'});
   if(push&&!window.__pbHistoryReplay)pbPushHistory({type:'main'});
 }
 function pbReplayHistory(state){
   window.__pbHistoryReplay=true;
   try{
     if(!state||!state.pb||state.type==='main'){pbShowMainView(false);return}
     if(state.type==='project'){pbNavigateToProject(state.projectId);return}
     if(state.type==='module'){
       const b=[...document.querySelectorAll('.cad-nav-item[data-module]')].find(x=>x.dataset.module===state.name);
       if(b){b.click();return}
       pbShowMainView(false);return;
     }
     if(state.type==='scroll'){
       pbShowMainView(false);
       const target=document.querySelector(state.target);
       target?.scrollIntoView({block:'start'});
       const b=[...document.querySelectorAll('.cad-nav-item[data-scroll]')].find(x=>x.dataset.scroll===state.target);
       document.querySelectorAll('.cad-nav-item').forEach(x=>x.classList.toggle('active',x===b));
       return;
     }
   }finally{
     setTimeout(()=>{window.__pbHistoryReplay=false},0);
   }
 }
 function pbBindHistoryNavigation(){
   if(window.__pbHistoryBound)return;window.__pbHistoryBound=true;
   try{
     if(!history.state?.pb)history.replaceState({pb:true,type:'main'},'',location.pathname+location.search+'#cockpit');
   }catch(_){}
   // Sidebar scroll entries get real browser history without page navigation.
   document.querySelectorAll('.cad-nav-item[data-scroll]').forEach(btn=>{
     btn.addEventListener('click',()=>{
       const target=btn.dataset.scroll;
       if(!window.__pbHistoryReplay)pbPushHistory({type:'scroll',target});
     },false);
   });
   window.addEventListener('popstate',e=>pbReplayHistory(e.state));
 }

 if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{bindErpNavigation();pbBindHistoryNavigation()},{once:true});
 else {bindErpNavigation();pbBindHistoryNavigation()}
})();


/* ============================================================
   2.9.88 – EINFACHE ORIENTIERUNG & GLOBALE SUCHE
   Ein Suchfeld für Kunde, Projekt und verknüpfte Daten.
   Keine Datenkopien; Treffer stammen aus dem zentralen Datenmodell.
   ============================================================ */
(()=>{
  const esc88=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  function pb88CustomerName(c){
    try{return typeof pbCustomerName==='function'?pbCustomerName(c):(c?.company||[c?.firstName,c?.lastName].filter(Boolean).join(' '))}catch(_){return c?.company||''}
  }
  function pb88Results(q){
    try{pbSynchronizeAllData?.()}catch(_){}
    const needle=String(q||'').trim().toLocaleLowerCase('de-CH');
    if(needle.length<2)return [];
    const out=[];
    const add=(type,title,sub,id,customerId,searchText)=>{
      const hay=String(searchText||`${title} ${sub}`).toLocaleLowerCase('de-CH');
      if(hay.includes(needle))out.push({type,title,sub,id,customerId});
    };
    (S.customers||[]).forEach(c=>add('Kunde',pb88CustomerName(c)||'Kunde',
      [c.number,c.email,c.phone,c.city].filter(Boolean).join(' · '),c.id,c.id,
      Object.values(c||{}).filter(v=>typeof v==='string'||typeof v==='number').join(' ')));
    (S.projects||[]).forEach(p=>{
      add('Projekt',p.name||'Projekt',[p.projectNo,p.customer,p.address,p.status].filter(Boolean).join(' · '),p.id,p.customerId,
        Object.values(p||{}).filter(v=>typeof v==='string'||typeof v==='number').join(' '));
      const linked=[
        ['Termin',p.appointments],['Aufgabe',p.projectTasks],['Auftrag',p.orders],
        ['Regiebericht',p.regieReports],['Rechnung',p.invoices],['Zahlung',p.payments],
        ['Vertrag',p.contracts],['Brief',p.letters],['E-Mail',p.sentEmails]
      ];
      linked.forEach(([type,rows])=>(rows||[]).forEach(r=>add(type,r.title||r.number||r.invoiceNo||r.offerNo||type,
        [p.name,p.customer,r.date,r.status,r.total].filter(v=>v!==undefined&&v!=='').join(' · '),p.id,p.customerId,
        Object.values(r||{}).filter(v=>typeof v==='string'||typeof v==='number').join(' ')+' '+p.name+' '+p.customer)));
      const pd=p.plattenleger?.phaseData||{};
      if(pd.offerte)add('Offerte',pd.offerte.offerNo||pd.offerte.number||'Offerte',[p.name,p.customer,pd.offerte.total].filter(Boolean).join(' · '),p.id,p.customerId,JSON.stringify(pd.offerte)+' '+p.name+' '+p.customer);
      if(pd.rechnung)add('Rechnung',pd.rechnung.invoiceNo||pd.rechnung.number||'Rechnung',[p.name,p.customer,pd.rechnung.total].filter(Boolean).join(' · '),p.id,p.customerId,JSON.stringify(pd.rechnung)+' '+p.name+' '+p.customer);
    });
    return out.slice(0,10);
  }
  function pb88OpenResult(r){
    const search=document.getElementById('pbGlobalSearch');
    if(search)search.value='';
    document.getElementById('pb88SearchResults')?.classList.add('hidden');
    if(r.type==='Kunde'){
      const btn=document.querySelector('.cad-nav-item[data-module="Kunden"]');btn?.click();
      setTimeout(()=>{
        const customer=(S.customers||[]).find(c=>c.id===r.customerId);
        const w=document.getElementById('pbModuleWorkspace');
        if(customer&&w&&typeof pbRenderCustomerEditorV2958==='function')pbRenderCustomerEditorV2958(w,customer);
      },50);
      return;
    }
    if(r.id&&typeof pbNavigateToProject==='function'){pbNavigateToProject(r.id);return}
  }
  function pb88BindSearch(){
    const input=document.getElementById('pbGlobalSearch');
    if(!input||input.dataset.pb88==='1')return;
    input.dataset.pb88='1';
    const box=document.createElement('div');
    box.id='pb88SearchResults';box.className='pb88-search-results hidden';
    input.parentElement?.appendChild(box);
    const render=()=>{
      const q=input.value.trim(),rows=pb88Results(q);
      if(q.length<2){box.classList.add('hidden');box.innerHTML='';return}
      box.innerHTML=rows.length?rows.map((r,i)=>`<button type="button" data-pb88-result="${i}">
        <span class="pb88-type">${esc88(r.type)}</span><span class="pb88-result-main"><strong>${esc88(r.title)}</strong><small>${esc88(r.sub||'')}</small></span><span class="pb88-arrow">›</span>
      </button>`).join(''):`<div class="pb88-no-result"><strong>Keine Treffer</strong><span>Versuchen Sie einen Kundennamen, eine Projektnummer oder Rechnungsnummer.</span></div>`;
      box.classList.remove('hidden');
      box.querySelectorAll('[data-pb88-result]').forEach(b=>b.addEventListener('click',()=>pb88OpenResult(rows[Number(b.dataset.pb88Result)])));
    };
    input.addEventListener('input',render);
    input.addEventListener('focus',render);
    input.addEventListener('keydown',e=>{
      if(e.key==='Escape'){input.value='';box.classList.add('hidden')}
      if(e.key==='Enter'){const first=pb88Results(input.value)[0];if(first)pb88OpenResult(first)}
    });
    document.addEventListener('pointerdown',e=>{if(!input.parentElement?.contains(e.target))box.classList.add('hidden')});
  }
  function pb88BindHome(){
    const top=document.querySelector('.cad-topbar-title');
    if(!top||top.dataset.pb88==='1')return;
    top.dataset.pb88='1';top.title='Zum Cockpit';top.tabIndex=0;top.setAttribute('role','button');
    const go=()=>document.querySelector('.cad-nav-item[data-scroll="#projects"]')?.click();
    top.addEventListener('click',go);top.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' ')go()});
  }
  const init=()=>{pb88BindSearch();pb88BindHome()};
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init,{once:true});else init();
})();


/* ============================================================
   2.9.89 – BAUDOKU: IDEEN- & PROJEKTDOKUMENTATION
   Erstgespräch -> Projektidee -> Fotoseiten -> PDF
   Maximal 3 Fotos pro Seite. Originalfoto bleibt als editorBase erhalten.
   ============================================================ */
function pbBdEsc(v){return String(v??'').replace(/[&<>\"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[c]))}
function pbBdData(p){
  p.baudoku=p.baudoku||{};const b=p.baudoku;
  b.cover=b.cover||{};b.pages=Array.isArray(b.pages)?b.pages:[];
  if(!b.cover.firstMeeting)b.cover.firstMeeting=p.startDate||new Date().toISOString().slice(0,10);
  if(!b.cover.projectIdea)b.cover.projectIdea='';if(!b.cover.customerWishes)b.cover.customerWishes='';
  if(!b.cover.existingSituation)b.cover.existingSituation='';if(!b.cover.solutions)b.cover.solutions='';
  if(!b.cover.materialColor)b.cover.materialColor='';if(!b.cover.openPoints)b.cover.openPoints='';if(!b.cover.nextSteps)b.cover.nextSteps='';
  return b;
}
function pbBdPage(){return{id:'bdp_'+Date.now()+'_'+Math.random().toString(36).slice(2,7),title:'Neue Dokumentationsseite',category:'Projektidee',date:new Date().toISOString().slice(0,10),note:'',decision:'',photos:[]}}
function pbBdPhoto(file,cb){const r=new FileReader();r.onload=()=>cb({id:'bdph_'+Date.now()+'_'+Math.random().toString(36).slice(2,7),title:file.name.replace(/\.[^.]+$/,''),note:'',data:r.result,image:r.result,editorBase:r.result,editorObjects:[],date:new Date().toISOString()});r.readAsDataURL(file)}
function pbBdCoverField(k,label,v,rows=3){return `<label><span>${label}</span><textarea data-bd-cover="${k}" rows="${rows}" placeholder="${label} …">${pbBdEsc(v||'')}</textarea></label>`}
function pbRenderBaudoku(target,p){
  if(!target||!p)return;const b=pbBdData(p),c=(S.customers||[]).find(x=>x.id===p.customerId)||{};
  target.innerHTML=`<div class="pb-bd">
    <div class="pb-bd-toolbar"><div><span class="eyebrow">PROJEKTGEDÄCHTNIS</span><h3>Baudoku</h3><p>Ideen, Kundenwünsche, Fotos und Entscheide vom Erstgespräch bis zur Abnahme.</p></div><div><button type="button" data-bd-add-page>＋ Neue Seite</button><button type="button" class="primary" data-bd-pdf>PDF erstellen</button></div></div>
    <section class="pb-bd-cover">
      <div class="pb-bd-cover-head"><div><span class="eyebrow">SEITE 1</span><h3>Projektidee / Erstgespräch</h3></div><span class="pb-bd-status">Ideenphase</span></div>
      <div class="pb-bd-master"><label>Projekt<input value="${pbBdEsc(p.name||'')}" disabled></label><label>Projekt-Nr.<input value="${pbBdEsc(p.projectNo||'')}" disabled></label><label>Kunde<input value="${pbBdEsc(c.company||p.customer||[c.firstName,c.lastName].filter(Boolean).join(' '))}" disabled></label><label>Objektadresse<input value="${pbBdEsc(p.address||'')}" disabled></label><label>Datum Erstgespräch<input type="date" data-bd-first-meeting value="${pbBdEsc(b.cover.firstMeeting||'')}"></label><label>Projektleiter<input value="${pbBdEsc(p.owner||p.projectManager||'')}" disabled></label></div>
      <div class="pb-bd-cover-grid">${pbBdCoverField('projectIdea','Projektidee',b.cover.projectIdea,5)}${pbBdCoverField('customerWishes','Kundenwünsche',b.cover.customerWishes,5)}${pbBdCoverField('existingSituation','Bestehende Situation',b.cover.existingSituation)}${pbBdCoverField('solutions','Besprochene Lösungen',b.cover.solutions)}${pbBdCoverField('materialColor','Material / Farbe',b.cover.materialColor)}${pbBdCoverField('openPoints','Offene Punkte / Abklärungen',b.cover.openPoints)}${pbBdCoverField('nextSteps','Nächste Schritte',b.cover.nextSteps)}</div>
    </section>
    <div class="pb-bd-pages">${b.pages.map((pg,i)=>pbBdPageHtml(pg,i)).join('')||'<div class="pb-bd-empty"><strong>Noch keine Fotoseite</strong><span>Mit „Neue Seite“ beginnt die visuelle Projektdokumentation.</span></div>'}</div>
  </div>`;
  target.querySelectorAll('[data-bd-cover]').forEach(el=>el.addEventListener('change',()=>{b.cover[el.dataset.bdCover]=el.value;save()}));
  target.querySelector('[data-bd-first-meeting]')?.addEventListener('change',e=>{b.cover.firstMeeting=e.target.value;save()});
  target.querySelector('[data-bd-add-page]')?.addEventListener('click',()=>{b.pages.push(pbBdPage());save();pbRenderBaudoku(target,p)});
  target.querySelector('[data-bd-pdf]')?.addEventListener('click',()=>pbBdPdf(p));
  pbBdBindPages(target,p,b);
}
function pbBdPageHtml(pg,i){
  const cats=['Erstgespräch','Bestand','Projektidee','Kundenwunsch','Material / Farbe','Detail','Aufmass','Ausführung','Änderung','Mangel','Abnahme'];
  return `<section class="pb-bd-page" data-bd-page="${pbBdEsc(pg.id)}"><div class="pb-bd-page-head"><div><span class="eyebrow">SEITE ${i+2}</span><input class="pb-bd-page-title" data-bd-title value="${pbBdEsc(pg.title||'')}"></div><div><button data-bd-up title="Nach oben">↑</button><button data-bd-down title="Nach unten">↓</button><button class="danger" data-bd-delete title="Seite löschen">Löschen</button></div></div>
  <div class="pb-bd-page-meta"><label>Kategorie<select data-bd-category>${cats.map(x=>`<option ${x===pg.category?'selected':''}>${x}</option>`).join('')}</select></label><label>Datum<input type="date" data-bd-date value="${pbBdEsc(pg.date||'')}"></label></div>
  <div class="pb-bd-photos">${(pg.photos||[]).map((ph,j)=>`<article class="pb-bd-photo"><div class="pb-bd-photo-image"><img src="${ph.image||ph.data}"><span>${j+1}</span></div><label>Titel<input data-bd-photo-title="${pbBdEsc(ph.id)}" value="${pbBdEsc(ph.title||'')}"></label><label>Beschreibung<textarea rows="4" data-bd-photo-note="${pbBdEsc(ph.id)}">${pbBdEsc(ph.note||'')}</textarea></label><div><button data-bd-edit-photo="${pbBdEsc(ph.id)}">✎ Foto bearbeiten</button><button class="danger" data-bd-delete-photo="${pbBdEsc(ph.id)}">Entfernen</button></div></article>`).join('')}${(pg.photos||[]).length<3?`<label class="pb-bd-add-photo"><input type="file" accept="image/*" data-bd-photo-input><strong>＋ Foto hinzufügen</strong><span>${3-(pg.photos||[]).length} von 3 Plätzen frei</span></label>`:''}</div>
  <div class="pb-bd-notes"><label>Seitennotiz<textarea rows="3" data-bd-note>${pbBdEsc(pg.note||'')}</textarea></label><label>Kundenentscheid / Gesprächsnotiz<textarea rows="3" data-bd-decision placeholder="z. B. Variante B gewählt …">${pbBdEsc(pg.decision||'')}</textarea></label></div></section>`
}
function pbBdBindPages(target,p,b){
  target.querySelectorAll('[data-bd-page]').forEach(sec=>{const pg=b.pages.find(x=>x.id===sec.dataset.bdPage);if(!pg)return;
    const bind=(sel,key)=>sec.querySelector(sel)?.addEventListener('change',e=>{pg[key]=e.target.value;pg.updatedAt=new Date().toISOString();save()});
    bind('[data-bd-title]','title');bind('[data-bd-category]','category');bind('[data-bd-date]','date');bind('[data-bd-note]','note');bind('[data-bd-decision]','decision');
    sec.querySelector('[data-bd-photo-input]')?.addEventListener('change',e=>{const files=[...e.target.files].slice(0,3-(pg.photos||[]).length);files.forEach(f=>pbBdPhoto(f,ph=>{pg.photos=pg.photos||[];pg.photos.push(ph);save();pbRenderBaudoku(target,p)}))});
    sec.querySelectorAll('[data-bd-photo-title]').forEach(el=>el.addEventListener('change',()=>{const ph=pg.photos.find(x=>x.id===el.dataset.bdPhotoTitle);if(ph){ph.title=el.value;save()}}));
    sec.querySelectorAll('[data-bd-photo-note]').forEach(el=>el.addEventListener('change',()=>{const ph=pg.photos.find(x=>x.id===el.dataset.bdPhotoNote);if(ph){ph.note=el.value;save()}}));
    sec.querySelectorAll('[data-bd-edit-photo]').forEach(btn=>btn.addEventListener('click',()=>{const ph=pg.photos.find(x=>x.id===btn.dataset.bdEditPhoto);if(ph&&window.ProjectBauPhotoEditor?.openExisting){window.ProjectBauPhotoEditor.openExisting(ph,null)}else pbActionToast('Fotoeditor wird geladen.')}));
    sec.querySelectorAll('[data-bd-delete-photo]').forEach(btn=>btn.addEventListener('click',()=>{if(!confirm('Foto aus dieser Baudoku-Seite entfernen?'))return;pg.photos=pg.photos.filter(x=>x.id!==btn.dataset.bdDeletePhoto);save();pbRenderBaudoku(target,p)}));
    sec.querySelector('[data-bd-delete]')?.addEventListener('click',()=>{if(!confirm('Diese Baudoku-Seite wirklich löschen?'))return;b.pages=b.pages.filter(x=>x.id!==pg.id);save();pbRenderBaudoku(target,p)});
    sec.querySelector('[data-bd-up]')?.addEventListener('click',()=>{const i=b.pages.indexOf(pg);if(i>0){[b.pages[i-1],b.pages[i]]=[b.pages[i],b.pages[i-1]];save();pbRenderBaudoku(target,p)}});
    sec.querySelector('[data-bd-down]')?.addEventListener('click',()=>{const i=b.pages.indexOf(pg);if(i<b.pages.length-1){[b.pages[i+1],b.pages[i]]=[b.pages[i],b.pages[i+1]];save();pbRenderBaudoku(target,p)}});
  });
}
function pbBdPdf(p){
  const J=window.jspdf?.jsPDF;if(!J){alert('PDF-Modul ist noch nicht geladen.');return}const b=pbBdData(p),c=(S.customers||[]).find(x=>x.id===p.customerId)||{};
  const doc=new J({unit:'mm',format:'a4'}),W=210,H=297,M=16,navy=[18,48,74],muted=[100,116,139];let page=0;
  const foot=()=>{doc.setDrawColor(215,224,231);doc.line(M,H-13,W-M,H-13);doc.setFontSize(7);doc.setTextColor(...muted);doc.text(`Projekt ${p.projectNo||p.name||''} | Ceramica Naturo`,M,H-8);doc.text(`Seite ${page}`,W-M,H-8,{align:'right'})};
  const newPage=()=>{if(page){foot();doc.addPage()}page++};const text=(s,x,y,w,size=9,bold=false)=>{doc.setFont('helvetica',bold?'bold':'normal');doc.setFontSize(size);doc.setTextColor(38,59,74);const lines=doc.splitTextToSize(String(s||'—'),w);doc.text(lines,x,y);return y+lines.length*(size*.42)+2};
  newPage();doc.setTextColor(...navy);doc.setFont('helvetica','bold');doc.setFontSize(22);doc.text('PROJEKTDOKUMENTATION',M,24);doc.setFont('helvetica','normal');doc.setFontSize(8);doc.setTextColor(...muted);doc.text('Erstgespräch | Projektidee | Kundenwünsche | Fotodokumentation',M,31);
  let y=43;[['Projekt',p.name],['Projekt-Nr.',p.projectNo],['Kunde',c.company||p.customer||[c.firstName,c.lastName].filter(Boolean).join(' ')],['Objektadresse',p.address],['Datum Erstgespräch',b.cover.firstMeeting],['Projektleiter',p.owner||p.projectManager]].forEach(([k,v])=>{doc.setFont('helvetica','bold');doc.setFontSize(8);doc.setTextColor(...navy);doc.text(k,M,y);doc.setFont('helvetica','normal');doc.setTextColor(38,59,74);doc.text(String(v||'—'),60,y);y+=8});y+=4;
  [['Projektidee',b.cover.projectIdea],['Kundenwünsche',b.cover.customerWishes],['Bestehende Situation',b.cover.existingSituation],['Besprochene Lösungen',b.cover.solutions],['Material / Farbe',b.cover.materialColor],['Offene Punkte / Abklärungen',b.cover.openPoints],['Nächste Schritte',b.cover.nextSteps]].forEach(([k,v])=>{if(y>250){newPage();y=24}doc.setFont('helvetica','bold');doc.setFontSize(10);doc.setTextColor(...navy);doc.text(k,M,y);y=text(v,M,y+5,W-2*M,8,false)+5});
  b.pages.forEach((pg,pi)=>{newPage();doc.setFont('helvetica','bold');doc.setFontSize(15);doc.setTextColor(...navy);doc.text(`${String(pi+1).padStart(2,'0')} - ${pg.title||'Dokumentationsseite'}`,M,22);doc.setFont('helvetica','normal');doc.setFontSize(7);doc.setTextColor(...muted);doc.text(`${pg.date||''} | Kategorie: ${pg.category||''}`,M,28);const photos=(pg.photos||[]).slice(0,3);let yy=35;
    photos.forEach((ph,i)=>{const img=ph.image||ph.data;if(!img)return;const ih=photos.length===1?92:photos.length===2?72:54;try{doc.addImage(img,'JPEG',M,yy,104,ih,undefined,'FAST')}catch(_){try{doc.addImage(img,'PNG',M,yy,104,ih,undefined,'FAST')}catch(__){}}doc.setFont('helvetica','bold');doc.setFontSize(8);doc.setTextColor(...navy);doc.text(`${i+1}. ${ph.title||'Foto'}`,124,yy+4);text(ph.note||'',124,yy+10,70,8,false);yy+=ih+8});
    if(pg.note){doc.setFont('helvetica','bold');doc.setFontSize(8);doc.text('Seitennotiz',M,yy);yy=text(pg.note,M,yy+5,W-2*M,8)+4}if(pg.decision){doc.setFont('helvetica','bold');doc.setFontSize(8);doc.setTextColor(...navy);doc.text('Kundenentscheid / Gesprächsnotiz',M,yy);text(pg.decision,M,yy+5,W-2*M,8)}
  });foot();doc.save(`Baudoku_${String(p.projectNo||p.name||'Projekt').replace(/[^a-z0-9äöüß_-]+/gi,'_')}.pdf`);
}


/* ============================================================
   2.9.91 – BUTTON-ROUTING & PC-UPLOAD SICHERUNG
   Repariert zentrale Navigationsbuttons und verhindert leere Module.
   ============================================================ */
(()=>{
  function pb91RepairRoutes(){
    const aliases={
      'Fotoberichte':'Fotoberichte','Foto-Berichte':'Fotoberichte','Baustellenfotos':'Baustellenfotos',
      'Grundrisse':'Grundrisse','2D / 3D CAD':'2D / 3D / CAD','2D / 3D / CAD':'2D / 3D / CAD',
      'Materialien':'Materialien','Leistungen':'Leistungen','Mitarbeiter':'Mitarbeiter','Textbausteine':'Textbausteine',
      'Formular Editor':'Formular Editor','Bestand':'Bestand','Verträge':'Verträge','Arbeitszeiten':'Arbeitszeiten',
      'Meine Aufträge':'Meine Aufträge','Berichte':'Berichte','Bautagebücher':'Bautagebücher','Regieberichte':'Regieberichte',
      'Einsatzplanung':'Einsatzplanung','Projektaufgaben':'Projektaufgaben'
    };
    document.querySelectorAll('.cad-nav-item').forEach(btn=>{
      const txt=String(btn.textContent||'').trim();
      if(!btn.dataset.module&&aliases[txt]){btn.dataset.module=aliases[txt];btn.removeAttribute('data-scroll')}
    });
  }
  function pb91AuditButtons(){
    pb91RepairRoutes();
    const nav=[...document.querySelectorAll('.cad-nav-item')];
    const bad=nav.filter(b=>!b.dataset.module&&!b.dataset.scroll&&!b.id);
    if(bad.length)console.warn('Projekt Bau 2.9.91: Navigationsbuttons ohne Route',bad);
    return {navigation:nav.length,withoutRoute:bad.length};
  }
  window.ProjectBauButtonAudit=pb91AuditButtons;
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(pb91AuditButtons,0),{once:true});else setTimeout(pb91AuditButtons,0);
})();
