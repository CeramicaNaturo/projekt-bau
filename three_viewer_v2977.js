...
function makeWallBoxSegment(w,objects,t0,t1,height,material,y0=0,extendStart=false,extendEnd=false){
  ...
  // v1.9.25: Wandsegmente laufen nicht durch die Ecke.
  // Ecken werden mit einer separaten Gehrungsfüllgeometrie geschlossen.
  let extraStart=0;
  let extraEnd=0;
  const centerShift=0;
...
function fitTopCamera(objects){
  if(!camera||!controls)return;
  const pts=buildPolygon(objects);
  if(!pts||pts.length<3)return;
  const xs=pts.map(p=>p.x),zs=pts.map(p=>p.y),minX=Math.min(...xs),maxX=Math.max(...xs),minZ=Math.min(...zs),maxZ=Math.max(...zs);
  const cx=(minX+maxX)/2,cz=(minZ+maxZ)/2,width=Math.max(.5,maxX-minX),depth=Math.max(.5,maxZ-minZ);
  const vfov=THREE.MathUtils.degToRad(camera.fov||42),aspect=Math.max(.25,camera.aspect||1);
  const distance=Math.max(depth/(2*Math.tan(vfov/2)),width/(2*Math.tan(vfov/2)*aspect))*1.18;
  controls.target.set(cx,0,cz);camera.up.set(0,0,-1);camera.position.set(cx,Math.max(2,distance),cz);
  camera.near=.01;camera.far=Math.max(100,distance*20);camera.updateProjectionMatrix();
  controls.minPolarAngle=0;controls.maxPolarAngle=Math.PI;controls.update();
}

function fitWallCamera(wall,objects){
  if(!camera||!controls||!wall)return;
  const f=wallFrame3D(wall,objects),roomH=Number(currentData?.record?.roomHeightM)||2.4;
  const vfov=THREE.MathUtils.degToRad(camera.fov||42),aspect=Math.max(.25,camera.aspect||1);
  const distance=Math.max(roomH/(2*Math.tan(vfov/2)),f.len/(2*Math.tan(vfov/2)*aspect))*1.18;
  const tx=f.x1+f.dx*.5+f.inx*f.th,tz=f.z1+f.dz*.5+f.inz*f.th;
  controls.target.set(tx,roomH/2,tz);camera.up.set(0,1,0);camera.position.set(tx+f.inx*distance,roomH/2,tz+f.inz*distance);
  camera.near=.01;camera.far=Math.max(100,distance*20);camera.updateProjectionMatrix();
  controls.minPolarAngle=THREE.MathUtils.degToRad(5);controls.maxPolarAngle=THREE.MathUtils.degToRad(175);controls.update();
}

function applyCameraMode(objects){
  const selectedId=currentData?.options?.wallViewSelectedId;
  if(selectedId){
    const wall=(objects||[]).find(o=>o.type==='wall'&&o.id===selectedId);
    fitWallCamera(wall,objects);return;
  }
  if(cameraMode==='top')fitTopCamera(objects);else fitReferenceCamera(objects);
}
...
function rebuild(){
  ...
  const selectedWallId=options?.wallViewSelectedId||'';
  const belongsToSelectedWall=o=>{
    if(!selectedWallId)return true;
    if(o.wallId)return o.wallId===selectedWallId;
    const hit=nearestWallForOpening3D(o,objects);
    return hit?.wall?.id===selectedWallId;
  };

  if(layerVisibility.walls!==false){
    (objects||[]).filter(o=>o.type==='wall'&&(!selectedWallId||o.id===selectedWallId)).forEach(w=>{
    ...
    if(!selectedWallId)addCleanWallCorners(rootGroup,objects,roomHeight,wallMat);
  }

  (objects||[]).filter(o=>o.type!=='wall'&&o.type!=='text'&&visible3D(o)&&belongsToSelectedWall(o)).forEach(o=>{
...
window.ProjectBau3D={
  open(container,data){
    ...
    currentData=data;
    cameraMode=data?.options?.wallViewSelectedId?'wall':'normal';
    resize();
    rebuild();
    requestAnimationFrame(()=>{
      resize();
      applyCameraMode(currentData?.objects||[]);
    });
  },
  update(data){
    currentData=data;
  ...
  topView(){cameraMode='top';resize();fitTopCamera(currentData?.objects||[])},
  normalView(){cameraMode='normal';resize();fitReferenceCamera(currentData?.objects||[])},
  wallView(wallId){
    if(currentData){currentData.options={...(currentData.options||{}),wallViewSelectedId:wallId||''};cameraMode=wallId?'wall':'normal';rebuild()}
  },
  resize
};
